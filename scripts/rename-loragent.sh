#!/usr/bin/env bash
# rename-loragent.sh — Replace all "loragent" references with a new agent name.
# Usage: bash scripts/rename-loragent.sh "alexgent"
set -euo pipefail

OLD="loragent"
NEW="${1:?Usage: $0 <newname> (e.g., alexgent)}"
ROOT="${2:-$HOME/.loragent}"
...
# 2. Capitalized version (Loragent → Alexgent)
CAP_OLD="Loragent"
...
# 3. Rename the directory if $ROOT is ~/.loragent
if [ "$ROOT" = "$HOME/.loragent" ] && [ "$OLD" = "loragent" ]; then
  NEW_ROOT="$HOME/.$NEW"
