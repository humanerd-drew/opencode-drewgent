...
Auth: MINIMAX_API_KEY (~/.loragent/.env)

출력: model별 (general, video) 현재 5h interval + 주간 사용량/리셋까지 남은 시간.

사용법:
    ~/.loragent/scripts/minimax_usage.py            # 컬러 출력
    ~/.loragent/scripts/minimax_usage.py --json     # machine-readable
    ~/.loragent/scripts/minimax_usage.py --watch 30 # 30초마다 refresh (Ctrl-C 종료)
"""
...
ENV_PATH = Path.home() / ".loragent" / ".env"
DEFAULT_CACHE_PATH = Path.home() / ".loragent" / "cache" / "minimax_usage.json"
...
def load_api_key() -> str | None:
    """Read MINIMAX_API_KEY from ~/.loragent/.env (no shell sourcing needed)."""
...
def main() -> int:
    ...
    if not api_key:
        print(red("error: MINIMAX_API_KEY not found in ~/.loragent/.env"), file=sys.stderr)
        return 1
