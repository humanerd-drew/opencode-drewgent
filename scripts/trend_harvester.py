...
_P4_TREND = _DREWGENT_HOME / "@memory" / "growth" / "trend-harvester"
...
def write_collected_index(base_dir: Path) -> None:
    """Generate collected/index.md for Obsidian graph visibility.
    Lists recent keeps and links to evaluated/ reports.
    """
    collected_dir = base_dir / "collected"
    evaluated_dir = base_dir / "evaluated"
    analyzed_keep_dir = base_dir / "analyzed" / "keep"
    report_file = base_dir / "report.json"

    # Read latest report data
    report = {}
    ...
    # Find evaluated/ .md files (they link out to graph)
    eval_md_files = sorted(evaluated_dir.glob("*.md"),
                           key=lambda f: f.stat().st_mtime, reverse=True)[:5]

    # Frontmatter links
    if eval_md_files:
        fm_links = "\n".join(f'  - "[[{f.stem}]]"'
                             for f in eval_md_files)
        links_block = f"links:\n{fm_links}"
    else:
        links_block = ""

    generated_at = report.get("generated_at", now.isoformat())
    total = report.get("total_collected", 0)
    keep = report.get("keep", 0)
    review = report.get("review", 0)
    graveyard = report.get("graveyard", 0)
    top_keeps = report.get("top_keeps", [])

    index_md = f"""---
title: "Trend Harvester — Collected Index"
type: index
space: growth
{links_block}
---
...
Last collection: `{generated_at}` | Total collected in last run: {total}

## Run Stats

| Keep | Review | Graveyard |
|------|--------|-----------|
| {keep} | {review} | {graveyard} |

## Top Scoring Items

| Item | Source | Score | Evaluation |
|------|--------|-------|------------|
"""
    ...
    for k in top_keeps:
        ...
        source = k.get("source", "?")
        score = k.get("score", 0)
        index_md += f"| [{name}]({url}) | {source} | {score} | TBD |\n"
    ...
    index_md += "\n---\n"
    index_md += f"_Auto-generated by trend_harvester.py at {now.isoformat()}_\n"

    index_file = collected_dir / "index.md"
    if not _DRY_RUN:
        index_file.write_text(index_md)
        log(f"Collected index written: {index_file}")
    else:
        log(f"[DRY-RUN] Collected index would be written: {index_file}")
...
def main() -> int:
    ...
    try:
        ...
        if not _DRY_RUN:
            ...
            write_collected_index(_P4_TREND)
