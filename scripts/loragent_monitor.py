...
# History storage
STATE_FILE = "/tmp/loragent_monitor_state.json"
...
def send_discord(message: str) -> bool:
    ...
    try:
        payload = {"content": message, "username": "Loragent Monitor"}
...
def main():
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Loragent Monitor started")
    ...
    while True:
        ...
        lines = [
            f"⏰ **{now.strftime('%Y-%m-%d %H:%M')} Loragent 상태 보고**",
            "",
        ]
