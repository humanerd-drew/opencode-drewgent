#!/usr/bin/env python3
"""
dispatch_once equivalent for content board kanban tasks.
...
ready_count = conn.execute('''
    ...
    WHERE status = "ready" AND (board = "content" OR board = "" OR board IS NULL)
''').fetchone()[0] or 0

running_count = conn.execute('''
    SELECT COUNT(*) FROM tasks
    WHERE status = "in_progress" AND (board = "content" OR board = "" OR board IS NULL)
''').fetchone()[0] or 0
...
stale = conn.execute('''
    ...
    WHERE status = "in_progress"
      ...
      AND claim_expires < ?
      AND (board = "content" OR board = "" OR board IS NULL)
    ORDER BY claim_expires ASC
''', (now_ts,)).fetchall()

# Reclaim to 'ready' so next dispatch can pick them up (not 'todo')
...
ready = conn.execute('''
    ...
    WHERE (board = "content" OR board = "" OR board IS NULL)
      AND status = "ready"
