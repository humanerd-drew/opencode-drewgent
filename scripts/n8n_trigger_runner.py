...
if TRIGGER == 'trend-evaluate':
    keep = check_dir(os.path.join(HOME, '.loragent/P4-cortex/growth/trend-harvester/analyzed/keep'))
    ev = check_dir(os.path.join(HOME, '.loragent/P4-cortex/growth/trend-harvester/evaluated'))
    ni = [f for f in keep if f not in set(ev)]
    print(f'{len(ni)}' if ni else 'silent')

elif TRIGGER == 'trend-retire':
    try:
        rpt = json.load(open(os.path.join(HOME, '.loragent/P4-cortex/growth/trend-harvester/.usage_report.json')))
    ...
    keep = check_dir(os.path.join(HOME, '.loragent/P4-cortex/growth/trend-harvester/analyzed/keep'))
    ap = check_dir(os.path.join(HOME, '.loragent/P4-cortex/growth/trend-harvester/applied'))
    ni = [f for f in keep if f not in set(ap)]
    print(f'{len(ni)}' if ni else 'silent')

elif TRIGGER == 'seo-analyze':
    d = os.path.join(HOME, '.loragent/P2-hippocampus/knowledge/seo-articles')
    ...
    d = os.path.join(HOME, '.loragent/P4-cortex/growth/seo/analyzed')
