...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/{{AGENT_NAME_LOWER}}-agent.git"
REPO_URL_HTTPS="https://github.com/NousResearch/{{AGENT_NAME_LOWER}}-agent.git"
HERMES_HOME="$HOME/.{{AGENT_NAME_LOWER}}"
INSTALL_DIR="${HERMES_INSTALL_DIR:-$HERMES_HOME/{{AGENT_NAME_LOWER}}-agent}"
PYTHON_VERSION="3.11"
NODE_VERSION="22"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --dir PATH     Installation directory (default: ~/.{{AGENT_NAME_LOWER}}/{{AGENT_NAME_LOWER}}-agent)"
...
print_banner() {
    ...
    echo "│             ⚕ {{AGENT_NAME}} Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by YOUR_ORG.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/NousResearch/{{AGENT_NAME_LOWER}}-agent/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$HERMES_HOME/node/bin/node" ]; then
        ...
        log_success "Node.js $found_ver found ({{AGENT_NAME}}-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.{{AGENT_NAME_LOWER}}/node/..."
    ...
    # Place into ~/.{{AGENT_NAME_LOWER}}/node/ and symlink binaries to ~/.local/bin/
    ...
    log_success "Node.js $installed_ver installed to ~/.{{AGENT_NAME_LOWER}}/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "{{AGENT_NAME}} Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="{{AGENT_NAME_LOWER}}-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if {{AGENT_NAME}} behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up {{AGENT_NAME_LOWER}} command..."

    if [ "$USE_VENV" = true ]; then
        HERMES_BIN="$INSTALL_DIR/venv/bin/{{AGENT_NAME_LOWER}}"
    else
        HERMES_BIN="$(which {{AGENT_NAME_LOWER}} 2>/dev/null || echo "")"
        if [ -z "$HERMES_BIN" ]; then
            log_warn "{{AGENT_NAME_LOWER}} not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$HERMES_BIN" ]; then
        log_warn "{{AGENT_NAME_LOWER}} entry point not found at $HERMES_BIN"
    ...
    mkdir -p "$HOME/.local/bin"
    ln -sf "$HERMES_BIN" "$HOME/.local/bin/{{AGENT_NAME_LOWER}}"
    log_success "Symlinked {{AGENT_NAME_LOWER}} → ~/.local/bin/{{AGENT_NAME_LOWER}}"
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# {{AGENT_NAME}} Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
    ...
    # Export for current session so {{AGENT_NAME_LOWER}} works immediately
    export PATH="$HOME/.local/bin:$PATH"

    log_success "{{AGENT_NAME_LOWER}} command ready"
}

copy_config_templates() {
    log_info "Setting up configuration files..."

    # Create ~/.{{AGENT_NAME_LOWER}} directory structure (config at top level, code in subdir)
    mkdir -p "$HERMES_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills,whatsapp/session}

    # Create .env at ~/.{{AGENT_NAME_LOWER}}/.env (top level, easy to find)
    if [ ! -f "$HERMES_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$HERMES_HOME/.env"
            log_success "Created ~/.{{AGENT_NAME_LOWER}}/.env from template"
        else
            touch "$HERMES_HOME/.env"
            log_success "Created ~/.{{AGENT_NAME_LOWER}}/.env"
        fi
    else
        log_info "~/.{{AGENT_NAME_LOWER}}/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.{{AGENT_NAME_LOWER}}/config.yaml (top level, easy to find)
    if [ ! -f "$HERMES_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$HERMES_HOME/config.yaml"
            log_success "Created ~/.{{AGENT_NAME_LOWER}}/config.yaml from template"
        fi
    else
        log_info "~/.{{AGENT_NAME_LOWER}}/config.yaml already exists, keeping it"
    fi
...
Edit this to customize how {{AGENT_NAME}} communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.{{AGENT_NAME_LOWER}}/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.{{AGENT_NAME_LOWER}}/"

    # Seed bundled skills into ~/.{{AGENT_NAME_LOWER}}/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.{{AGENT_NAME_LOWER}}/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.{{AGENT_NAME_LOWER}}/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$HERMES_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$HERMES_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.{{AGENT_NAME_LOWER}}/skills/"
        fi
    fi
}
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ...
            *)
                ...
                log_info "This is standard Playwright setup — {{AGENT_NAME}} itself does not require root access."
...
run_setup_wizard() {
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Setup wizard skipped (no terminal available). Run '{{AGENT_NAME_LOWER}} setup' after install."
        return 0
    fi
    ...
    # Run {{AGENT_NAME_LOWER}} setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m {{AGENT_NAME_LOWER}}_cli.main setup < /dev/tty
    else
        python -m {{AGENT_NAME_LOWER}}_cli.main setup < /dev/tty
    fi
}
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for {{AGENT_NAME}} to send/receive messages."
    ...
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            log_info "Running '{{AGENT_NAME_LOWER}} whatsapp' to pair via QR code..."
            echo ""
            read -p "Pair WhatsApp now? [Y/n] " -n 1 -r
            echo
            if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
                HERMES_CMD="$HOME/.local/bin/{{AGENT_NAME_LOWER}}"
                [ ! -x "$HERMES_CMD" ] && HERMES_CMD="{{AGENT_NAME_LOWER}}"
                $HERMES_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run '{{AGENT_NAME_LOWER}} whatsapp' to pair."
        fi
    fi

    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run '{{AGENT_NAME_LOWER}} gateway install' later."
        return 0
    fi
        ...
        if command -v systemctl &> /dev/null; then
            ...
            if $HERMES_CMD gateway install 2>/dev/null; then
                ...
                if $HERMES_CMD gateway start 2>/dev/null; then
                    ...
                    log_warn "Service installed but failed to start. Try: {{AGENT_NAME_LOWER}} gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: {{AGENT_NAME_LOWER}} gateway"
            fi
        else
            log_info "systemd not available — starting gateway in background..."
            nohup $HERMES_CMD gateway > "$HERMES_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.{{AGENT_NAME_LOWER}}/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: {{AGENT_NAME_LOWER}} gateway"
        fi
    else
        log_info "Skipped. Start the gateway later with: {{AGENT_NAME_LOWER}} gateway"
    fi
}
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.{{AGENT_NAME_LOWER}}/):${NC}"
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    ~/.{{AGENT_NAME_LOWER}}/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  ~/.{{AGENT_NAME_LOWER}}/.env"
    echo -e "   ${YELLOW}Data:${NC}      ~/.{{AGENT_NAME_LOWER}}/cron/, sessions/, logs/"
    echo -e "   ${YELLOW}Code:${NC}      ~/.{{AGENT_NAME_LOWER}}/{{AGENT_NAME_LOWER}}-agent/"
    echo ""
    ...
    echo ""
    echo -e "   ${GREEN}{{AGENT_NAME_LOWER}}${NC}              Start chatting"
    echo -e "   ${GREEN}{{AGENT_NAME_LOWER}} setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}{{AGENT_NAME_LOWER}} config${NC}       View/edit configuration"
    echo -e "   ${GREEN}{{AGENT_NAME_LOWER}} config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}{{AGENT_NAME_LOWER}} gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}{{AGENT_NAME_LOWER}} update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    echo -e "${YELLOW}⚡ Reload your shell to use '{{AGENT_NAME_LOWER}}' command:${NC}"
    echo ""
    LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
