...
# init-template.sh — Initialize a fork of opencode-drewgent
...
echo -e "${BLUE}╔══════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║     opencode-drewgent Template Setup        ║${NC}"
echo -e "${BLUE}╚══════════════════════════════════════════════╝${NC}"
echo ""
...
if [[ -z "$NAME" ]]; then
  case "$DIRNAME" in
    .drewgent|drewgent|opencode-drewgent)
...
echo -e "${BLUE}[3/4] Renaming 'drewgent' → '$AGENT_NAME'...${NC}"
# Only rename if this is a fresh clone (contains "drewgent" extensively)
if grep -r "drewgent" --include="*.md" --include="*.jsonc" --include="*.json" --include="*.sh" --include="*.py" . 2>/dev/null | grep -q "drewgent"; then
  echo -e "${YELLOW}  Found drewgent references. Replace all? [y/N]${NC}"
  read -r CONFIRM
  if [[ "$CONFIRM" =~ ^[yY] ]]; then
    find . \( -name "*.md" -o -name "*.jsonc" -o -name "*.json" -o -name "*.sh" -o -name "*.py" \) \
      -not -path "*/node_modules/*" -not -path "*/.git/*" \
      -exec sed -i '' "s/drewgent/$AGENT_NAME/g" {} \;
    echo -e "${GREEN}  ✓ Renamed drewgent → $AGENT_NAME${NC}"
  else
    echo -e "${YELLOW}  ⚠ Skipped rename (run skill(\"rename-drewgent\") later)${NC}"
  fi
else
  echo -e "${GREEN}  ✓ No drewgent references to rename${NC}"
fi
