#!/usr/bin/env python3
"""vault — cross-platform encrypted secrets manager for Loragent.
...
VAULT_DIR = Path.home() / ".config" / "loragent"
VAULT_FILE = VAULT_DIR / "vault.enc"
KEYRING_SERVICE = "loragent-vault"
...
SECRET_PATTERNS = [
    ...
    re.compile(r'"(DISCORD_BOT_TOKEN|DISCORD_TOKEN|OPENCODE_API_KEY|OPENAI_API_KEY|ANTHROPIC_API_KEY|GROQ_API_KEY|MINIMAX_API_KEY|LAZYWEB_API_KEY|NOTION_TOKEN|_[A-Z_]+KEY[A-Z_]*|_[A-Z_]+TOKEN[A-Z_]*)":\s*"([^"]+)"'),
...
REPLACE_PATTERNS = [
    (re.compile(r'export\s+(DISCORD_BOT_TOKEN|DISCORD_TOKEN|OPENCODE_API_KEY|OPENAI_API_KEY|ANTHROPIC_API_KEY|GROQ_API_KEY|MINIMAX_API_KEY|LAZYWEB_API_KEY|NOTION_TOKEN|_[A-Z_]+KEY[A-Z_]*|_[A-Z_]+TOKEN[A-Z_]*)="?([^"\'\n#]+)"?'), True),
    (re.compile(r'"(DISCORD_BOT_TOKEN|DISCORD_TOKEN|OPENCODE_API_KEY|OPENAI_API_KEY|ANTHROPIC_API_KEY|GROQ_API_KEY|MINIMAX_API_KEY|LAZYWEB_API_KEY|NOTION_TOKEN|[a-zA-Z_]+(?:KEY|TOKEN|API_KEY))":\s*"([^"]+)"'), False),
]
...
def main():
    parser = argparse.ArgumentParser(description="Loragent vault — encrypted secrets manager")
