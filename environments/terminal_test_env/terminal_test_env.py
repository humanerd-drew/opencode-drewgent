...
Training tasks (3):
    1. Create ~/greeting.txt with "Hello from Drewgent Agent"
...
TRAIN_TASKS = [
    {
        "prompt": "Create a file at ~/greeting.txt containing exactly the text: Hello from Drewgent Agent",
        "verify_path": "~/greeting.txt",
        "expected_content": "Hello from Drewgent Agent",
    },
    {
...
class TerminalTestEnv(DrewgentAgentBaseEnv):
    ...
    def config_init(cls) -> Tuple[TerminalTestEnvConfig, List[APIServerConfig]]:
        ...
        Claude for inference. API keys loaded from ~/drewgent-agent/.env.
        """
        env_config = TerminalTestEnvConfig(
            ...
            tokenizer_name="YOUR_ORG/q-30b-t-h45-e1",
