"""
DrewgentAgentBaseEnv -- Abstract Base Environment for Drewgent-Agent + Atropos

Provides the Atropos integration plumbing that all drewgent-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via DrewgentAgentLoop
...
# Ensure the drewgent-agent repo root is on sys.path so that imports like
...
# Load API keys from drewgent-agent/.env so all environments can access them
...
from environments.agent_loop import AgentResult, DrewgentAgentLoop
from environments.tool_context import ToolContext

# Import drewgent-agent toolset infrastructure
...
class DrewgentAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for drewgent-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of drewgent toolsets to enable (e.g., ['terminal', 'file', 'web']). "
...
class DrewgentAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for drewgent-agent Atropos integration.
    ...
    name: Optional[str] = "drewgent-agent"
    env_config_cls = DrewgentAgentEnvConfig

    def __init__(
        self,
        config: DrewgentAgentEnvConfig,
        ...
        # Set terminal environment variables so drewgent tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY drewgent-agent tool (terminal, file, web,
