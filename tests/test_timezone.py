"""
Tests for timezone support (drewgent_time module + integration points).
...
import drewgent_time


# =========================================================================
# drewgent_time.now() — core helper
# =========================================================================

class TestDrewgentTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        drewgent_time.reset_cache()

    def teardown_method(self):
        drewgent_time.reset_cache()
        os.environ.pop("DREW_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["DREW_TIMEZONE"] = "Asia/Kolkata"
        result = drewgent_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["DREW_TIMEZONE"] = "UTC"
        result = drewgent_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["DREW_TIMEZONE"] = "America/New_York"
        result = drewgent_time.now()
        assert result.tzinfo is not None
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["DREW_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="drewgent_time"):
            result = drewgent_time.now()
    ...
    def test_cache_invalidation(self):
        ...
        os.environ["DREW_TIMEZONE"] = "UTC"
        drewgent_time.reset_cache()
        r1 = drewgent_time.now()
        assert r1.utcoffset() == timedelta(0)

        os.environ["DREW_TIMEZONE"] = "Asia/Kolkata"
        drewgent_time.reset_cache()
        r2 = drewgent_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["DREW_TIMEZONE"] = "Europe/London"
        tz = drewgent_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["DREW_TIMEZONE"] = "Not/A/Timezone"
        tz = drewgent_time.get_timezone()
        assert tz is None

    def test_get_timezone_name(self):
        os.environ["DREW_TIMEZONE"] = "Asia/Tokyo"
        assert drewgent_time.get_timezone_name() == "Asia/Tokyo"
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When DREW_TIMEZONE is set, child process sees TZ env var."""
        import json as _json
        os.environ["DREW_TIMEZONE"] = "Asia/Kolkata"
    ...
    def test_tz_not_injected_when_empty(self):
        """When DREW_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("DREW_TIMEZONE", None)
    ...
    def test_drewgent_timezone_not_leaked_to_child(self):
        """DREW_TIMEZONE itself must NOT appear in child env (only TZ)."""
        import json as _json
        os.environ["DREW_TIMEZONE"] = "Asia/Kolkata"

        with patch("model_tools.handle_function_call", side_effect=self._mock_handle):
            result = _json.loads(self._execute_code(
                code='import os; print(os.environ.get("DREW_TIMEZONE", "NOT_SET"))',
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=drewgent_tz) which shifted
        absolute time when system-local tz != Drewgent tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_drewgent_tz(self):
        """Already-aware datetimes should be normalized to Drewgent tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Drewgent tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Drewgent tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Drewgent is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Drewgent tz
        ...
        # Use a Drewgent timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _drewgent_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["DREW_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        drewgent_time.reset_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Drewgent timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["DREW_TIMEZONE"] = "US/Eastern"
        drewgent_time.reset_cache()
