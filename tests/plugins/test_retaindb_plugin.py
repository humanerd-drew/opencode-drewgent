...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure DREW_HOME and RETAINDB vars are isolated."""
...
class TestClient:
    ...
    def test_get_agent_model_path(self):
        ...
        with patch.object(c, "request") as mock_req:
            ...
            c.get_agent_model("drewgent")
            mock_req.assert_called_once_with(
                "GET", "/v1/memory/agent/drewgent/model",
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", drewgent_home=str(tmp_path / ".hermes"))
    ...
    def test_initialize_profile_project(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", drewgent_home=profile_home)
        assert p._client.project == "drewgent-coder"
        p.shutdown()
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        drewgent_home = tmp_path / ".hermes"
        drewgent_home.mkdir(exist_ok=True)
        monkeypatch.setenv("DREW_HOME", str(drewgent_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", drewgent_home=str(drewgent_home))
        return p
