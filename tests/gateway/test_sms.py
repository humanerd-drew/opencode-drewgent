...
class TestSmsToolset:
    def test_drewgent_sms_toolset_exists(self):
        from toolsets import get_toolset

        ts = get_toolset("drewgent-sms")
        assert ts is not None
        assert "tools" in ts

    def test_drewgent_sms_in_gateway_includes(self):
        from toolsets import get_toolset

        gw = get_toolset("drewgent-gateway")
        assert gw is not None
        assert "drewgent-sms" in gw["includes"]
