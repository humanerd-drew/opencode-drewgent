...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        ...
        assert "brew upgrade drewgent-agent" in result
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _drewgent_home to tmp_path and project_root to a dir without .git
        fake_root = tmp_path / "project"
        fake_root.mkdir()
        with patch("gateway.run._drewgent_home", tmp_path), \
        ...
        with patch("gateway.run._drewgent_home", tmp_path):
            ...
            # project_root will be the real drewgent-agent dir (which HAS .git).
    ...
    @pytest.mark.asyncio
    async def test_no_drewgent_binary(self, tmp_path):
        """Returns error when drewgent is not on PATH and drewgent_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "drewgent update" in result

    @pytest.mark.asyncio
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m drewgent_cli.main when drewgent not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        with patch("gateway.run._drewgent_home", drewgent_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", return_value=None), \
        ...
        assert "Starting Drewgent update" in result
        call_args = mock_popen.call_args[0][0]
        # The update_cmd uses sys.executable -m drewgent_cli.main
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "drewgent_cli.main" in joined or "bash" in call_args[0]

    @pytest.mark.asyncio
    async def test_resolve_drewgent_bin_prefers_which(self, tmp_path):
        """_resolve_drewgent_bin returns argv parts from shutil.which when available."""
        from gateway.run import _resolve_drewgent_bin

        with patch("shutil.which", return_value="/custom/path/drewgent"):
            result = _resolve_drewgent_bin()

        assert result == ["/custom/path/drewgent"]

    @pytest.mark.asyncio
    async def test_resolve_drewgent_bin_fallback(self):
        """_resolve_drewgent_bin falls back to sys.executable argv when which fails."""
        import sys
        from gateway.run import _resolve_drewgent_bin
        ...
        assert result == [sys.executable, "-m", "drewgent_cli.main"]

    @pytest.mark.asyncio
    async def test_resolve_drewgent_bin_returns_none_when_both_fail(self):
        """_resolve_drewgent_bin returns None when both strategies fail."""
        from gateway.run import _resolve_drewgent_bin
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        with patch("gateway.run._drewgent_home", drewgent_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", side_effect=lambda x: "/usr/bin/drewgent" if x == "drewgent" else "/usr/bin/setsid"), \
             patch("subprocess.Popen"):
            result = await runner._handle_update_command(event)

        pending_path = drewgent_home / ".update_pending.json"
        assert pending_path.exists()
        data = json.loads(pending_path.read_text())
        assert data["platform"] == "telegram"
        assert data["chat_id"] == "99999"
        assert "timestamp" in data
        assert not (drewgent_home / ".update_exit_code").exists()
    ...
    async def test_popen_failure_cleans_up(self, tmp_path):
        ...
        assert not (drewgent_home / ".update_pending.json").exists()
        assert not (drewgent_home / ".update_exit_code").exists()
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (drewgent_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = drewgent_home / ".update_pending.claimed.json"
        claimed_path.write_text(json.dumps({
            "platform": "telegram", "chat_id": "67890", "user_id": "12345",
        }))
        (drewgent_home / ".update_output.txt").write_text("done")
        (drewgent_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_notification_with_output(self, tmp_path):
        ...
        (drewgent_home / ".update_pending.json").write_text(json.dumps(pending))
        (drewgent_home / ".update_output.txt").write_text(
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (drewgent_home / ".update_pending.json").write_text(json.dumps(pending))
        (drewgent_home / ".update_output.txt").write_text("x" * 5000)
        (drewgent_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (drewgent_home / ".update_pending.json").write_text(json.dumps(pending))
        (drewgent_home / ".update_output.txt").write_text("Traceback: boom")
        (drewgent_home / ".update_exit_code").write_text("1")
    ...
    async def test_cleans_up_files_after_notification(self, tmp_path):
        ...
        pending_path = drewgent_home / ".update_pending.json"
        output_path = drewgent_home / ".update_output.txt"
        exit_code_path = drewgent_home / ".update_exit_code"
        pending_path.write_text(json.dumps({
