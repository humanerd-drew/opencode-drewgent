...
def _isolate(tmp_path, monkeypatch):
    """Isolate DREW_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal DrewgentCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = DrewgentCLI.__new__(DrewgentCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["drewgent-core"]
        obj.compact = False
