...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._drewgent_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_drewgent_bin(self):
        """After _INSTALL_FAILED, manual install in DREW_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        drewgent_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(drewgent_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(drewgent_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._drewgent_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == drewgent_bin
            assert _tirith_mod._resolved_path == drewgent_bin
            mock_clear.assert_called_once()
...
class TestDrewgentHomeIsolation:
    def test_drewgent_bin_dir_respects_drewgent_home(self):
        """_drewgent_bin_dir must use DREW_HOME, not hardcoded ~/.drewgent."""
        from tools.tirith_security import _drewgent_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"DREW_HOME": tmpdir}):
            result = _drewgent_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_drewgent_home(self):
        """_failure_marker_path must use DREW_HOME, not hardcoded ~/.drewgent."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"DREW_HOME": "/custom/drewgent"}):
            result = _failure_marker_path()
        assert result == "/custom/drewgent/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets DREW_HOME; verify it's active."""
        drewgent_home = os.getenv("DREW_HOME")
        assert drewgent_home is not None, "DREW_HOME should be set by conftest"
        assert "drewgent_test" in drewgent_home, "Should point to test temp dir"

    def test_get_drewgent_home_fallback(self):
        """Without DREW_HOME set, falls back to ~/.drewgent."""
        from tools.tirith_security import _get_drewgent_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove DREW_HOME entirely
            os.environ.pop("DREW_HOME", None)
            result = _get_drewgent_home()
