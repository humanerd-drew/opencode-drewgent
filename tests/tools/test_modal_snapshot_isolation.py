...
def _restore_tool_modules():
    original_drewgent_home = os.environ.get("DREW_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "drewgent_cli"
        or name.startswith("drewgent_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_drewgent_home is None:
            os.environ.pop("DREW_HOME", None)
        else:
            os.environ["DREW_HOME"] = original_drewgent_home
        _reset_modules(("tools", "drewgent_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    drewgent_cli = types.ModuleType("drewgent_cli")
    drewgent_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["drewgent_cli"] = drewgent_cli
    drewgent_home = tmp_path / "drewgent-home"
    os.environ["DREW_HOME"] = str(drewgent_home)
    sys.modules["drewgent_cli.config"] = types.SimpleNamespace(
        get_drewgent_home=lambda: drewgent_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="drewgent-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": drewgent_home / "modal_snapshots.json",
