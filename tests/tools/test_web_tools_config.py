...
class TestFirecrawlClientConfig:
    ...
    def setup_method(self):
        ...
        for key in (
            "DREW_ENABLE_NOUS_MANAGED_TOOLS",
            "FIRECRAWL_API_KEY",
            "FIRECRAWL_API_URL",
        ...
        os.environ["DREW_ENABLE_NOUS_MANAGED_TOOLS"] = "1"
    ...
    def test_nous_auth_token_respects_drewgent_home_override(self, tmp_path):
        """Auth lookup should read from DREW_HOME/auth.json, not ~/.drewgent/auth.json."""
...
class TestBackendSelection:
    ...
    ``drewgent tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
    ...
    def setup_method(self):
        ...
        for key in self._ENV_KEYS:
            if key != "DREW_ENABLE_NOUS_MANAGED_TOOLS":
                os.environ.pop(key, None)
