...
Verifies that Drewgent-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed Drewgent secrets.

See: https://github.com/YOUR_GITHUB_USER/YOUR_REPO_NAME/issues/1002
See: https://github.com/YOUR_GITHUB_USER/YOUR_REPO_NAME/issues/1264
"""
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.drewgent/.env must not leak."""
...
class TestForceEnvOptIn:
    """Callers can opt in to passing a blocked var via _DREW_FORCE_ prefix."""

    def test_force_prefix_passes_blocked_var(self):
        """_DREW_FORCE_OPENAI_API_KEY in self.env should inject OPENAI_API_KEY."""
        result_env = _run_with_env(self_env={
