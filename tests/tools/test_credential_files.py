...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        drewgent_home.mkdir()
        (drewgent_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"DREW_HOME": str(drewgent_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(drewgent_home / "token.json")
    ...
    def test_dict_with_name_key_fallback(self, tmp_path):
        ...
        drewgent_home = tmp_path / ".hermes"
        drewgent_home.mkdir()
        (drewgent_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        drewgent_home = tmp_path / ".hermes"
        drewgent_home.mkdir()
        (drewgent_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        drewgent_home = tmp_path / ".hermes"
        drewgent_home.mkdir()
        (drewgent_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    ...
    def test_custom_container_base(self, tmp_path):
        drewgent_home = tmp_path / ".hermes"
        (drewgent_home / "skills").mkdir(parents=True)
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape DREW_HOME."""
        monkeypatch.setenv("DREW_HOME", str(tmp_path / ".hermes"))
        (tmp_path / ".hermes").mkdir()

        # Create a sensitive file one level above drewgent_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside drewgent_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside DREW_HOME must still be registered."""
        drewgent_home = tmp_path / ".hermes"
        drewgent_home.mkdir()
        monkeypatch.setenv("DREW_HOME", str(drewgent_home))
        (drewgent_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_drewgent_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of DREW_HOME must be allowed."""
        drewgent_home = tmp_path / ".hermes"
        drewgent_home.mkdir()
        subdir = drewgent_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside DREW_HOME pointing outside must be rejected."""
        drewgent_home = tmp_path / ".hermes"
        drewgent_home.mkdir()
        monkeypatch.setenv("DREW_HOME", str(drewgent_home))

        # Create a sensitive file outside drewgent_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside drewgent_home pointing outside
        symlink = drewgent_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes DREW_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, drewgent_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape DREW_HOME."""
        drewgent_home = tmp_path / ".hermes"
        drewgent_home.mkdir()
        monkeypatch.setenv("DREW_HOME", str(drewgent_home))
        ...
        sensitive.write_text("{}")
        self._write_config(drewgent_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(drewgent_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside DREW_HOME via config must still mount."""
        drewgent_home = tmp_path / ".hermes"
        drewgent_home.mkdir()
        monkeypatch.setenv("DREW_HOME", str(drewgent_home))

        (drewgent_home / "oauth.json").write_text("{}")
        self._write_config(drewgent_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        drewgent_home = tmp_path / ".hermes"
        drewgent_home.mkdir()
        (drewgent_home / "cache" / "documents").mkdir(parents=True)
        (drewgent_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("DREW_HOME", str(drewgent_home))
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        drewgent_home = tmp_path / ".hermes"
        drewgent_home.mkdir()
        # Use legacy dir name — get_drewgent_dir prefers old if it exists
        (drewgent_home / "document_cache").mkdir()
        (drewgent_home / "image_cache").mkdir()
        monkeypatch.setenv("DREW_HOME", str(drewgent_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(drewgent_home / "document_cache") in host_paths
        assert str(drewgent_home / "image_cache") in host_paths
    ...
    def test_empty_drewgent_home(self, tmp_path, monkeypatch):
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        drewgent_home = tmp_path / ".hermes"
        doc_dir = drewgent_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        drewgent_home = tmp_path / ".hermes"
        ss_dir = drewgent_home / "cache" / "screenshots"
