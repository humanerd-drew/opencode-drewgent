...
_ANTI_PATTERNS = [
    (r"(?:don't|not)\s+(?:do|use|want|need|like|run)\s+([A-Za-z][^.!?]{5,})", "anti_preference"),
]
...
class AutoLearner:
    ...
    def _save_insight(self, insight: Insight) -> bool:
        ...
        try:
            ...
            # Also write to knowledge.db for cross-session FTS5 search
            if success:
                self._write_to_knowledge_db(insight)
    ...
    def _write_to_knowledge_db(self, insight: Insight) -> None:
        """Write insight to knowledge.db for FTS5 retrieval."""
        import hashlib, sqlite3
        from datetime import datetime, timezone
        from pathlib import Path

        db_path = Path(self._wiki_path).parent / ".agent" / "memory" / "knowledge.db"
