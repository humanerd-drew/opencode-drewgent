...
from agent.drewgent_constants import OPENROUTER_BASE_URL
...
from agent.utils import atomic_json_write, env_var_enabled

_MEMORY_DB_PATH = get_drewgent_home() / ".agent" / "memory" / "knowledge.db"
_CREDENTIAL_STATE_PATH = get_drewgent_home() / ".agent" / "memory" / "credential_state.json"


def _fts_safe_query(text: str) -> str:
    """Sanitize user text for FTS5 MATCH syntax.

    Prefers ASCII (English) terms since the knowledge.db index stores
    mostly English facts.  Falls back to the full term list so that
    pure-Korean queries still have a chance.
    """
    text = re.sub(r'[+\-*()"~<@^]', " ", text)
    words = [w for w in text.split() if len(w) > 1]
    if not words:
        return ""
    # Prefer ASCII terms (English tech terms like 'bridge', 'agent', 'memory')
    ascii_words = [w for w in words if w.isascii()]
    terms = ascii_words[:8] if ascii_words else words[:8]
    return " ".join(f'"{w}"' for w in terms)


def _query_memory_db(query: str, limit: int = 5) -> list[dict]:
    """Query agent-memory knowledge.db for relevant preferences/decisions/incidents."""
    ...
    try:
        ...
        if not fts_query:
            ...
            return []
        rows = db.execute(
            "SELECT fact, type FROM memory_fts WHERE memory_fts MATCH ? ORDER BY rank LIMIT ?",
            (fts_query, limit),
        ).fetchall()
        # Fallback: if FTS5 matched nothing (common with cross-language queries),
        # return the most recent entries so the agent still has context.
        if not rows:
            rows = db.execute(
                "SELECT fact, type FROM memory_fts ORDER BY rowid DESC LIMIT ?",
                (limit,),
            ).fetchall()
        db.close()
        return [{"fact": r["fact"], "type": r["type"]} for r in rows]
    except Exception:
        return []


def _read_credential_state() -> str:
    """Read credential state registry and return a summary of known failures.

    The credential diagnostic runs hourly and records credential failures.
    This injects a concise summary so the agent avoids dead providers
    without trial-and-error discovery.
    """
    try:
        import json
        if not _CREDENTIAL_STATE_PATH.exists():
            return ""
        reg = json.loads(_CREDENTIAL_STATE_PATH.read_text())
        ...
        for key, entry in sorted(entries.items()):
            ...
            lines.append(f"- {key}: {mode} (×{count})")
        if not lines:
            return ""
        return "Known credential issues (prefer working providers):\n" + "\n".join(lines)
    except Exception:
        return ""
...
class AIAgent:
    ...
    def __init__(
        ...
        from agent.drewgent_logging import setup_logging, setup_verbose_logging
        from agent.drewgent_constants import get_drewgent_home
...
# Brain signal system: initialize signal processor + awareness reporter
            ...
            try:
                ...
                if _mem_provider_name:
                    ...
                    if self._memory_manager.providers:
                        from agent.drewgent_constants import get_drewgent_home as _ghh
    ...
    def _ensure_tool_schemas_expanded(self, tool_calls) -> None:
        ...
        from agent.model_tools import get_full_schema_for_tools
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        if self.api_mode == "anthropic_messages":
            ...
            from agent.model_tools import get_tool_manifest, _CORE_MANIFEST_TOOLS
    ...
    def run_conversation(
        ...
        # Agent-memory context: query knowledge.db for stored preferences/decisions/incidents.
        # Complements wiki brain context with cross-session memory recall.
        _agent_memory_context = ""
        if original_user_message:
            try:
                _memories = _query_memory_db(original_user_message, limit=5)
                _agent_memory_context = "\n".join(
                    f"- [{m['type']}] {m['fact']}" for m in _memories
                ) if _memories else ""

                # Semantic fallback: if FTS5 returned only fallback entries
                # (no exact match), try embedding search from auto_learner
                if self._auto_learner and hasattr(self._auto_learner, "_vector_store") and self._auto_learner._vector_store:
                    try:
                        sem = self._auto_learner.semantic_search(original_user_message, limit=3)
                        if sem:
                            high = [r for r in sem if r.get('score', 0) >= 0.5]
                            if high:
                                _agent_memory_context = "\n".join(
                                    f"- [{r.get('itype', 'memory')}] {r['content']}" for r in high
                                )
                    except Exception:
                        pass
            except Exception:
        ...
        # Auto-provision: scan user message for API key patterns and store
        # the key automatically so the agent doesn't need to ask twice.
        _auto_credential_note = ""
        if original_user_message:
            try:
                from tools.credential_tool import auto_provision_from_message
                _auto_credential_note = auto_provision_from_message(original_user_message) or ""
            except Exception:
                pass
        if _auto_credential_note:
            _injected_parts.append(_auto_credential_note)

        # Pre-LLM skill matching: if the user message keywords match a skill
        # name or description, suggest loading it before answering.
        _skill_match_note = ""
        if original_user_message:
            try:
                from agent.skill_commands import scan_skill_commands
                _skills = scan_skill_commands()
                if _skills:
                    _query_lower = original_user_message.lower()
                    _matched = []
                    for _sname, _sinfo in _skills.items():
                        _desc = str(_sinfo.get("description", "")).lower()
                        _keywords = _query_lower.split()
                        _match_score = sum(1 for kw in _keywords if len(kw) > 2 and kw in _desc)
                        if _match_score >= 2:
                            _matched.append((_sname, _match_score))
                    if _matched:
                        _matched.sort(key=lambda x: -x[1])
                        _top = _matched[0][0]
                        _skill_match_note = f"[Skill match] '{_top}' matches your request. Load it with skill_view(\"{_top}\") and follow its instructions."
            except Exception:
                pass
        if _skill_match_note:
            _injected_parts.append(_skill_match_note)

        if _brain_context:
            _injected_parts.append(f"[Brain context]\n{_brain_context}")
        if _agent_memory_context:
            _injected_parts.append(f"[Agent memories]\n{_agent_memory_context}")

        # Credential health context: known credential failures are injected
        # so the agent knows which providers are unavailable without trial/error.
        _credential_context = _read_credential_state()
        if _credential_context:
            _injected_parts.append(f"[Credential state]\n{_credential_context}")
            ...
            while retry_count < max_retries:
                try:
                    ...
                    if (
                        ...
                        from agent.drewgent_constants import display_drewgent_home as _dhh_fn
...
def main(
    ...
    if list_tools:
        ...
        from agent.toolsets import get_all_toolsets, get_toolset_info
