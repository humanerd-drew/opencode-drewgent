...
# ---------------------------------------------------------------------------
# knowledge.db bridge — every explicit memory write is also stored in
# the FTS5 knowledge database so _query_memory_db() can retrieve it
# across sessions without depending on the wiki pipeline.
# ---------------------------------------------------------------------------

_KNOWLEDGE_DB_PATH = get_drewgent_home() / ".agent" / "memory" / "knowledge.db"


def _write_to_knowledge_db(fact: str, entry_type: str = "memory"):
    """Upsert a fact into knowledge.db so it's FTS5-searchable."""
    try:
        _KNOWLEDGE_DB_PATH.parent.mkdir(parents=True, exist_ok=True)
        db = sqlite3.connect(str(_KNOWLEDGE_DB_PATH))
        db.execute(
        ...
        db.execute(
            ...
            (fact, entry_type, content_hash),
        )
        db.execute(
            "INSERT OR IGNORE INTO source_meta (content_hash, source_file, synced_at) VALUES (?, ?, ?)",
            (content_hash, "memory_tool", now),
        )
        db.commit()
        db.close()
    except Exception:
        pass  # knowledge.db is best-effort
...
class MemoryStore:
    ...
    def add(self, target: str, content: str) -> Dict[str, Any]:
        ...
        with self._file_lock(self._path_for(target)):
            ...
            _write_to_knowledge_db(f"{target}: {content}", "memory")
    ...
    def replace(self, target: str, old_text: str, new_content: str) -> Dict[str, Any]:
        ...
        with self._file_lock(self._path_for(target)):
            ...
            self._set_entries(target, entries)
            self.save_to_disk(target)
            _write_to_knowledge_db(f"{target}: {new_content}", "memory")
