...
Handles: drewgent honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "drewgent" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Drewgent profiles and creates host blocks for any that don't
    have one yet. Inherits settings from the default host block.
    """
    try:
        from drewgent_cli.profiles import list_profiles
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'drewgent honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print(
            "  Honcho not configured on default profile. Run 'drewgent honcho setup' first.\n"
        )
        return
    ...
    for p in profiles:
        ...
        if clone_honcho_for_profile(p.name):
            print(f"  + {p.name} -> drewgent.{p.name}")
            created += 1
        else:
            skipped += 1
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `drewgent update` -- no output, no exceptions.
    """
    try:
        from drewgent_cli.profiles import list_profiles
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Drewgent profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $DREW_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_drewgent_home() / "honcho.json"
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Drewgent persistent cross-session memory.")
    ...
    drewgent_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'drewgent honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = drewgent_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt(
        "Your name (user peer)", default=current_peer or os.getenv("USER", "user")
    )
    if new_peer:
        drewgent_host["peerName"] = new_peer

    current_ai = drewgent_host.get("aiPeer") or cfg.get("aiPeer", "drewgent")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        drewgent_host["aiPeer"] = new_ai

    current_workspace = drewgent_host.get("workspace") or cfg.get(
        "workspace", "drewgent"
    )
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        drewgent_host["workspace"] = new_workspace

    # --- 4. Observation mode ---
    current_obs = drewgent_host.get("observationMode") or cfg.get(
    ...
    if new_obs in ("unified", "directional"):
        drewgent_host["observationMode"] = new_obs
    else:
        drewgent_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(
        drewgent_host.get("writeFrequency") or cfg.get("writeFrequency", "async")
    )
    print("\n  Write frequency:")
    ...
    try:
        drewgent_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        drewgent_host["writeFrequency"] = (
            new_wf if new_wf in ("async", "turn", "session") else "async"
        )

    # --- 6. Recall mode ---
    _raw_recall = drewgent_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    current_recall = (
    ...
    if new_recall in ("hybrid", "context", "tools"):
        drewgent_host["recallMode"] = new_recall

    # --- 7. Session strategy ---
    current_strat = drewgent_host.get("sessionStrategy") or cfg.get(
    ...
    if new_strat in ("per-session", "per-repo", "per-directory", "global"):
        drewgent_host["sessionStrategy"] = new_strat

    drewgent_host["enabled"] = True
    drewgent_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        drewgent_config = load_config()
        drewgent_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(drewgent_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: drewgent config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    drewgent honcho status     -- show full config")
    print("    drewgent honcho mode       -- change recall/observation mode")
    print("    drewgent honcho tokens     -- tune context and dialectic budgets")
    print("    drewgent honcho peer       -- update peer names")
    print("    drewgent honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Drewgent profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: drewgent honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'drewgent honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: drewgent honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        drewgent = hosts.get(_host_key(), {})
        user = drewgent.get("peerName") or cfg.get("peerName") or "(not set)"
        ai = drewgent.get("aiPeer") or cfg.get("aiPeer") or _host_key()
        lvl = (
            drewgent.get("dialecticReasoningLevel")
            or cfg.get("dialecticReasoningLevel")
            or "low"
        )
        max_chars = (
            drewgent.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        )
        print("\nHoncho peers\n" + "─" * 40)
        ...
        print(f"  AI peer:     {ai}")
        print(
            "    Drewgent' identity in Honcho. Seed with 'drewgent honcho identity <file>'."
        )
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: drewgent honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("    AI-to-AI inference. Drewgent asks Honcho's AI peer a question")
        ...
        print("\n  Set with: drewgent honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'drewgent honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'drewgent honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print()
        print(
            "    drewgent honcho identity --show        — show both peer representations"
        )
        print(
            "    drewgent honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n"
        )
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Drewgent + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Drewgent\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print(
            "  Honcho is a cloud memory service that gives Drewgent persistent memory"
        )
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  drewgent honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'drewgent honcho setup' now?", default="y")
        if answer.lower() in ("y", "yes"):
            ...
            print()
            print(
                "  Run 'drewgent honcho setup' when ready, then re-run this walkthrough."
            )
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  drewgent honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'drewgent'")
        print("  with Honcho configured and no prior session history.")
        print("  (Drewgent calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    drewgent honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'drewgent honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Drewgent, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'drewgent honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    drewgent honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  drewgent honcho identity <path/to/SOUL.md>")
    ...
    print("    Drewgent:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print(
        "    Drewgent:   Honcho context fetched async at turn end, injected next turn."
    )
    print(
    ...
    print("    Drewgent:   Honcho observes every message and updates representations")
    ...
    print("    Drewgent:   per-session by default — each run gets its own session")
    print("              Map a custom name:  drewgent honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. drewgent honcho setup              — configure API key (required)")
        print("  2. drewgent honcho migrate            — re-run this walkthrough")
    else:
        print("  1. drewgent honcho status             — verify Honcho connection")
        print("  2. drewgent                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. drewgent honcho identity --show    — verify AI peer representation")
        print(
            "  4. drewgent honcho tokens             — tune context and dialectic budgets"
        )
        print("  5. drewgent honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'drewgent memory setup'...\n")
        from drewgent_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``drewgent honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``drewgent honcho``.
    """
    import argparse
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to drewgent memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Drewgent Honcho",
    )
