...
updated: 2026-05-20
links:
  - "[[P0-brainstem/_agent/COLLAB/ENGINE_USAGE]]"
  ...
  - "[[P4-cortex/knowledge/laws-of-ux-wiki/index]]"
  - "[[P4-cortex/knowledge/obsidian-vault-site-principle]]"
  - "[[P4-cortex/plans/gateway_decomposition_plan]]"
  - "[[P4-cortex/plans/stabilization_report]]"
  - "[[P4-cortex/growth/harness-autonomous-behaviors]]"
  - "[[P4-cortex/growth/brain-signal-post-processing-20260531]]"
  - "[[P2-hippocampus/memories/index]]"
  - "[[P2-hippocampus/memories/MEMORY_wiki]]"
---


# Loragent Self-Model (P5-Ego)
...
| TTL 만료 | `_reclaim_stale_tasks()` → worker reclaim | loragent_kanban_db.py |
...
스킬 추가:
  ~/.loragent/skills/<name>/   ← SKILL.md + references/ + scripts/
...
```
P5: "이 도구/스킬은 Loragent의 identity에 어떻게 통합되는가?"
...
## Kanban — Loragent's Task Queue (Brain-Integrated)

Loragent의 kanban 시스템은 brain structure(P0~P6) 안에 완전히 녹아들어 있다.

### 상태 저장소 (P2-hippocampus)

```
~/.loragent/P2-hippocampus/kanban/
├── state/loragent_tasks.db   ← SQLite canonical store (tasks, task_links, task_events, task_runs, boards, kanban_notify_subs)
...
10개 agent tool이 `loragent_kanban_db.py`에 구현됨:
...
- `kanban-dispatcher-default` — `~/.loragent/scripts/dispatch_once_default.py`
- `kanban-dispatcher-content` — `~/.loragent/scripts/dispatch_once_content.py`
- `kanban-dispatcher-integrations` — `~/.loragent/scripts/dispatch_once_integrations.py`

각 dispatcher는 `Popen`으로 worker spawn 시 `KANBAN_WORKER_MODE=1` env flag 주입.
worker는 LLM 호출 없이 직접 sqlite3 read/write (결정론적). stdout/stderr는 `~/.loragent/P4-cortex/scripts/kanban/logs/workers/{task_id}.log`로 redirect (pipe deadlock 회피).
...
### Core Identity
- [[P1-limbic/persona/SOUL]] — P1-Limbic identity and voice (who Loragent is)
- [[P1-limbic/persona/writing-style-guide]] — Korean writing style guide for blog content (Loragent P1-limbic tone)

### Architecture & Data Flow
- [[P3-sensors/gateway/loragent-architecture-dataflow]] — **End-to-end data flow**: full message lifecycle (Platform → Gateway → AIAgent → Platform), session/memory flow, file reference table with line numbers
- [[P3-sensors/skills/SKILL-INDEX]] — Skill category index with 28 DESCRIPTION.md references
- [[P3-sensors/resolver/RESOLVER.md]] — Context routing table (on-demand document loading)

### Brain & Governance
- [[P0-brainstem/brain/rules]] — P0 Brainstem absolute rules (禁 rules, Karpathy principles)
- [[P4-cortex/knowledge/NEURONFS_RULES]] — NeuronFS file system architecture rules

### Growth & Integration
- [[P4-cortex/growth/INTEGRATION_PROTOCOL]] — Tool/Skill absorption protocol (P5-Ego integration procedure)
- [[P4-cortex/plans/gateway_decomposition_plan]] — GatewayRunner decomposition into isolated components
- [[P4-cortex/plans/stabilization_report]] — Loragent runtime stabilization report (P0-brainstem import fix, launchd label fix)

### Content & Automation
- [[P4-cortex/knowledge/laws-of-ux-wiki/index]] — Laws of UX knowledge wiki (external-link content, graph excluded)

### Planning & Memory
- [[P6-prefrontal/plans/growth-2026]] — Loragent growth plan 2026 (skills, memory, automation goals)
- [[P6-prefrontal/migrations/loragent-root-consolidation-20260506]] — 2026-05-06 root consolidation migration (canonical runtime, quarantine)
- [[P2-hippocampus/memories/SCHEMA]] — Wiki schema conventions (tags, wikilinks, entity/concept/insight structure)
