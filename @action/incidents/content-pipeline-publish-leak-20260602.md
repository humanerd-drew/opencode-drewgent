...
links:
  ...
  - "[[@memory/growth/humanerd-site-url-mapping]]"
  - "[[@memory/portfolio/quartz-publishing]]"
  - "[[skills/humanerd-site]]"
  - "[[skills/humanerd-content-status-enforcement]]"
  - "[[@action/incidents/cron-job-failure-20260518]]"
---
...
**Status**: ✅ Resolved
**Affected scope**: YOUR_DOMAIN public site

## Symptom

5/23~5/26 + 6/1 자동 생성된 5개 trend article이 human 검토 없이 YOUR_DOMAIN에 publish됨:
...
총 6개 article. 모두 YOUR_DOMAIN에 200 OK로 노출.
...
`site-spec-audit` skill으로 YOUR_DOMAIN 점검 중. Live URL 검증:

```bash
$ curl -sI https://YOUR_DOMAIN/blog/2026-05-23-gemini-cli-shutdown/ | head -1
HTTP/2 200    ← ❌ draft가 200으로 노출

$ curl -sI https://YOUR_DOMAIN/blog/2026-05-26-claude-code-essence/ | head -1
HTTP/2 200    ← ❌ draft가 200으로 노출
```
...
- [x] YOUR_DOMAIN live URL 18개 article 200 OK
- [x] YOUR_DOMAIN live URL 12개 article 404 OK
...
- [x] YOUR_DOMAIN curl 18 article 200 / 12 article 404
- [x] YOUR_DOMAIN homepage에서 3개 title 0 occurrence
...
- [[@memory/portfolio/quartz-publishing]] — pipeline overview
- [[@memory/growth/humanerd-site-url-mapping]] — 3-pillar URL mapping + rolling checklist
- [[skills/humanerd-site]] — main site skill (v2)
- [[skills/humanerd-content-status-enforcement]] — agent self-apply skill
- [[@action/incidents/cron-job-failure-20260518]] — 직전 incident
- [[@identity/brain/rules]] — P0 brainstem governance
