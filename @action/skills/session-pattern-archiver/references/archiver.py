...
# ── Config ──────────────────────────────────────────────────────────────────
VAULT         = Path("/Volumes/{{AGENT_NAME_LOWER}}_storage/_agent/MEMORY")
...
SCAN_DIRS = [
    ("brain/",          3.0),   # highest: {{AGENT_NAME}} brain rules
...
def _env(key: str, fallback="") -> str:
    p = Path.home() / ".{{AGENT_NAME_LOWER}}" / ".env"
    if p.exists():
...
CHANNEL_DIR       = Path.home() / ".{{AGENT_NAME_LOWER}}" / "channel_directory.json"
THREAD_STATE_FILE = Path.home() / ".{{AGENT_NAME_LOWER}}" / "discord_threads.json"
...
**Auto-generated by {{AGENT_NAME}} Session Archiver**  
