...
{{AGENT_NAME}} 에이전트의 스킬 활용을 주기적으로 분석하여 **미사용 스킬의 활용 가능성을 추천**하고, **삭제 고려 대상을 표시**하는 자기反省 시스템.
...
> 최종 판단은 사용자의 몫입니다. {{AGENT_NAME}}가 임의로 삭제하지 않습니다.
...
│                                                         │
│  ~/.{{AGENT_NAME_LOWER}}/memories/                                  │
│    → MEMORY.md (키워드 추출)                            │
│    → insights/ recent 노트들                           │
│    → entities/ (환경/선호도 맥락)                      │
│                                                         │
│  ~/.{{AGENT_NAME_LOWER}}/sessions/session_*.json                   │
│    → 최근 10개 세션에서 user 메시지 키워드 추출         │
│                                                         │
│  ~/.{{AGENT_NAME_LOWER}}/skills/*/SKILL.md                        │
...
4. **삭제 권유가 아님** — 최종 판단은 사용자. {{AGENT_NAME}}는 표시만 함
...
1. `~/.{{AGENT_NAME_LOWER}}/state.db`에 읽기 권한
2. `~/.{{AGENT_NAME_LOWER}}/skills/`에 스킬 목록 존재
3. `~/.{{AGENT_NAME_LOWER}}/memories/`에 MEMORY.md 존재

스크립트를 직접 실행하여 확인할 수 있습니다:

```bash
python3 ~/.{{AGENT_NAME_LOWER}}/skills/skill-gym/scripts/gather_skill_data.py | python3 -c "
import json, sys
