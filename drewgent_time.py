"""
Timezone-aware clock for Drewgent.

Provides a single ``now()`` helper that returns a timezone-aware datetime
based on the user's configured IANA timezone (e.g. ``Asia/Kolkata``).

Resolution order:
  1. ``DREW_TIMEZONE`` environment variable
  2. ``timezone`` key in ``~/.drewgent/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — Drewgent never
...
try:
    ...
    # Python 3.8 fallback (shouldn't be needed — Drewgent requires 3.9+)
...
def _resolve_timezone_name() -> str:
    ...
    tz_env = os.getenv("DREW_TIMEZONE", "").strip()
    if tz_env:
        return tz_env
