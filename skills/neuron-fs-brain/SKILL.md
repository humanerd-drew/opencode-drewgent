...
description: NeuronFS-style brain governance for {{AGENT_NAME}}. Manages filesystem-based AI constraints via folder structure and special tokens (vorq, 禁).
version: 1.0.0
...
This skill implements NeuronFS-style brain governance for {{AGENT_NAME}}, treating the filesystem as a constraint engine where folder structure and special tokens force AI behavior.
...
ACTION: 
1. Check ~/.{{AGENT_NAME_LOWER}}/brain/active/禁/my_pattern/
...
```
[[@identity/brain/rules]]     ← P0, brainstem layer
[[@identity/persona/SOUL]]      ← P1, limbic layer
[[@identity/SELF_MODEL]]           ← P5, ego layer
[[@memory/knowledge/NEURONFS_RULES]] ← P4, cortex layer
```
...
{{AGENT_NAME}} vault에는 중복 파일명이 존재한다:

| Short name | 중복 수 | Canonical path (올바른 링크) |
|------------|---------|------------------------------|
| `[[SOUL]]` | **4** | `[[@identity/persona/SOUL]]` |
| `[[index]]` | **29** | `[[@memory/memories/index]]` |
| `[[SCHEMA]]` | **3** | `[[@memory/memories/SCHEMA]]` |
...
```
~/.{{AGENT_NAME_LOWER}}/brain/myproject/P0-brainstem/禁secrets_in_code.neuron
~/.{{AGENT_NAME_LOWER}}/brain/myproject/P1-limbic/professional_tone.neuron
~/.{{AGENT_NAME_LOWER}}/brain/myproject/P4-cortex/python_patterns.neuron
```
...
- Verify `~/.{{AGENT_NAME_LOWER}}/brain/active_brain.txt` exists
...
NeuronFS brain governance 외에도, {{AGENT_NAME}}의 **에이전트 내비게이션과 결정 맥락 기록**은 `AGENTS.md`에 통합되어 있다.

### AGENTS.md

`~/.{{AGENT_NAME_LOWER}}/AGENTS.md`는 현재 아키텍처, 파일 구조, 에이전트가 참조해야 할 문서들을 정의한 **agent-first navigation guide**다. 이 파일은 세션 시작 시 컨텍스트에 자동 로드된다.
...
`references/taste-framework.md`는 Pratik Bhavsar의 "How to Be a 30x AI Engineer with a Taste"를 요약한 참고 자료다. Recognition/Compass/Vision taste의 세 가지 형태와 5가지 가치 창출 영역을 정의하며, {{AGENT_NAME}}의 아키텍처 결정을 평가하는 프레임워크로 사용할 수 있다.
