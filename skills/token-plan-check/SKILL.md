...
description: "MiniMax Token Plan 사용량/리셋을 터미널에서 즉시 확인하거나, zsh RPROMPT statusline으로 영구 통합하는 절차. 사용자가 토큰 얼마나 남았는지, 리셋까지 얼마인지, 주간 한도 현황을 물을 때 사용. API endpoint: GET https://api.minimax.io/v1/api/openplatform/coding_plan/remains. 기존 script 위치: ~/.{{AGENT_NAME_LOWER}}/scripts/minimax_usage.py. Statusline 파일: ~/.{{AGENT_NAME_LOWER}}/.zshrc_aliases."
title: Token Plan Check — Terminal Usage & Statusline
domain: brain
space: growth
type: workflow
tags: [token-plan, minimax, statusline, rprompt, zsh, terminal, {{AGENT_NAME_LOWER}}]
created: 2026-06-02
updated: 2026-06-02
links:
...
```bash
API_KEY=$(grep "^MINIMAX_API_KEY=" ~/.{{AGENT_NAME_LOWER}}/.env | cut -d= -f2-)
...
1. `~/.zshrc`에 한 줄 추가:
   ```bash
   source ~/.{{AGENT_NAME_LOWER}}/.zshrc_aliases
   ```
...
## 4단계: {{AGENT_NAME}}에서 활용

에이전트가 "내 토큰 상황 알려줘" 또는 "리셋까지 몇 분?" 류 task 받으면:

```bash
# Option A: RPROMPT 모드 호출 (cache-first, 1-2초 응답)
python3 ~/.{{AGENT_NAME_LOWER}}/scripts/minimax_usage.py --rprompt

# Option B: 즉시 fresh 데이터 (API 직접, ~500ms)
python3 ~/.{{AGENT_NAME_LOWER}}/scripts/minimax_usage.py

# Option C: JSON 파싱해서 structured 답변
python3 ~/.{{AGENT_NAME_LOWER}}/scripts/minimax_usage.py --json | jq '.model_remains[] | select(.model_name=="general") | {interval_used: (100 - .current_interval_remaining_percent), weekly_used: (100 - .current_weekly_remaining_percent), reset_in_h: (.remains_time / 3600000)}'
```
...
```bash
# 1. syntax check
python3 -c "import ast; ast.parse(open('~/.{{AGENT_NAME_LOWER}}/scripts/minimax_usage.py').read())"

# 2. 직접 호출 — 200 OK
~/.{{AGENT_NAME_LOWER}}/scripts/minimax_usage.py
# → "Token Plan usage" 헤더 + general/video 섹션

# 3. RPROMPT 모드 — 1줄 출력
~/.{{AGENT_NAME_LOWER}}/scripts/minimax_usage.py --rprompt
# → "Token Plan  5h:N% XhYm  ·  wk:N% XdYh"

# 4. zsh 통합 — source 후 RPROMPT 설정 확인
zsh -c 'source ~/.{{AGENT_NAME_LOWER}}/.zshrc_aliases; echo "RPROMPT=$RPROMPT"'
# → "RPROMPT=$(_mm_rprompt)"

# 5. cache lock cleanup
ls ~/.{{AGENT_NAME_LOWER}}/cache/minimax_usage.json.lock 2>/dev/null
...
**수정**: `~/.{{AGENT_NAME_LOWER}}/.zshrc_aliases` 에 `setopt prompt_subst 2>/dev/null` 한 줄 추가.
**검증**:
```bash
zsh -c 'source ~/.{{AGENT_NAME_LOWER}}/.zshrc_aliases
...
### P9: "{{AGENT_NAME}}이 동작 안 한 터미널에도 RPROMPT 나옴" (scope 오해)
**증상**: 사용자가 평소 쓰는 zsh 터미널 ({{AGENT_NAME_LOWER}} 실행 안 함) 에도 Token Plan RPROMPT 표시됨. "{{AGENT_NAME_LOWER}} 터미널에서만 나올 줄 알았는데 왜 로컬에도 따라다니지?" 라는 feedback.
**원인**: RPROMPT는 zsh 전역 기능. source .zshrc_aliases 만 하면 모든 zsh shell (iTerm, Terminal.app, tmux, ssh, vscode integrated terminal) 에 적용. "{{AGENT_NAME}}" 래 wrapping이 아니라 zsh 레벨.
**오해 패턴**: agent가 "tmux/ssh 공통" 이라고 디자인한 의도를 사용자는 "{{AGENT_NAME_LOWER}} 만" 으로 해석. **양방향 모두 가능성 있음** — 사용자가 명확히 의도 말하지 않으면 추측하면 안 됨.
**수정**: tty 기반 marker file 로 scope 한정.
- {{AGENT_NAME}} wrapper 함수: launch 시 `touch /tmp/{{AGENT_NAME_LOWER}}-rprompt-${tty##*/}`, exit 시 `rm`.
- `_mm_rprompt()`: marker 없으면 즉시 return (RPROMPT 비움).
- 결과: {{AGENT_NAME_LOWER}} 실행 중인 tty만 RPROMPT 켜짐. 다른 터미널 / tmux pane / ssh 세션은 자동으로 꺼짐.

**zsh function 정의 함정**: `{{AGENT_NAME_LOWER}}` 가 alias로 이미 정의돼있으면 `{{AGENT_NAME_LOWER}}() { ... }` 가 parse error (`defining function based on alias`).
- 해결: `unalias {{AGENT_NAME_LOWER}} 2>/dev/null` 먼저 호출 후 function 정의.
- zsh 동작: function > alias 이므로 unalias 하면 function 이 우선.

**검증 시나리오**:
| 상태 | tty1 ({{AGENT_NAME_LOWER}}) | tty2 (idle) |
|---|---|---|
| 둘 다 fresh | (empty) | (empty) |
| tty1에서 {{AGENT_NAME_LOWER}} 실행 | on | (empty) |
| tty1 {{AGENT_NAME_LOWER}} 종료 | (empty) | (empty) |
| tmux pane1={{AGENT_NAME_LOWER}}, pane2=idle | on | (empty) |

**교훈**:
- "전역/특정 컨텍스트" 디자인은 사용자에게 명시적으로 확인. 추측으로 디자인하면 양쪽 다 어긋남.
- "Universal tmux/ssh"와 "{{AGENT_NAME_LOWER}} only"는 **mutually exclusive design choices**. 사용자 의도가 모호하면 둘 중 하나 정해달고 묻기.
- Scope 한정이 필요할 때의 정석: marker file (per-tty) 또는 env var. PID 검사 (parent process) 는 fragile.

### P10: "{{AGENT_NAME_LOWER}}이 안 켜져요" — Incomplete class rename from upstream fork
**증상**: `{{AGENT_NAME_LOWER}}` 또는 `{{AGENT_NAME_LOWER}} acp --stdio` 실행 시:
```
2026-06-02 17:17:09 [INFO] acp_adapter.entry: Loaded env from ~/.{{AGENT_NAME_LOWER}}/.env
2026-06-02 17:17:09 [INFO] acp_adapter.entry: Starting {{AGENT_NAME_LOWER}}-agent ACP adapter
ACP dependencies not installed.
Install them with:  pip install -e '.[acp]'
```
**진짜 에러**: `{{AGENT_NAME_LOWER}}_cli/main.py`의 `cmd_acp()` 가 `try/except ImportError` 로 `acp_main()` 호출. 실제 ImportError 메시지가 catch되어 "ACP dependencies not installed" 로 출력. **사용자에게는 가짜 메시지**가 보임.

**진단 단계**:
1. `pip install -e '.[acp]'` 시도 → 그래도 실패 (venv의 pip shebang 깨졌을 수 있음 — `~/{{AGENT_NAME_LOWER}}_workspace/...` 옛 path 가리킴)
2. `python -m pip install -e .` 로 editable install → 일단 됨
3. `{{AGENT_NAME_LOWER}} acp --stdio` 재시도 → 여전히 같은 에러
...
5. `python -c "from acp_adapter.entry import main; main()"` 실행 → 진짜 에러 보임:
   ```
   ImportError: cannot import name '{{AGENT_NAME}}ACPAgent' from 'acp_adapter.server'
   ```

**진짜 root cause**: 2026-04-08 initial fork commit (`6409efb72 Initial fork: hermes-agent → {{AGENT_NAME}} Agent`) 에서 Hermes → {{AGENT_NAME}} rebrand 시 `acp_adapter/entry.py`만 `{{AGENT_NAME}}ACPAgent`로 import 하도록 변경, `acp_adapter/server.py`의 class 정의는 `HermesACPAgent` 그대로 남음. **불완전한 rename**.
...
+class {{AGENT_NAME}}ACPAgent(acp.Agent):
+    """ACP Agent implementation wrapping the AIAgent core."""
```

**검증**:
- `python -c "from acp_adapter.entry import main"` → OK
- `{{AGENT_NAME_LOWER}} acp --stdio` 실행 → ACP 부팅 → JSON-RPC `initialize` 응답:
  ```json
  {"jsonrpc":"2.0","id":1,"result":{"agentInfo":{"name":"{{AGENT_NAME_LOWER}}-agent","version":"0.7.1"},...}}
  ```
...
1. 사용자: "{{AGENT_NAME_LOWER}} 안 켜져" → 1차 가설: wrapper 문제
...
**증상**: `{{AGENT_NAME_LOWER}}` 또는 `{{AGENT_NAME_LOWER}} acp --stdio` 실행 시:
```
[INFO] acp_adapter.entry: Loaded env from ~/.{{AGENT_NAME_LOWER}}/.env
[INFO] acp_adapter.entry: Starting {{AGENT_NAME_LOWER}}-agent ACP adapter
...
- 그러므로 plain terminal UX는 `{{AGENT_NAME_LOWER}} chat` 이 정답.

**해결**:
- wrapper 함수가 `command "$DREW_BIN" acp --stdio "$@"` 호출 → `command "$DREW_BIN" chat "$@"` 로 변경.
- `{{AGENT_NAME_LOWER}} chat` 은 prompt_toolkit 기반 TUI 채팅. user 가 stdin 으로 직접 메시지 입력, agent 가 stdout 으로 응답.
...
**`{{AGENT_NAME_LOWER}} chat --help` 핵심 옵션**:
...
timeout 1 ~/.local/bin/{{AGENT_NAME_LOWER}} acp --stdio </dev/null
# → "ACP client connected" 후 hang (timeout kill)

timeout 1 ~/.local/bin/{{AGENT_NAME_LOWER}} chat </dev/null
...
- [[@identity/brain/{{AGENT_NAME}}-brain/P0-brainstem/禁/禁secrets_in_code.neuron]] — API key는 .env, 절대 코드에 금지
- [[@identity/brain/{{AGENT_NAME}}-brain/P0-brainstem/禁/禁filesystem_truth.neuron]] — 캐시 = truth, stale이면 표시
- `~/.{{AGENT_NAME_LOWER}}/scripts/minimax_usage.py` — 메인 스크립트
- `~/.{{AGENT_NAME_LOWER}}/.zshrc_aliases` — zsh 통합 파일
- `~/.{{AGENT_NAME_LOWER}}/cache/minimax_usage.json` — 캐시

### P12: {{AGENT_NAME}} chat TUI status bar에 Token Plan 통합
**배경**: 기존 status bar는 모델, context, %, session duration을 표시. Token Plan (5h interval + weekly)도 같은 줄에 통합하면 user가 한 번에 모든 사용량 확인 가능.

**구현 위치**: `cli.py:{{AGENT_NAME}}CLI` ({{AGENT_NAME}} chat TUI)
...
**Cache read 동작**:
- `~/.{{AGENT_NAME_LOWER}}/cache/minimax_usage.json` read (synchronous, ~5ms)
