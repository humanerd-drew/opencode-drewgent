...
```bash
grep -rn "Done" ~/.loragent/P0-brainstem ~/.loragent/P4-cortex ~/.loragent/P2-hippocampus 2>/dev/null | head -20
grep -rn "tools/.*\.py" ~/.loragent/P4-cortex/growth 2>/dev/null | head -10
```

"tools/loragent_kanban_db.py (Phase 1) Done" 같은 줄 추출.
...
grep -rn "threshold:" ~/.loragent/P4-cortex/growth 2>/dev/null | head -10
grep -rn "0.5 → 0.9\|0.5 -> 0.9" ~/.loragent/P4-cortex ~/.loragent/P6-prefrontal 2>/dev/null | head -10
grep -rn "applied\|patched\|configured" ~/.loragent/P4-cortex/growth 2>/dev/null | head -10
```
...
```bash
# 2a. 단일 path 직접 확인
test -f ~/.loragent/source/loragent-agent/tools/loragent_kanban_db.py && echo EXISTS || echo MISSING

# 2b. 이름으로 전체 검색
find ~/.loragent -name "loragent_kanban_db.py" -type f 2>/dev/null

# 2c. wildcard 패턴 검증
ls ~/.loragent/source/*/tools/*.py 2>/dev/null | wc -l
```

### Step 3: Symlink / Root confusion 처리

Loragent는 root가 바뀔 수 있음:
- 옛 root: `~/.loragent/source/claude-code`
- 신 root: `~/.loragent/source/loragent-agent`
- quarantine: `~/.loragent/P6-prefrontal/archive/quarantine-.../`

```bash
readlink -f ~/.loragent/source/loragent-agent 2>/dev/null
ls -la ~/.loragent/source/ 2>/dev/null
ls -la ~/.loragent/P6-prefrontal/archive/ 2>/dev/null
```
...
| ~/.loragent/source/old/ path | 새 root로 이동 | MEDIUM (outdated) |
...
- brain root를 source root로 착각 — `~/.loragent` (brain home) ≠ `~/.loragent/source/loragent-agent` (code root)
- quarantine 안에 진짜 있을 수도 — 옛 파일이 archive로 이동했을 수 있음
- vault path ≠ filesystem path — wikilink `[[P4-cortex/...]]` ↔ `~/.loragent/P4-cortex/...` 변환 필요
...
N_REAL=$(find ~/.loragent/source -name "*.py" | wc -l)
echo "claimed=$N_CLAIM real=$N_REAL"
```

## Example (2026-06-01)

- Claim: tools/loragent_kanban_db.py Done
- Reality: find 0 hits
...
- [[P0-brainstem/brain/Loragent-brain/P0-brainstem/禁/禁filesystem_truth.neuron]] — P0 강제 규칙
- [[P5-ego/SELF_MODEL]] — self-model에 path claim
- [[P6-prefrontal/migrations/loragent-root-consolidation-20260506]] — root 변경 migration
