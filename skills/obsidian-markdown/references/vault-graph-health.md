...
Techniques to ensure Obsidian Graph View shows meaningful connections in an AI agent vault (Loragent P0-P6 architecture).
...
```bash
grep -rln '\[\[TargetSlug\]\]' .loragent/  # find inbound links
```
...
`loragent_graph_gap_analysis.sh` checks:
...
du -sh ~/.loragent/cron/output/*/ | sort -rh | head -10
```
...
2. **Cron 출력 정리** (선택):
   ```bash
   mv ~/.loragent/cron/output/<job_id> ~/.loragent/.trash/
   ```
...
```bash
find ~/.loragent -name '*.md' -not -path '*/cron/*' -not -path '*/.trash/*' \
