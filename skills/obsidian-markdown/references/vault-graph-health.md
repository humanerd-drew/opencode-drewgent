...
Techniques to ensure Obsidian Graph View shows meaningful connections in an AI agent vault ({{AGENT_NAME}} P0-P6 architecture).
...
```bash
grep -rln '\[\[TargetSlug\]\]' .{{AGENT_NAME_LOWER}}/  # find inbound links
```
...
`{{AGENT_NAME_LOWER}}_graph_gap_analysis.sh` checks:
...
du -sh ~/.{{AGENT_NAME_LOWER}}/cron/output/*/ | sort -rh | head -10
```
...
2. **Cron 출력 정리** (선택):
   ```bash
   mv ~/.{{AGENT_NAME_LOWER}}/cron/output/<job_id> ~/.{{AGENT_NAME_LOWER}}/.trash/
   ```
...
```bash
find ~/.{{AGENT_NAME_LOWER}} -name '*.md' -not -path '*/cron/*' -not -path '*/.trash/*' \
