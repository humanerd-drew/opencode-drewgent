...
metadata:
  hermes:
    ...
    related_skills: [plan, systematic-debugging, simplify-code, python-large-file-patch-{{AGENT_NAME_LOWER}}]
links:
...
11. **Wrong project copy (Synology Drive dual-sync).** When the project is synced via Synology Drive, macOS creates separate mount points per sync task (e.g., `SynologyDrive-{{AGENT_NAME_LOWER}}/` and `SynologyDrive-Log-Project/`). A project may exist in MULTIPLE directories with different versions and package.json. Always confirm which copy the user is running from before editing files. Use `pwd`, `git log --oneline -1`, or check `package.json` version field.
