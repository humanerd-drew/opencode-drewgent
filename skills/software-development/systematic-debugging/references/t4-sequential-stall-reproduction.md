...
grep "Running job" ~/.{{AGENT_NAME_LOWER}}/P6-prefrontal/logs/gateway.log | tail -5

# 4. What jobs are due?
python3 -c "
import json
d = json.load(open('~/.{{AGENT_NAME_LOWER}}/cron/jobs.json'))
for j in d.get('jobs', []):
    if j.get('enabled'):
...
2. **`{{AGENT_NAME_LOWER}}_cron_watchdog.sh`** auto-kickstarts if 0 fires in 5 min.
...
grep "Running job" ~/.{{AGENT_NAME_LOWER}}/P6-prefrontal/logs/gateway.log | tail -5
# Expected:
#   ... Running job 'kanban-dispatcher (all boards, consolidated)' (ID: {{AGENT_NAME_LOWER}}-cron-runner-001)
# NOT expected:
