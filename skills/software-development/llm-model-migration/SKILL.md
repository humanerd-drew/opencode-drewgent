...
space: growth
type: workflow
tags: [llm, model-migration, provider-update, token-plan, loragent]
created: 2026-06-01
updated: 2026-06-01
links:
  ...
  - "[[P0-brainstem/brain/Loragent-brain/P0-brainstem/禁/禁blind_write.neuron]]"
  - "[[P0-brainstem/brain/Loragent-brain/P0-brainstem/禁/禁karpathy_coding_principles.neuron]]"
...
Loragent vault는 같은 파일이 여러 위치에 복사돼있을 수 있다. catalog/config 단일 파일로 가정하면 안 됨:

```bash
# catalog 후보 파일이 몇 개 있는지 먼저 파악
find ~/.loragent/source -name "model_metadata.py" 2>/dev/null
find ~/.loragent/P0-brainstem -name "model_metadata.py" 2>/dev/null
find ~/.loragent -maxdepth 6 -name "config.yaml" 2>/dev/null
```

**왜 중요한가**: 같은 `model_metadata.py`가 보통 3군데 있어:
- `source/loragent-agent/agent/` — canonical (runtime이 import)
...
- [ ] `MiniMaxAI/MiniMax-M3` entry가 canonical(`source/loragent-agent/...`)에 있는지가 1순위
- [ ] frozen copy는 통상 **변경하지 않음** — 만약 변경됐다면 잘못된 위치로 적용된 것
- [ ] provider model list (`loragent_cli/models.py`)는 catalog와 별개 — 둘 다 migration 필요
...
- Loragent vault 구조상 같은 파일이 3~4개 위치에 존재 가능:
...
- `loragent_cli/models.py` (또는 provider config)는 **provider가 노출하는 모델 목록** (e.g. `["MiniMax-M3", "MiniMax-M3-highspeed"]`)
...
- **대응**:
  - Step 2 grep inventory에 provider list도 포함 (e.g. `loragent_cli/models.py`, `config.yaml`의 model/provider 섹션)
...
- **6-spot rule (catalog claim 동기화 시)**:
  ...
  Production 4 spots:
    top/models.py
    top/setup.py (또는 top/loragent_cli/setup.py)
    source/models.py
    source/setup.py
...
- **대응**:
  - Step 2 grep inventory를 **mirror 양쪽**에 동시 수행:
    ...
    #   git grep -l "minimax-m2.7" -- 'source/loragent-agent/loragent_cli/' 'loragent_cli/'
    #   git grep -l "minimax-m2.7" -- 'source/loragent-agent/tests/' 'tests/'
    ```
  - Step 6-3 grep sweep에 test mirror 별도 추가:
    ```bash
    git grep -l "M2.7" -- 'tests/test_setup_model_selection.py' \
        'source/loragent-agent/tests/test_setup_model_selection.py'
    ```
...
- **flip 후 검증 한 줄 명령**:
  ```bash
  # 6 spots 일관성 한 번에 확인
  for p in loragent_cli/models.py loragent_cli/setup.py \
           source/loragent-agent/loragent_cli/models.py source/loragent-agent/loragent_cli/setup.py \
           tests/test_setup_model_selection.py source/loragent-agent/tests/test_setup_model_selection.py; do
...
- [ ] **Provider model list (`loragent_cli/models.py`) 별도 grep** — catalog만 추가하고 provider list 빠뜨리지 않았는지
...
| **P1 (보이는 UI)** | `loragent_cli/models.py`, `loragent_cli/setup.py` | provider catalog — `/model` picker에 M3 안 뜸 |
...
- `loragent_cli/models.py`의 `minimax: [...]` list에는 M3 없음
...
- loragent_cli/models.py L146-152: provider catalog에 M3 없음  ❌
- loragent_cli/setup.py L121-122: 동일  ❌
...
- **발견 방법**:
  ...
  # git grep -l "minimax-m2.7" -- 'tests/test_*' 'source/loragent-agent/tests/test_*'
...
- [[P0-brainstem/brain/Loragent-brain/P0-brainstem/禁/禁task_qa_gate.neuron]] — verification 단계
- [[P0-brainstem/brain/Loragent-brain/P0-brainstem/禁/禁karpathy_coding_principles.neuron]] — surgical changes, scope 준수
- [[P0-brainstem/brain/Loragent-brain/P0-brainstem/禁/禁filesystem_truth.neuron]] — audit mode의 governance 근거 (CHANGELOG 신뢰 X, filesystem = truth)
