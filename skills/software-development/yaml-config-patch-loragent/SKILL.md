---
name: yaml-config-patch-loragent
description: Patch Loragent config.yaml safely — handles the dual-config pattern (~/.loragent/config.yaml + P5-ego/config/config.yaml) and avoids mcp_patch YAML gotchas
links:
  - "[[P3-sensors/skills/SKILL-INDEX]]"
  - "[[P0-brainstem/brain/rules]]"
---

# YAML Config Patch — Loragent Dual-Config Pattern

Loragent는 config.yaml을 **두 곳에 중복**으로 유지한다. compression이나
...
Loragent는 두 개의 config.yaml을 운영한다:

| File | Used by | What it has |
|---|---|---|
| `~/.loragent/config.yaml` | gateway, CLI, cron scheduler, run_kanban_worker | `auxiliary.compression` (provider/model) 만 |
| `~/.loragent/P5-ego/config/config.yaml` | P5-ego layer (identity snapshot) | `compression:` (summary_*) + `auxiliary.compression:` (provider/model) 둘 다 |
...
**변경 위치 3곳**:
1. `~/.loragent/config.yaml` `auxiliary.compression.{provider, model}`
2. `~/.loragent/P5-ego/config/config.yaml` `auxiliary.compression.{provider, model}`
3. `~/.loragent/P5-ego/config/config.yaml` `compression.{summary_provider, summary_model}`
...
- `~/.loragent/P6-prefrontal/plans/path-integrity-report-2026-05-17` — config 중복 의도
- `~/.loragent/P4-cortex/knowledge/token-compression-headroom-20260602` — cost 결과 패턴
...
- `~/.loragent/source/loragent-agent/agent/model_metadata.py:130` — MiniMax-M2.5 catalog entry (204800, legacy)
