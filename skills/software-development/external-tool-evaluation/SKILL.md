...
description: Evaluate a third-party GitHub repo or external tool for Loragent integration. Use raw markdown curl + GitHub API to bypass UI, identify algorithm taxonomy, map to Loragent hot spots, score 3-4 integration modes, recommend POC-first.
domain: software-development
space: outcome
type: workflow
tags: [external-tool, evaluation, github, integration, loragent]
created: 2026-06-02
updated: 2026-06-14
links:
...
# External Tool Evaluation — Loragent Integration Workflow

## When to use

User shares a GitHub repo link OR asks "what about X for Loragent" / "should we use X" / "evaluate X for integration." Trigger keywords: "토큰 똑똑하게", "이 도구 쓸 수 있을까", "X 통합 검토", "headroom / RTK / lean-ctx / kompress / X 평가해줘".
...
### 5. Map to Loragent hot spots
For each algorithm/feature, list Loragent files that would benefit:
...
- **Hermes skill** (`~/.loragent/skills/<category>/SKILL.md`) — injected per task, Loragent-native
...
### 6. Score integration modes against Loragent
...
- C. **Wrap CLI** (`headroom wrap claude`) — conflicts with our `loragent` wrapper
...
- E. **Ruleset-only adoption** (for agent behavior rulesets only) — copy the portable ruleset into Loragent's AGENTS.md or a Hermes skill. Zero code change, no vendor lock-in. The plugin/commands layer is platform-specific (Claude Code vs Codex vs Cursor) and should be evaluated separately. Scoring axes differ: instead of latency/code-change, evaluate **ruleset completeness** (does it guardrail security?), **deferral hygiene** (are shortcuts tracked?), and **over-minimization risk** (what abstractions does it skip that Loragent needs?).
...
- **Unified vault** (Loragent's `.loragent/P0-P6`): agent architecture + knowledge in one tree with wikilinks between policy, experience, and memory. Simpler, no cross-repo sync, but harder to swap agent frameworks.

**Decision heuristic**: If the tool is primarily a *search/retrieval engine* (embedding, vector DB, knowledge graph), **MCP integration** is cleaner than repo restructuring — Loragent keeps its unified vault, the tool serves as an additional retrieval layer.
...
- **Don't propose proxy mode by default** — Loragent's gateway is already a proxy. Adding another proxy layer creates debugging hell. Library/SDK in our existing call site is almost always better.
- **Benchmarks are on the tool's own workloads** — 92% savings on "code search" may not translate to Loragent's pattern. Always map to our actual hot spots.
...
- [ ] Loragent hot spots identified with file paths
- [ ] 3-4 integration modes scored against Loragent's gateway constraint
...
- [[P4-cortex/knowledge/garry-tan-unified-architecture-loragent-review]] — "fat skills, thin harness" lens for evaluating if the tool's complexity fits
