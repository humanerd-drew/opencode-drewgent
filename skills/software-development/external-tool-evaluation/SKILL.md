...
description: Evaluate a third-party GitHub repo or external tool for {{AGENT_NAME}} integration. Use raw markdown curl + GitHub API to bypass UI, identify algorithm taxonomy, map to {{AGENT_NAME}} hot spots, score 3-4 integration modes, recommend POC-first.
domain: software-development
space: outcome
type: workflow
tags: [external-tool, evaluation, github, integration, {{AGENT_NAME_LOWER}}]
created: 2026-06-02
updated: 2026-06-14
links:
  - "[[@memory/growth/INTEGRATION_PROTOCOL]]"
  - "[[@action/skills/SKILL-INDEX]]"
  - "[[@memory/knowledge/garry-tan-unified-architecture-{{AGENT_NAME_LOWER}}-review]]"
...
### 5. Map to {{AGENT_NAME}} hot spots
For each algorithm/feature, list {{AGENT_NAME}} files that would benefit:
...
- **Hermes skill** (`~/.{{AGENT_NAME_LOWER}}/skills/<category>/SKILL.md`) — injected per task, {{AGENT_NAME}}-native
...
### 6. Score integration modes against {{AGENT_NAME}}
...
- C. **Wrap CLI** (`headroom wrap claude`) — conflicts with our `{{AGENT_NAME_LOWER}}` wrapper
...
- E. **Ruleset-only adoption** (for agent behavior rulesets only) — copy the portable ruleset into {{AGENT_NAME}}'s AGENTS.md or a Hermes skill. Zero code change, no vendor lock-in. The plugin/commands layer is platform-specific (Claude Code vs Codex vs Cursor) and should be evaluated separately. Scoring axes differ: instead of latency/code-change, evaluate **ruleset completeness** (does it guardrail security?), **deferral hygiene** (are shortcuts tracked?), and **over-minimization risk** (what abstractions does it skip that {{AGENT_NAME}} needs?).
...
- **Unified vault** ({{AGENT_NAME}}'s `.{{AGENT_NAME_LOWER}}/P0-P6`): agent architecture + knowledge in one tree with wikilinks between policy, experience, and memory. Simpler, no cross-repo sync, but harder to swap agent frameworks.

**Decision heuristic**: If the tool is primarily a *search/retrieval engine* (embedding, vector DB, knowledge graph), **MCP integration** is cleaner than repo restructuring — {{AGENT_NAME}} keeps its unified vault, the tool serves as an additional retrieval layer.
...
- **Don't propose proxy mode by default** — {{AGENT_NAME}}'s gateway is already a proxy. Adding another proxy layer creates debugging hell. Library/SDK in our existing call site is almost always better.
- **Benchmarks are on the tool's own workloads** — 92% savings on "code search" may not translate to {{AGENT_NAME}}'s pattern. Always map to our actual hot spots.
...
- [ ] {{AGENT_NAME}} hot spots identified with file paths
- [ ] 3-4 integration modes scored against {{AGENT_NAME}}'s gateway constraint
...
- [[@memory/knowledge/garry-tan-unified-architecture-{{AGENT_NAME_LOWER}}-review]] — "fat skills, thin harness" lens for evaluating if the tool's complexity fits
