---
name: yaml-config-patch-{{AGENT_NAME_LOWER}}
description: Patch {{AGENT_NAME}} config.yaml safely — handles the dual-config pattern (~/.{{AGENT_NAME_LOWER}}/config.yaml + P5-ego/config/config.yaml) and avoids mcp_patch YAML gotchas
links:
...
{{AGENT_NAME}}는 config.yaml을 **두 곳에 중복**으로 유지한다. compression이나
...
|---|---|---|
| `~/.{{AGENT_NAME_LOWER}}/config.yaml` | gateway, CLI, cron scheduler, run_kanban_worker | `auxiliary.compression` (provider/model) 만 |
| `~/.{{AGENT_NAME_LOWER}}/P5-ego/config/config.yaml` | P5-ego layer (identity snapshot) | `compression:` (summary_*) + `auxiliary.compression:` (provider/model) 둘 다 |
...
**변경 위치 3곳**:
1. `~/.{{AGENT_NAME_LOWER}}/config.yaml` `auxiliary.compression.{provider, model}`
2. `~/.{{AGENT_NAME_LOWER}}/P5-ego/config/config.yaml` `auxiliary.compression.{provider, model}`
3. `~/.{{AGENT_NAME_LOWER}}/P5-ego/config/config.yaml` `compression.{summary_provider, summary_model}`
...
- `~/.{{AGENT_NAME_LOWER}}/P6-prefrontal/plans/path-integrity-report-2026-05-17` — config 중복 의도
- `~/.{{AGENT_NAME_LOWER}}/P4-cortex/knowledge/token-compression-headroom-20260602` — cost 결과 패턴
...
- `~/.{{AGENT_NAME_LOWER}}/source/{{AGENT_NAME_LOWER}}-agent/agent/model_metadata.py:130` — MiniMax-M2.5 catalog entry (204800, legacy)
