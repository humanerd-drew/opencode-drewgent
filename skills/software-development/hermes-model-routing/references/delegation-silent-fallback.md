...
`~/.{{AGENT_NAME_LOWER}}/agents/<name>.md` 또는 `~/.hermes/agents/<name>.md` profile에 model 박혀있으면 config보다 우선. 역할별 세분화 가능.

```python
delegate_task(
    goal="Review this PR for security issues",
    agent_profile="reviewer",   # ~/.{{AGENT_NAME_LOWER}}/agents/reviewer.md 사용
    context="..."
)
```
