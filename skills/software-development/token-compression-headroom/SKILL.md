---
title: headroom token compression — Loragent POC + integration guide
name: token-compression-headroom
description: How to measure and cut token cost on Loragent tool outputs. Two paths — headroom_ai library (POC done, deferred) and Loragent-native 4-layer cap pattern (3 patches live, 78-80% savings). Auto-apply the cap pattern whenever you add or modify a tool that emits over 5K chars of JSON. Use when evaluating token compression, seeing high token cost on tool output, or adding a new tool that returns large records.
...
links:
  ...
  - "[[P0-brainstem/brain/Loragent-brain/P0-brainstem/禁/禁task_qa_gate.neuron]]"
  - "[[P0-brainstem/brain/rules]]"---

# headroom (token compression) — Loragent POC + integration guide
...
4. **Apply the pattern (see "Loragent-native alternative" section below for full code template).**
...
You're evaluating `headroom-ai` (chopratejas/headroom, 4.3k stars, 60B+ tokens saved) for Loragent, OR you're cutting token cost on Loragent tool outputs. Use this skill to:
- Install headroom-ai in Loragent's Python 3.14 venv (ABI3 workaround required)
- Run a POC to verify the 60-92% claim on real Loragent tool outputs
- Apply the **Loragent-native 4-layer cap pattern** (no library needed) — see "Loragent-native alternative" section
...
PYO3_USE_ABI3_FORWARD_COMPATIBILITY=1 \
  ~/.loragent/source/loragent-agent/.venv/bin/python3 -m pip install headroom-ai
```
...
## Expected results — Loragent tool inventory
...
- **Loragent-native 4-layer cap** (see next section) — no library, surgical patches, 78-80% savings on kanban/read
- Loragent's existing `agent/context_compressor.py` (conversation-level summary, 0.9 threshold, M3 1M compatible)
- Custom JSON-specific compression (schema-aware row drop)

## Loragent-native alternative — 4-layer cap pattern (no library)

If headroom_ai integration feels heavy, Loragent already has a working token-saving pattern applied as surgical patches across 4 tool layers. **No library, no latency, no SDK compatibility concerns.**
...
Use the Loragent-native cap pattern when:
...
- Source code files — both approaches protect code (router:protected:recent_code for headroom, model would re-read with offset for Loragent-native)
...
### 40/60 vs 50/50 split — why Loragent uses 40/60

Loragent tool outputs are usually "context header + log dump" shaped. The model's next-action decision lives in the last error / result, which is in the tail. 50/50 would risk cutting the tail. 40/60 is the safe default — header gets less but the model only needs schema there, not body.
...
- Acceptable (Loragent already does 1-3s API roundtrips)
- If latency-sensitive, gate on message length

**Loragent-native 4-layer cap**: 0ms added latency. The truncation is a string operation (`s[:head] + marker + s[-tail:]`), no parsing, no library call.

## Info preservation verification (Loragent-specific)
...
**For Loragent-native cap**: the marker is explicit, so verification is just checking `_truncated: true` in the JSON response. No library-specific tokens to count.

## Verification after integration (headroom_ai)

1. Measure `tokens_saved` over a 10-turn Loragent session (target: 30-60% aggregate)
...
## Verification after Loragent-native cap patch
...
4. `tail -50 ~/.loragent/logs/gateway.log` — confirm "80 tools loaded" and no import errors
...
- `[[P4-cortex/portfolio/loragent]]` — Loragent architecture
...
- `[[P4-cortex/knowledge/token-compression-headroom-20260602]]` — Loragent-native 4-layer cap result (sibling doc)
- `[[P0-brainstem/brain/Loragent-brain/P0-brainstem/禁/禁task_qa_gate.neuron]]` — QA gate (production integration 필수)
- `[[agent/context_compressor]]` — Loragent's existing conversation-level summarizer (complementary, not redundant)
