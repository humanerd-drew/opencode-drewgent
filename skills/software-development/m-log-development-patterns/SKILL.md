...
2. **NAS에서 sync할 때는 반드시 diff 확인**:
   ```bash
   diff ~/Library/CloudStorage/SynologyDrive-Log-Project/m-log/frontend/app/js/views/<file> \
        $HOME/m-log/frontend/app/js/views/<file>
   ```
...
6. **문제 발생 시 대처 순서:**
   ...
   2. 아니라면 NAS sync가 원인일 가능성 → NAS 버전과 local 버전 비교
      ```bash
      diff ~/Library/CloudStorage/SynologyDrive-Log-Project/m-log/.../<file> \
           $HOME/m-log/.../<file>
      ```
