...
description: Clean up {{AGENT_NAME}}'s persistent memory file (MEMORY.md) when it nears the 8K char cap. Identify resolved/one-time entries, preserve operational facts, verify after.
tags: []
created: 2026-05-20
updated: 2026-06-10
category: brain
...
- Both are in `~/.{{AGENT_NAME_LOWER}}/P2-hippocampus/memories/`. Both are git-versioned wikilinked.
...
- **Pillar 3 (Graph traversal)** ✓ via `~/.hermes/scripts/{{AGENT_NAME_LOWER}}_graph_lookup.sh` (wikilink traversal across P2/P0/P4/P6) — `{{AGENT_NAME_LOWER}}_graph_lookup.sh <topic>` returns direct hits + incoming/outgoing wikilinks
- **Pillar 4 (Gap analysis)** ✓ via `~/.hermes/scripts/{{AGENT_NAME_LOWER}}_graph_gap_analysis.sh` (dangling wikilinks + orphan vault files). Use `--dangling-only` or `--missing-links` to filter.
...
2. Call `write_file` 직접 on `~/.{{AGENT_NAME_LOWER}}/P2-hippocampus/memories/MEMORY.md`
...
~/.{{AGENT_NAME_LOWER}}/memories/MEMORY.md.bak.<timestamp>. Resolve the drift first —
...
- **Verified 2026-06-10 21:00**: after memory split (MEMORY.md 5KB + MEMORY_wiki.md 9.5KB), `memory(action="add")` *still* rejected with drift error. Even §-delimited compact content in MEMORY.md was rejected because the *split itself* (two-file state) triggered drift detection. **Conclusion**: the `memory` tool's tracking is fundamentally broken for the {{AGENT_NAME}} two-file pattern. **Bypass with `write_file` permanently.** Document the workaround in incident doc section 6.7.
...
- `~/.{{AGENT_NAME_LOWER}}/P2-hippocampus/memories/SCHEMA.md` — memory schema
