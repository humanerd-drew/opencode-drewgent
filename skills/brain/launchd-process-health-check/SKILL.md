...
The opposite case is more dangerous and is the one that actually broke Loragent infra on 2026-06-10 (cron / n8n / quartz-fswatch all dead, never restarted, undetected for 6+ days):
...
This is the **literal value** "do not restart" — and it can appear in plists that were written to start once and stay gone, or copy-pasted from a "run-once" template. Verified on `com.loragent.quartz-fswatch.plist` 2026-06-10: fswatch died (likely on macOS fswatch binary upgrade or unrelated crash) and stayed dead because the plist explicitly told launchd "if it exits, leave it dead." **This is the worst-case launchd config for a daemon.**
...
If `<key>Label</key>` inside the plist differs from the plist filename, all `launchctl bootout/kickstart/load` commands that use the filename will **silently fail** (e.g. `launchctl bootout ai.loragent.gateway` → "No such process" because the registered label is `ai.custom-agent.gateway`).

Verified on 2026-06-10: `~/Library/LaunchAgents/ai.loragent.gateway.plist` had `Label = ai.custom-agent.gateway`. Only commands using the registered label worked. Watchdog scripts that hardcode the filename (e.g. `ai.loragent.gateway`) silently miss the service.
...
```
Then rename the label to match the filename (or vice versa). Convention: **Label = filename minus `.plist`**. Loragent labels that must match filenames: `ai.loragent.{cron-runner,gateway,kanban-dashboard,n8n}` + `com.loragent.quartz-{fswatch,deploy}`.

### Sub-pattern 6 (verified 2026-06-10): jobs.json patch has zero effect on a dead scheduler

If the gateway/cron-runner process is dead, editing `~/.loragent/cron/jobs.json` to set `next_run_at = now-5s` does **NOT** trigger a spawn. The in-memory state was loaded at process start; file edits do not propagate. Verified 2026-06-10: a Pattern A recovery patch from 5/30/6/1 incident was correct in code, but the gateway died 4 days later and the patch could not help.
...
1. Verify scheduler is alive (gateway `ps aux | grep loragent_cli.main.*gateway`).
...
grep -cE '=== 2026-' ~/.loragent/logs/cron-runner/$(date +%Y-%m-%d).log
# If the count is consistently 2x the expected, this bug is firing.
```

**Workaround**: make dispatcher scripts idempotent (the cron_runner.py at `~/.loragent/scripts/cron_runner.py` already is — uses sqlite claim logic). **Fix**: TBD; tracked in incident doc section 6.7.
...
Loragent historically had **two parallel paths** running the same dispatcher logic:
- (a) `ai.loragent.cron-runner.plist` → `scripts/cron_runner.py` (launchd StartInterval=60)
- (b) gateway internal scheduler → `loragent-cron-runner-001` jobs.json entry (script=scripts/cron_runner.py, interval=1min)
...
grep -E '=== 2026-' ~/.loragent/logs/cron-runner/$(date +%Y-%m-%d).log \
...
1. Add a single jobs.json entry with `script: ~/.loragent/scripts/cron_runner.py` and `schedule: interval=1min`
2. Restart gateway: `launchctl kickstart -k gui/$(id -u)/ai.loragent.gateway`
3. Wait 60s; verify the entry fired in `cron-runner/<date>.log` (look for new `=== ISO ===` line)
4. `launchctl bootout gui/$(id -u)/ai.loragent.cron-runner` to disable the launchd plist
5. Keep the plist on disk for rollback: `mv ~/Library/LaunchAgents/ai.loragent.cron-runner.plist ~/Library/LaunchAgents/disabled/`
...
**Why this is dangerous**: Layer 3.5 of `loragent_harmony_check.sh` correctly detects ("⚠ jobs.json modified 6270s after last dispatcher tick"), but the Layer 1/2 launchd check shows the gateway is *alive* — so the alert surfaces a real condition without obvious cause. Manual intervention (`launchctl kickstart -k`) revives it.
...
if tick_count % WIKI_MAINTENANCE_EVERY == 0:
    try:
        ...
        wiki_path = get_loragent_home() / 'memories'
        if wiki_path.exists():
...
**Related sub-pattern**: 2 gateway processes running simultaneously (PID 29156 *foreground* + PID 36947 *background*) can each have their own cron ticker. If you see cron-runner.log showing 2 fires per minute, check `ps -A | grep -c loragent_cli.main` — if > 1, you have duplicate gateways. Kill the foreground one (the one with a TTY in `ps -o tty`).

**Why the gateway cron loop is structured this way** (Loragent-specific): housekeeping is bundled into the cron tick thread to avoid running a separate scheduler. If you have a similar architecture, this Sub-pattern 10 applies. If your housekeeping is in a separate process (e.g. a cron job), it can't kill the main scheduler, so the impact is limited to "housekeeping runs less often."

**Related sub-pattern (NEW, discovered 6/10 23:53)**: "gateway alive, cron fires stop" can ALSO be caused by an LLM-based cron job blocking the sequential `tick()` loop. The process is fully alive, doing work (running an LLM agent), but script-based jobs stuck behind it can't execute. **Diagnosis**: check gateway log for `Running job <ID>` — if the latest Running job has no `script:` field in jobs.json and the previous Running job of `loragent-cron-runner-001` is 5+ min old → `cron-jobs-stalled` Pattern G.
...
1. **Watchdog cron** (`no_agent: true`) — every 5 minutes. **CRITICAL CONSTRAINT**: with Hermes `cronjob` tool, `no_agent=True` requires the script to live under `~/.hermes/scripts/` and be referenced by bare filename (the tool rejects absolute paths with "Place scripts in ~/.hermes/scripts/ and use just the filename"). Symlink the actual implementation to that path.
   ...
   The verified script (loragent_launchd_watchdog.sh) is in `references/2026-06-10-incident-fix-recipe.md` section 0. Copy verbatim, symlink under `~/.hermes/scripts/`, then register with `cronjob(action="create", no_agent=True, schedule="every 5m", script="loragent_launchd_watchdog.sh")`.
...
4. **Make `hermes status` (or `loragent doctor`) include a launchd health row** — table of (label, PID, last-log-mtime, ok/stale) — so a checkup surfaces dead services in one screen.
...
6. **Harmony check cron** (`no_agent: true`) — daily 09:00 KST. Compares 4 layers: launchd view, ps aux, jobs.json next_run_at, memory claims. **Layer 3.5** of `loragent_harmony_check.sh` is the *stalled detection* that catches Sub-pattern 9 — when the gateway cron ticker stops mid-session. Threshold: `last dispatcher tick > 60s ago = alert`. Verified 6/10 19:38 to detect the gateway stall that started 1.7h earlier. Recipe: **`references/2026-06-10-harmony-check-recipe.md`**.

## Customize Layer for Loragent + Hermes (verified 2026-06-10 17:30)

When the user wants hermes-cli to work *for Loragent's environment* (not
...
`~/.loragent/customize/`. **Do not edit `~/.hermes/hermes-agent/` files
...
1. Gateway plist `<key>PYTHONPATH</key><string>~/.loragent/customize</string>` in `EnvironmentVariables`
2. `~/.zshrc` exports `PYTHONPATH="$HOME/.loragent/customize:${PYTHONPATH:-}"`
...
87299  -15  ai.loragent.kanban-dashboard
```
...
- **plist**: `.venv/bin/python` (loragent-agent venv)
...
When writing shell scripts that run on macOS (Loragent's host), the default `bash` is **3.2** (legacy), which has significant differences from bash 4+:

- **No associative arrays**: `declare -A name=([key]=value)` is a syntax error. Use parallel indexed arrays: `arr=("v1" "v2")` and access by position. This is why `loragent_harmony_check.sh` uses `PROC_PATTERNS=(...)` and iterates by index, not by key.
- **`set -u` is fragile**: with `set -u`, any reference to an unset variable exits with error. The dotted label keys (`"ai.loragent.gateway"`) trip arithmetic-context parsing in some bash 3.2 versions. Use `set -o pipefail` (or nothing) instead. Verify with `bash -n <script>` before running.
...
- `loragent-runtime-checkup` — Phase 4b (cron-runner wrapper registration)
