...
**Path**: `~/.hermes/scripts/{{AGENT_NAME_LOWER}}_harmony_check.sh` (Hermes cron requirement)

**Symlink to**: `~/.{{AGENT_NAME_LOWER}}/P4-cortex/scripts/{{AGENT_NAME_LOWER}}_harmony_check.sh` (canonical source)
...
| 4. memory claims | What *prior agent* believed | `~/.{{AGENT_NAME_LOWER}}/P2-hippocampus/memories/MEMORY.md` |

**Output** (verified, 6/10 17:14 KST run):
```
🔍 {{AGENT_NAME}} harmony check @ 2026-06-10 17:14:50 KST

## Layer 1: launchd view (PID table)
  ~ ai.{{AGENT_NAME_LOWER}}.cron-runner: PID=- exit=0 (graceful stop, ok)
  ✓ ai.{{AGENT_NAME_LOWER}}.gateway: PID=82811
  ✓ ai.{{AGENT_NAME_LOWER}}.kanban-dashboard: PID=1543
  ✓ ai.{{AGENT_NAME_LOWER}}.n8n: PID=81753
  ⚠ com.{{AGENT_NAME_LOWER}}.quartz-fswatch: PID=- exit=254
  ⚠ com.{{AGENT_NAME_LOWER}}.quartz-deploy: PID=- exit=1

## Layer 2: ps aux (ground truth)
  ❌ ai.{{AGENT_NAME_LOWER}}.cron-runner: no process found via ps (pattern: cron_runner.py)
  ✓ ai.{{AGENT_NAME_LOWER}}.gateway: ps matches launchd (PID=82811)
  ...
...
## Layer 4: memory claims vs filesystem
  ...
  ✓ gateway plist: Label = ai.{{AGENT_NAME_LOWER}}.gateway (matches filename)
...
# Cross-layer state diff for {{AGENT_NAME}} infrastructure.
...
```
cronjob(action="create", name="{{AGENT_NAME}} harmony check (cross-layer diff)",
        no_agent=True, schedule="0 9 * * *",
        script="{{AGENT_NAME_LOWER}}_harmony_check.sh")
```
...
|---|---|---|---|
| `ai.{{AGENT_NAME_LOWER}}.cron-runner` | Layer 2 (ps) | `StartInterval=60` means process is idle between ticks; ps grep misses it | Layer 1 exit=0 + cron output mtime < 5min |
| `com.{{AGENT_NAME_LOWER}}.quartz-deploy` | Layer 1 (launchd) | `spawn scheduled` — runs on fswatch trigger only, not continuously | Layer 2 shows no `quartz-deploy.sh` process — that's correct |
| `com.{{AGENT_NAME_LOWER}}.quartz-fswatch` | Layer 2 (ps) | Wrapper script exits after spawning `fswatch` binary; `fswatch` is the long-running process but doesn't match the wrapper pattern | `pgrep fswatch` should return a PID; if not, that's a real outage |
...
4. **Bash arithmetic on dotted variable names** — `drift_count=$((drift_count+1))` works, but `drift.ai.{{AGENT_NAME_LOWER}}=$((...))` would trip. Don't use dots in arithmetic-context variable names.
...
- These are *not* launchd-level problems. Use `{{AGENT_NAME_LOWER}}-runtime-checkup` skill for those.
