...
**Path**: `~/.hermes/scripts/loragent_launchd_watchdog.sh` (symlink to `~/.loragent/P4-cortex/scripts/loragent_launchd_watchdog.sh`)
...
message="🚨 **Loragent launchd watchdog** @ $timestamp
...
```
cronjob(action="create", name="Loragent launchd watchdog",
        no_agent=True, schedule="every 5m",
        script="loragent_launchd_watchdog.sh")
```
...
Standard Loragent plist inventory (verify these exist + the Label key matches the filename):

| Filename | Label | Notes |
|---|---|---|
| `ai.loragent.cron-runner.plist` | `ai.loragent.cron-runner` | dispatcher loop, StartInterval=60 |
| `ai.loragent.gateway.plist` | `ai.loragent.gateway` | **label MUST match filename** (see Sub-pattern 4) |
| `ai.loragent.kanban-dashboard.plist` | `ai.loragent.kanban-dashboard` | flask on port 5555 |
| `ai.loragent.n8n.plist` | `ai.loragent.n8n` | may be missing — n8n case study in Sub-pattern 1 |
| `com.loragent.quartz-fswatch.plist` | `com.loragent.quartz-fswatch` | vault → wrangler pipeline |
| `com.loragent.quartz-deploy.plist` | `com.loragent.quartz-deploy` | explicit deploy trigger |

**Label mismatch (Sub-pattern 4)** is silent: `launchctl bootout ai.loragent.gateway` returns "No such process" because the registered label is `ai.custom-agent.gateway` (the value inside the plist). Always cross-check filename vs. `<Label>` before restart commands.
...
```bash
UID_NUM=$(id -u)
SERVICE="ai.loragent.gateway"  # the LABEL, not the filename
...
launchctl bootstrap gui/$UID_NUM ~/Library/LaunchAgents/ai.loragent.gateway.plist
...
**Source**: `~/.loragent/humanerd-site/content/`
**Watch list** (in `quartz-fswatch.sh`):
- `~/.loragent/memories/insights`
- `~/.loragent/P4-cortex/growth`
- `~/.loragent/P4-cortex/knowledge`
- `~/.loragent/humanerd-site/content`
...
- [ ] `launchctl list | grep loragent` → count of running (PID present) vs. stopped (PID=-)
...
- [ ] `~/.loragent/cron/jobs.json`: all `enabled` jobs have future-dated `next_run_at`
- [ ] `~/.loragent/P6-prefrontal/logs/cron-runner.log` mtime < 5 min (means scheduler is ticking)
- [ ] `~/.loragent/logs/gateway.log` last entry is recent + "Cron ticker started" / equivalent
...
- [ ] Incident doc written to `~/.loragent/P6-prefrontal/incidents/<descriptive-name>-<YYYYMMDD>.md` with: symptoms, root causes (multi-factor), resolution steps, verification, lessons
