...
-	0	ai.loragent.cron-runner
-	1	com.loragent.quartz-deploy
1543	0	ai.loragent.kanban-dashboard
-	0	com.loragent.quartz-fswatch
-	64	com.loragent.nas-mount
```

Five loragent services; only `kanban-dashboard` (PID 1543) had a real PID. Four had `-` (dead). The fact that no service had crashed and recovered — no pattern of "died, restarted" — tells us KeepAlive was not actually keeping them up.

## Per-service findings

| Service | launchctl state | Last log mtime | Symptom |
|---|---|---|---|
| `ai.loragent.cron-runner` | PID=- exit 0 | cron-runner.log absent (no per-tick logging) | All jobs.json last_run_at stuck at 2026-05-19 |
| `ai.loragent.n8n` | PID=- (no row even) | n8n.log mtime 2026-06-04 16:41, ends with "Received SIGTERM. Shutting down..." | Port 5678 not listening |
| `com.loragent.quartz-fswatch` | active count = 0, "not running" | unknown | Source path `~/humanerd` missing entirely |
| `com.loragent.quartz-deploy` | "spawn scheduled" (not yet attempted) | unknown | Same source-path issue; deploy never got to its 5s debounce |
| `ai.loragent.kanban-dashboard` | running PID 1543 | log mtime 6/10 (alive) | Port 5555 returns HTTP 000 — process alive but socket not bound |

## jobs.json state (sample, from `~/.loragent/cron/jobs.json`)
...
1. **Add `KeepAlive: { SuccessfulExit: false, ThrottleInterval: 10 }` to every plist** that has bare `<true/>` or no `KeepAlive` at all. Current candidates:
   - `~/Library/LaunchAgents/ai.loragent.cron-runner.plist` (no KeepAlive per memory)
   - `~/Library/LaunchAgents/ai.loragent.n8n.plist` (had `<true/>` per memory — bare form, vulnerable to SIGTERM→exit0 trap)
   - `~/Library/LaunchAgents/com.loragent.quartz-fswatch.plist`
   - `~/Library/LaunchAgents/com.loragent.quartz-deploy.plist`
   - `~/Library/LaunchAgents/ai.loragent.kanban-dashboard.plist`
2. **Register the watchdog cron** in the main SKILL.md — no_agent=True, every 5 min, Discord webhook
3. **Resolve Quartz source path** — `~/humanerd` is gone; need to pick one of `~/Sites/quartz` vs `~/.loragent/humanerd-site` as canonical, symlink the other, then fix fswatch script to use the symlink
