# {{AGENT_NAME}} Vault Graph — 2026-06-10 Implementation Recipe

## What this is

{{AGENT_NAME}}'s memory + skill + incident vault, with a lightweight GBrain-style
...
- GBrain's 4-pillar model ([garrytan/gbrain](https://github.com/garrytan/gbrain))
  fits {{AGENT_NAME}}'s vault shape almost 1:1, with one big difference: GBrain
  is a separate service, while we adopt the pattern **inside the vault
  itself** (no extra service to install).

## The 4 pillars ({{AGENT_NAME}}-flavored)

| Pillar | GBrain tool | {{AGENT_NAME}} equivalent | Status |
|---|---|---|---|
| 1. Repo | git-versioned knowledge | `~/.{{AGENT_NAME_LOWER}}/P2-hippocampus/memories/MEMORY.md` + skills + incidents | ✓ had this |
...
| 3. Graph traversal | typed-edge walk | `~/.hermes/scripts/{{AGENT_NAME_LOWER}}_graph_lookup.sh` (wikilink walker) | ✓ implemented 6/10 |
| 4. Gap analysis | auto-detect missing edges | `~/.hermes/scripts/{{AGENT_NAME_LOWER}}_graph_gap_analysis.sh` (dangling + missing) | ✓ implemented 6/10 |
...
├── P0-brainstem/brain/{{AGENT_NAME}}-brain/P0-brainstem/禁*.neuron  # 13禁 rules
└── skills/                                       # 48 skills

~/.hermes/scripts/
├── {{AGENT_NAME_LOWER}}_graph_lookup.sh          # Pillar 3
├── {{AGENT_NAME_LOWER}}_graph_gap_analysis.sh    # Pillar 4
├── {{AGENT_NAME_LOWER}}_harmony_check.sh         # Layer 3.5 mtime drift detector (catches Sub-pattern 9)
├── {{AGENT_NAME_LOWER}}_launchd_watchdog.sh      # 5-min launchd health poll
├── {{AGENT_NAME_LOWER}}_log_rotate.sh            # daily 04:00 log rotation
...
bash ~/.hermes/scripts/{{AGENT_NAME_LOWER}}_graph_gap_analysis.sh --missing-links
...
- **No `set -u`** — dotted label arithmetic quirk. Variables with dots (e.g. `ai.{{AGENT_NAME_LOWER}}.gateway`) inside `${...}` get parsed as math.
...
5. **Single-machine, single-user** — GBrain's distributed / multi-user features are overkill for {{AGENT_NAME}}
...
- **Multi-vault**: if {{AGENT_NAME}} grows to multiple repos (e.g. separate skills, separate incidents dirs), the graph_lookup needs a `-r` flag. (TBD.)

## Reference: state at end of 6/10 sweep

```bash
$ bash ~/.hermes/scripts/{{AGENT_NAME_LOWER}}_graph_gap_analysis.sh
🔍 **{{AGENT_NAME}} graph gap analysis** @ 2026-06-10 19:33:37 KST
Mode: all
...
---
Run graph lookup: bash ~/.hermes/scripts/{{AGENT_NAME_LOWER}}_graph_lookup.sh <topic>
```
