---
name: {{AGENT_NAME_LOWER}}-runtime-checkup
title: {{AGENT_NAME}} Runtime Checkup
description: {{AGENT_NAME}} 코어 시스템 (AIAgent, signal, kanban, dispatcher, worker) 의 기본기를 점검하는 6-Phase 절차. docs와 reality mismatch 발견 시 methodology.
type: skill
space: growth
...
```bash
python3 ~/.{{AGENT_NAME_LOWER}}/scripts/dispatch_once_default.py
...
# 0 pass면: source/{{AGENT_NAME_LOWER}}-agent/agent/context_compressor.py 의 _estimate_tokens() 확인
...
2. `cd ~/.{{AGENT_NAME_LOWER}}/source/{{AGENT_NAME_LOWER}}-agent && git diff HEAD -- gateway/run.py | grep "^-.*def "` — 클래스에서 제거된 메서드 목록
...
- `jobs.json`에 cron job entry가 없는데 script는 `~/.{{AGENT_NAME_LOWER}}/scripts/`에 존재
- `last_run_at`이 며칠 전에서 멈춤
- `launchctl list | grep {{AGENT_NAME_LOWER}}` → ai.{{AGENT_NAME_LOWER}}.gateway 또는 ai.{{AGENT_NAME_LOWER}}.cron-runner plist 없음
...
뒤 script job ({{AGENT_NAME_LOWER}}-cron-runner-001)이 fire 못 함. 최대 26분 stall 검증됨.

**진단**: gateway log에서 "Running job" 순서 확인:
```bash
grep "Running job" ~/.{{AGENT_NAME_LOWER}}/P6-prefrontal/logs/gateway.log | tail -20
# d1ef68ced116만 있고 {{AGENT_NAME_LOWER}}-cron-runner-001 없으면 = tick loop block
```
...
3. **Cron-runner fire frequency 모니터링**: harmony check Layer 3.5b가 5분마다
   ...
   `{{AGENT_NAME_LOWER}}_cron_watchdog.sh`가 자동 kickstart.
...
python3 -c "
import sqlite3
conn = sqlite3.connect('~/.{{AGENT_NAME_LOWER}}/P2-hippocampus/kanban/state/{{AGENT_NAME_LOWER}}_tasks.db')
...
Obsidian vault `.{{AGENT_NAME_LOWER}}/`의 wikilink graph 무결성 점검. **vault = agent의 장기 기억**. 링크가 끊어지면 agent가 관련 문서를 찾지 못함.

**진단**: `~/.hermes/scripts/{{AGENT_NAME_LOWER}}_graph_gap_analysis.sh` 실행:
```bash
...
- **`.neuron` → `.md` 미인식**: Obsidian이 `.neuron` 파일을 markdown으로 인식 못 함
  → `~/.{{AGENT_NAME_LOWER}}/.obsidian/app.json`에 `"extensionOverrides": [".neuron"]` 추가 (파일 rename 불필요)
...
- **Monitor/ brain signal orphan**: `~/.{{AGENT_NAME_LOWER}}/monitor/brain_signals_*.md` 파일들 (수천 개)
...
**검증**:
```bash
bash ~/.hermes/scripts/{{AGENT_NAME_LOWER}}_graph_gap_analysis.sh --dangling-only
...
grep -E "os\\.environ\\[" ~/.{{AGENT_NAME_LOWER}}/scripts/run_kanban_worker.py
# 3. Python tool 코드에서 그 env를 참조하는지
grep -rln "KANBAN_WORKER_MODE" ~/.{{AGENT_NAME_LOWER}}/source/{{AGENT_NAME_LOWER}}-agent --include="*.py"
```

## CRITICAL PITFALL — Shell Env Var Pattern

{{AGENT_NAME}}는 자주 **shell env로 subprocess에 mode를 전달**한다. Python 코드에 그 env var가 안 보일 수 있다. 이게 **"미구현"이 아니라 "정상 구현"** 인 경우:
...
1. `~/.{{AGENT_NAME_LOWER}}/scripts/*.py` 확인 (dispatcher / worker script는 `source/{{AGENT_NAME_LOWER}}-agent` 밖)
...
- "log rotation" (NEW follow-up surfaced during F1-F3) → step 1 NO (no existing tool), step 3 real reliability → step 1: write the tool. Consolidated: `{{AGENT_NAME_LOWER}}_log_rotate.sh` + cron registration. Done.
...
**Right pattern**:
> "대전제: .{{AGENT_NAME_LOWER}} 구조 유지 + hermes는 *내* 맥락에 맞춤. 적용:
> - D1 hermes → customize layer (PYTHONPATH hijack), upstream PR 안 함
> - D3 memory → .{{AGENT_NAME_LOWER}} canonical, codex 무시 (policy)
...
find ~/.{{AGENT_NAME_LOWER}}/customize -name '__pycache__' -type d  # check for python cache
find /tmp -name '{{AGENT_NAME_LOWER}}_*' 2>/dev/null  # check for temp residue
...
- "Removed: `{{AGENT_NAME_LOWER}}-hermes` wrapper (redundant after `~/.local/bin/hermes` patch)"
...
- `~/.{{AGENT_NAME_LOWER}}/scripts/dispatch_once_*.py` (3개 board) — stdout=PIPE → DEVNULL 변경
- `~/.{{AGENT_NAME_LOWER}}/scripts/run_kanban_worker.py` — watchdog 추가
- `P4-cortex/growth/KANBAN-REVIEW-20260520.md` — docs 표현 정정 ("Python mode 분기" → "shell env flag")

## Related

- [[@action/gateway/{{AGENT_NAME_LOWER}}-architecture-dataflow]] — 전체 데이터 흐름
- [[@memory/kanban/KANBAN_INDEX]] — kanban 시스템 개요
- [[@identity/brain/{{AGENT_NAME}}-brain/P0-brainstem/禁/禁filesystem_truth.neuron]] — "files are truth" 원칙
...
- `~/.{{AGENT_NAME_LOWER}}/P0-brainstem/brain/{{AGENT_NAME}}-brain/P0-brainstem/禁incident_aware.neuron` (P0 policy) — auto-loads this skill + the 6/10 incident doc when watchdog fires or user requests "에이전트 상태 점검". Trip-wires the cross-layer diff path so the agent doesn't re-discover the 6 root causes from scratch.
