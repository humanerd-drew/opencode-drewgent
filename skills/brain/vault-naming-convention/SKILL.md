...
- `source/` — {{AGENT_NAME_LOWER}}-agent upstream code
...
`~/.{{AGENT_NAME_LOWER}}/.obsidian/app.json` controls what Obsidian indexes:
...
- **Source code** ({{AGENT_NAME_LOWER}}-agent upstream) — not vault content, 725MB of noise
...
grep -rl '^title: Skill$' ~/.{{AGENT_NAME_LOWER}}/skills --include='SKILL.md' \
...
grep -roh '\[\[[^]]*\]\]' ~/.{{AGENT_NAME_LOWER}} --include='*.md' \
  | sed 's/\[\[//;s/\]\]//' | sort | uniq -c | sort -rn | head -20

# Files with N=1 outbound link (daisy chain)
grep -c '\[\[[^]]*\]\]' ~/.{{AGENT_NAME_LOWER}}/P0-brainstem/**/*.md 2>/dev/null
...
`~/.{{AGENT_NAME_LOWER}}/` is shared by two systems:
...
| P-layer dirs | **{{AGENT_NAME}} vault** knowledge | Always use full-path wikilinks | `P1-limbic/persona/SOUL.md`, `P0-brainstem/brain/rules.md` |

Hermes Agent's `_ensure_default_soul_md()` in `hermes_cli/config.py` seeds `~/SOUL.md` if missing — this is correct behavior. The root `SOUL.md` is the **Hermes system prompt**, not {{AGENT_NAME}}'s identity document. {{AGENT_NAME}}'s identity lives at `P1-limbic/persona/SOUL.md`.

**Rule**: Never create a vault wikilink to a root-level file. Root = Hermes runtime. P-layers = {{AGENT_NAME}} knowledge.
...
for f in ~/.{{AGENT_NAME_LOWER}}/path/to/cluster/*/SKILL.md; do
...
grep -A2 'related_skills:' ~/.{{AGENT_NAME_LOWER}}/path/to/cluster/*/SKILL.md
...
for link in "list/of/newly/added/links"; do
  find ~/.{{AGENT_NAME_LOWER}} -path "*/${link}.md" | grep -q . && echo "OK" || echo "BROKEN"
done
```
