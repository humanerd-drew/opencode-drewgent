...
- Total files: ~9,600
- Total size: ~45MB
- Hub nodes: `{{AGENT_NAME_LOWER}}-kanban-implementation-plan` (11,576 hits), `kanban-orchestrator` (11,565), `kanban-dashboard` (11,564)
...
| SCHEMA.md | 3 | Nested ~/.{{AGENT_NAME_LOWER}}/~/.{{AGENT_NAME_LOWER}}/ moved to .trash. Source copy excluded. |
...
2. Renamed root SOUL.md → HERMES_SYSTEM_PROMPT.md (Hermes system prompt, not {{AGENT_NAME}} identity)
3. Renamed docker/SOUL.md → DOCKER_PERSONA.md
4. Moved nested ~/.{{AGENT_NAME_LOWER}}/~/.{{AGENT_NAME_LOWER}}/ to .trash/
...
| P3-sensors/gateway/{{AGENT_NAME_LOWER}}-architecture-dataflow.md | 8 | 9 | NEURONFS_RULES |
