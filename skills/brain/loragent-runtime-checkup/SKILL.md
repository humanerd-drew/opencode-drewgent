---
name: loragent-runtime-checkup
title: Loragent Runtime Checkup
description: Loragent 코어 시스템 (AIAgent, signal, kanban, dispatcher, worker) 의 기본기를 점검하는 6-Phase 절차. docs와 reality mismatch 발견 시 methodology.
type: skill
space: growth
...
# Loragent Runtime Checkup

Loragent 코어 시스템의 "기본기"를 점검할 때 사용하는 표준 절차. 핵심 철학: **"docs에서 Done이라고 한 것 ≠ 실제 구현"**. 항상 filesystem ground truth 로 verify.
...
- 새 모델 / 새 환경에서 Loragent 설치 직후 sanity check

## 6-Phase Checkup (in order)

### Workdir 주의
터미널 workdir 는 turn 사이에서 휘발됨. 모든 명령은 `cd ~/.loragent/source/loragent-agent &&` prefix 필수. 절대 workdir 에 의존하지 말 것.

### Phase 1 — Core Imports (1분)
AIAgent, signal_processor, context_compressor, brain_signals, event_bus 모두 import. **import 실패 = P0 즉시 보고**.

```bash
cd ~/.loragent/source/loragent-agent && source .venv/bin/activate
python3 -c "
...
python3 -c "
import sqlite3
conn = sqlite3.connect('P2-hippocampus/kanban/state/loragent_tasks.db')
...
```bash
python3 ~/.loragent/scripts/dispatch_once_default.py
...
# 0 pass면: source/loragent-agent/agent/context_compressor.py 의 _estimate_tokens() 확인
...
2. `cd ~/.loragent/source/loragent-agent && git diff HEAD -- gateway/run.py | grep "^-.*def "` — 클래스에서 제거된 메서드 목록
...
- `jobs.json`에 cron job entry가 없는데 script는 `~/.loragent/scripts/`에 존재
- `last_run_at`이 며칠 전에서 멈춤
- `launchctl list | grep loragent` → ai.loragent.gateway 또는 ai.loragent.cron-runner plist 없음
...
뒤 script job (loragent-cron-runner-001)이 fire 못 함. 최대 26분 stall 검증됨.

**진단**: gateway log에서 "Running job" 순서 확인:
```bash
grep "Running job" ~/.loragent/P6-prefrontal/logs/gateway.log | tail -20
# d1ef68ced116만 있고 loragent-cron-runner-001 없으면 = tick loop block
```
...
3. **Cron-runner fire frequency 모니터링**: harmony check Layer 3.5b가 5분마다
   ...
   `loragent_cron_watchdog.sh`가 자동 kickstart.
...
python3 -c "
import sqlite3
conn = sqlite3.connect('~/.loragent/P2-hippocampus/kanban/state/loragent_tasks.db')
...
Obsidian vault `.loragent/`의 wikilink graph 무결성 점검. **vault = agent의 장기 기억**. 링크가 끊어지면 agent가 관련 문서를 찾지 못함.

**진단**: `~/.hermes/scripts/loragent_graph_gap_analysis.sh` 실행:
```bash
...
- **`.neuron` → `.md` 미인식**: Obsidian이 `.neuron` 파일을 markdown으로 인식 못 함
  → `~/.loragent/.obsidian/app.json`에 `"extensionOverrides": [".neuron"]` 추가 (파일 rename 불필요)
...
- **Monitor/ brain signal orphan**: `~/.loragent/monitor/brain_signals_*.md` 파일들 (수천 개)
...
grep -A 10 "subprocess.Popen" ~/.loragent/scripts/dispatch_once_default.py
# 2. worker가 그 env를 어떻게 read하는지
grep -E "os\\.environ\\[" ~/.loragent/scripts/run_kanban_worker.py
# 3. Python tool 코드에서 그 env를 참조하는지
grep -rln "KANBAN_WORKER_MODE" ~/.loragent/source/loragent-agent --include="*.py"
```

## CRITICAL PITFALL — Shell Env Var Pattern

Loragent는 자주 **shell env로 subprocess에 mode를 전달**한다. Python 코드에 그 env var가 안 보일 수 있다. 이게 **"미구현"이 아니라 "정상 구현"** 인 경우:
...
1. `~/.loragent/scripts/*.py` 확인 (dispatcher / worker script는 `source/loragent-agent` 밖)
...
- "log rotation" (NEW follow-up surfaced during F1-F3) → step 1 NO (no existing tool), step 3 real reliability → step 1: write the tool. Consolidated: `loragent_log_rotate.sh` + cron registration. Done.
...
**Right pattern**:
> "대전제: .loragent 구조 유지 + hermes는 *내* 맥락에 맞춤. 적용:
> - D1 hermes → customize layer (PYTHONPATH hijack), upstream PR 안 함
> - D3 memory → .loragent canonical, codex 무시 (policy)
...
find ~/.loragent/customize -name '__pycache__' -type d  # check for python cache
find /tmp -name 'loragent_*' 2>/dev/null  # check for temp residue
...
- "Removed: `loragent-hermes` wrapper (redundant after `~/.local/bin/hermes` patch)"
...
- `~/.loragent/scripts/dispatch_once_*.py` (3개 board) — stdout=PIPE → DEVNULL 변경
- `~/.loragent/scripts/run_kanban_worker.py` — watchdog 추가
- `P4-cortex/growth/KANBAN-REVIEW-20260520.md` — docs 표현 정정 ("Python mode 분기" → "shell env flag")

## Related

- [[P3-sensors/gateway/loragent-architecture-dataflow]] — 전체 데이터 흐름
- [[P2-hippocampus/kanban/KANBAN_INDEX]] — kanban 시스템 개요
- [[P0-brainstem/brain/Loragent-brain/P0-brainstem/禁/禁filesystem_truth.neuron]] — "files are truth" 원칙
...
- `~/.loragent/P0-brainstem/brain/Loragent-brain/P0-brainstem/禁incident_aware.neuron` (P0 policy) — auto-loads this skill + the 6/10 incident doc when watchdog fires or user requests "에이전트 상태 점검". Trip-wires the cross-layer diff path so the agent doesn't re-discover the 6 root causes from scratch.
