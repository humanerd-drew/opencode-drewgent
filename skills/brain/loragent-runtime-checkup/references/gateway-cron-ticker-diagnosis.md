...
# Only d1ef68ced116 entries, NO loragent-cron-runner-001 = block
```
...
And `grep "Running job.*loragent-cron-runner-001" gateway.log` → fires
...
**Fix**: `rm -f ~/.loragent/cron/.tick.lock`
...
Full script at `~/.hermes/scripts/loragent_cron_watchdog.sh`. Covers:
