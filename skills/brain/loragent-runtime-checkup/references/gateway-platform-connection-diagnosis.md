...
grep -E "Connecting|connected|Connecting to" ~/.loragent/logs/gateway.log | tail -20
```
...
grep DISCORD_BOT_TOKEN ~/.loragent/.env | head -1
# Should show the bot token (first 20 chars visible)

# 2. Check if connect logic exists but isn't wired
grep -rn "connect_all\|connect_adapter" ~/.loragent/source/loragent-agent/gateway/adapters.py | head -5
# Should show the method definition

# 3. Verify no call site exists
grep -rn "connect_all" ~/.loragent/source/loragent-agent/gateway/run.py
# If empty = orphaned method, needs wiring

# 4. Check GatewayRunner.start()
grep -A 5 "async def start" ~/.loragent/source/loragent-agent/gateway/run.py
...
kill -TERM $(pgrep -f "loragent_cli.main gateway")
...
```bash
sleep 5 && grep -E "Connecting|connected" ~/.loragent/logs/gateway.log | tail -10
```
