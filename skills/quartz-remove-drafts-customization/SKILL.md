...
for src in $(grep -l "^status: draft$" ~/.loragent/memories/insights/*.md); do
...
inline patch (`quartz.config.ts` 안에 const CustomRemoveDrafts 정의)보다 별도 파일이 깨끗하고 재사용 가능. **Loragent는 이 방식을 적용함 (quartz/plugins/filters/humanerd-draft.ts).**
...
# live site
for url in $(grep -l "^status: draft$" ~/.loragent/memories/insights/*.md | \
...
`includeStatus: ["polished", "published"]`, `excludeStatus: ["draft", "in_review", "archived"]` 라는 list만으로는 부족. Loragent humanerd 5-state machine:
...
`loragent-content-status-enforcement` skill의 recovery procedure 참조.
...
- [[P3-sensors/gateway/loragent-architecture-dataflow]] — Quartz build → wrangler deploy 흐름
