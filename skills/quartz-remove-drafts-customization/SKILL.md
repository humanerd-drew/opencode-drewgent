...
for src in $(grep -l "^status: draft$" ~/.{{AGENT_NAME_LOWER}}/memories/insights/*.md); do
...
**{{AGENT_NAME}} 6/1 incident에서 채택 이유:**
...
inline patch (`quartz.config.ts` 안에 const CustomRemoveDrafts 정의)보다 별도 파일이 깨끗하고 재사용 가능. **{{AGENT_NAME}}는 이 방식을 적용함 (quartz/plugins/filters/YOUR_PREFIX-draft.ts).**

**1단계 — plugin 파일 생성** (`quartz/plugins/filters/YOUR_PREFIX-draft.ts`):
```typescript
import { QuartzFilterPlugin } from "../types"

export const RemoveDraftsCustom: QuartzFilterPlugin<{}> = () => ({
  name: "RemoveDraftsCustom",
  shouldPublish(_ctx, [_tree, vfile]) {
...
```typescript
export { RemoveDraftsCustom } from "./YOUR_PREFIX-draft"
```
...
import { QuartzConfig } from "./quartz/cfg"
import * as Plugin from "./quartz/plugins"
import { RemoveDraftsCustom } from "./quartz/plugins/filters"  // ← 추가

const config: QuartzConfig = {
  // ...
  filters: [Plugin.RemoveDraftsCustom()],  // ← Plugin.RemoveDrafts() 대신
}
```
...
- 코드 가독성: `RemoveDraftsCustom()`라는 이름이 의도를 명확히 표현
...
- 단, YOUR_USERNAME-style "먼저 다 쓰고 검토 후 publish" 워크플로와 충돌할 수 있음
...
# live site
for url in $(grep -l "^status: draft$" ~/.{{AGENT_NAME_LOWER}}/memories/insights/*.md | \
...
**별도 파일 방식 (Option A, 추천)을 쓰면 이 함정이 사라짐** — `RemoveDraftsCustom`가 export에서 unique하므로 중복 import 자체가 컴파일 에러로 잡힘.
...
`includeStatus: ["polished", "published"]`, `excludeStatus: ["draft", "in_review", "archived"]` 라는 list만으로는 부족. YOUR_USERNAME 5-state machine:
...
이 전이는 `Option D`의 transformer plugin에서 자동 enforce 안 됨. cron job이나 YOUR_USERNAME manual edit에서만 발동. 자동 lint를 원하면 별도 script.

### 8. **state field는 `status` 하나 — `published: true/false` 사용 안 함**

일부 YOUR_USERNAME article은 `published: true` (boolean) 또는 `published_date: YYYY-MM-DD`를 쓰지만, canonical state field는 `status` 하나. 둘 다 쓰면 plugin이 `status`만 본다 (Option D design).
...
- 따라서 content-pipeline cron이 `status: draft`로 자동 생성 + YOUR_USERNAME이 검토 후 `status: published` 명시 변경이 **default workflow**가 됨
...
`{{AGENT_NAME_LOWER}}-content-status-enforcement` skill의 recovery procedure 참조.

## Related

- [[skills/content-pipeline/SKILL]] — content-pipeline skill (Phase 3에서 status: draft 작성)
- [[skills/YOUR_SITE]] — YOUR_SITE 운영 skill
- [[skills/YOUR_PREFIX-content-status-enforcement]] — agent self-apply 규칙 (status field 자동 enforce)
...
- [[@action/gateway/{{AGENT_NAME_LOWER}}-architecture-dataflow]] — Quartz build → wrangler deploy 흐름
