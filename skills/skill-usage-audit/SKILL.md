...
description: Loragent skills/ 디렉토리의 121개 SKILL.md를 3-criteria hard evidence 기준으로 dead/active 분류 — cron 등록, wikilink 참조, mtime 임계치
...
updated: 2026-06-03
links:
  - "[[P0-brainstem/brain/Loragent-brain/P0-brainstem/禁/禁no_linear_workflow]]"
...
`~/.loragent/skills/` 의 모든 SKILL.md를 dead/active로 분류하는 진단 스킬.
...
Loragent는 hugh-kim 박스에서 ~100개 skill을 가져왔는데, 그 중 60%는 한 번도 안 쓰는 dead 코드.
...
```bash
find ~/.loragent/skills -name SKILL.md 2>/dev/null | wc -l
find ~/.loragent/skills -name SKILL.md 2>/dev/null | xargs -I{} dirname {} | xargs -I{} basename {} | sort -u
```

기대치: 100~150개 (HUGH 박스 + Loragent 자체).

### Step 2: mtime 분류 — 3-tier 임계치 (5/14, 5/20, 6/1)

```bash
for d in ~/.loragent/skills/*/; do
  name=$(basename "$d")
...
- 5/20 ~ 5/24 = Loragent 자체 setup 시점 (kanban/neuron-fs 등 코어 1세대) → active 가능
...
```bash
cat ~/.loragent/cron/jobs.json | python3 -c "
import json, sys
...
```bash
grep -rEho "skills/[a-z0-9-]+" ~/.loragent/P2-hippocampus ~/.loragent/P4-cortex ~/.loragent/P5-ego ~/.loragent/P6-prefrontal 2>/dev/null | sort -u
```
...
test -d ~/.loragent/skills/productivity/linear && echo "POLICY VIOLATION: linear skill"

# 禁kanban_* 등 미구현
ls ~/.loragent/skills/kanban-*/SKILL.md 2>/dev/null
```
...
- mlops/* (Loragent는 API consumer, trainer 아님)
...
| Option | 작업 | risk | 효과 |
|--------|------|------|------|
| H1. 정책 위반 1개만 제거 (linear) | `rm -rf ~/.loragent/skills/productivity/linear` | 0 | 정책 복구 |
...
- **HUGH 박스 mtime 착각**: 5/20이 Loragent setup 시점이고, 그 이전 mtime = "한 번도 안 건드림". 그러나 core kanban/neuron-fs도 5/20 mtime → mtime 단독 판정 금지
...
- **trend-harvester는 skills/ 에 없음**: `~/.loragent/scripts/trend_harvester.py` 결정론적 script로 cron이 직접 호출. SKILL.md로 따로 load 안 함. SKILL.md 없다고 dead 아님
...
N_CRON=$(jq -r '.jobs[].skills[]' ~/.loragent/cron/jobs.json 2>/dev/null | sort -u | wc -l)
N_WIKI=$(grep -rEho "skills/[a-z0-9-]+" ~/.loragent/P2-hippocampus ~/.loragent/P4-cortex ~/.loragent/P5-ego 2>/dev/null | sort -u | wc -l)
echo "cron_skills=$N_CRON wikilink_skills=$N_WIKI"

# 2. dead 의심 (C1, C2 모두 0, mtime 5/14 batch)
DEAD=$(find ~/.loragent/skills -name SKILL.md -newermt "2026-05-14" ! -newermt "2026-05-15" 2>/dev/null | wc -l)
echo "mtime_5_14_batch=$DEAD"

# 3. 정책 위반 scan
test -d ~/.loragent/skills/productivity/linear && echo "VIOLATION: linear"

# 4. 빈 카테고리 scan
EMPTY=$(for d in $(ls -d ~/.loragent/skills/*/); do
...
for batch in "apple gaming leisure smart-home social-media email red-teaming data-science note-taking" \
  ...
  N=$(find ~/.loragent/skills -name SKILL.md | wc -l)
  echo "  remaining: $N"
done
```
...
- [[P0-brainstem/brain/Loragent-brain/P0-brainstem/禁/禁no_linear_workflow]] — 정책 위반 scan
- [[P0-brainstem/brain/Loragent-brain/P0-brainstem/禁/禁filesystem_truth.neuron]] — "filesystem = truth" 원칙
