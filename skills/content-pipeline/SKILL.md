...
yourdomain.com은 자동 뉴스 블로그가 아니라 **Drew가 AI, 도구, 코드, 글쓰기, 시스템 설계를 통해 어떻게 사고하고 만드는지를 보여주는 개인 작업실**이다.
...
cd ~/.drewgent/scripts           # local install for puppeteer (Step 4)
...
**Template** (1200×630, yourdomain.com dark theme):
```svg
...
```python
draft_file = f"~/.drewgent/P2-hippocampus/memories/insights/{YYYY-MM}-{slug}.md"
kanban_complete(
    task_id,
...
Phase 5 regex: `r'memories/insights/(20\d{2}-\d{2}-[^.]+\.md)'` 또는 `r'~/.drewgent/P2-hippocampus/memories/insights/(20\d{2}-\d{2}-[^.]+\.md)'`
...
|---|--------|-------|---------|------------|
| 1 | trend | {title} | {id} | ~/.drewgent/P2-hippocampus/memories/insights/{YYYY-MM}-{slug}.md |
| 2 | seo | {title} | {id} | ~/.drewgent/P2-hippocampus/memories/insights/{YYYY-MM}-{slug}.md |
...
| 2 | {title} | {id} | ~/.drewgent/P2-hippocampus/memories/insights/{filename}.md |
...
| # | Source | Topic | Task ID | Draft File |
|---|--------|-------|---------|------------|
| 1 | trend | {title} | {task_id} | ~/.drewgent/P2-hippocampus/memories/insights/{filename}.md |
| 2 | seo | {title} | {task_id} | ~/.drewgent/P2-hippocampus/memories/insights/{filename}.md |
...
Obsidian에서 `status: published`로 바꾸면 → fswatch가 vault 변경 감지 → quartz build → wrangler pages deploy → yourdomain.com 게시 (약 10초 내외)
...
Content-manager drafts live at `~/.drewgent/P2-hippocampus/memories/insights/`. The Quartz site maps this directory into its content tree via symlink:
...
```bash
readlink ~/.drewgent/humanerd-site/content/insights
# Should point to: ~/.drewgent/P2-hippocampus/memories/insights
```
...
[ ] fswatch가 자동 감지 → yourdomain.com 게시 (~10초)
[ ] yourdomain.com에서 게시 확인
```
...
Quartz at yourdomain.com has Mermaid enabled (`quartz/components/scripts/mermaid.inline.ts`). Write ````mermaid code blocks directly in blog post markdown:
...
```markdown
<!-- COVER: Dark mode dev dashboard showing architecture, amber accents, yourdomain.com style. -->
```
...
5. **yourdomain.com에서 확인** — 1~2분 내 게시

### Alias 규칙 (SEO-Friendly URL)

| Frontmatter | Site URL |
|------------|----------|
| `aliases: ['/blog/2026/agent-workflow']` | yourdomain.com/blog/2026/agent-workflow |
| `aliases: ['/projects/kanban-review']` | yourdomain.com/projects/kanban-review |
| `aliases: ['/lab/dream-system']` | yourdomain.com/lab/dream-system |
...
Drafts can be pushed to the yourdomain.com WordPress site via the WordPress MCP server.
...
Draft paths in agent profiles, cron prompts, and workflow documents must be **absolute paths** (`~/.drewgent/P2-hippocampus/memories/insights/`). Do NOT use:
...
✅ Safe: `~/.drewgent/P2-hippocampus/memories/insights/filename.md`
