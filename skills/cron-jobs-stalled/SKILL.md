...
- `~/.loragent/cron/jobs.json` enabled jobs with `last_run_at` > 1 day old
- All enabled jobs may have `next_run_at: null`
- `~/.loragent/cron/output/{job_id}/` has no recent files
- Output files exist but contain junk (empty body, trivial title)

## Step 1: Read jobs.json directly (filesystem truth)

```bash
python3 -c "import json; d=json.load(open('$HOME/.loragent/cron/jobs.json')); [print(f'{j.get(\"name\",\"?\"):30} enabled={j.get(\"enabled\")} last={str(j.get(\"last_run_at\",\"\"))[:19]} next={str(j.get(\"next_run_at\",\"\"))[:19] or \"NULL\"} status={j.get(\"last_status\",\"\")}') for j in d.get('jobs',[])]"

ls -lt ~/.loragent/cron/output/*/ 2>/dev/null | head -30
```
...
```bash
# Find junk articles
ls -lt ~/.loragent/cron/output/{job_id}/ | head -10
# Check content of recent files
for f in $(ls -t ~/.loragent/cron/output/{job_id}/2026-*.md | head -5); do
  echo "=== $f ==="
  head -20 "$f"
done
```
...
- `launchctl list | grep loragent` shows no `ai.loragent.cron-runner` plist
...
tail -5 ~/.loragent/P6-prefrontal/logs/cron-runner.log
...
find ~/.loragent/cron/output/*/ -name "*.md" -mmin -10
...
- `launchctl list | grep ai.loragent.cron-runner` PID=- → launchd may not track detached processes
...
JOBS = "~/.loragent/cron/jobs.json"
OUT = Path("~/.loragent/cron/output")
...
- `launchctl list | grep loragent` shows `ai.loragent.gateway` with `PID=-` and old exit code
- `~/.loragent/P6-prefrontal/logs/cron-runner.log` mtime is hours/days stale
...
- `launchctl list | grep ai.loragent.gateway` shows `PID=-` with old exit code (likely -15 for SIGTERM, 0 for graceful stop)
...
ps aux | grep loragent_cli.main.*gateway | grep -v grep
...
sleep 8
tail -5 ~/.loragent/logs/gateway.log
...
python3 -c "import json; d=json.load(open('~/.loragent/cron/jobs.json')); [print(f\"{j.get('name','?'):30} next={str(j.get('next_run_at',''))[:19]}\") for j in d.get('jobs',[]) if j.get('enabled')]"
...
**Layer 3.5 of `loragent_harmony_check.sh` correctly detects this**:
```
...
- Script file exists at `~/.loragent/scripts/<name>.sh` and `~/.hermes/scripts/<name>.sh`
...
**Root cause:** `_run_script_subprocess()` in `~/.loragent/source/loragent-agent/cron/scheduler.py` (line 801–851):

1. **Relative path not resolved to scripts_dir**: `os.path.expanduser(script_path)` + `os.path.isfile()` checks the **current working directory** (`~/.loragent/source/loragent-agent/`) instead of `scripts_dir` (`~/.loragent/scripts/`).
...
2. Relative paths resolved against `get_loragent_home() / "scripts/"` (not cwd)
...
**Script location double-check:** no_agent scripts need to exist at BOTH locations. Canonical origin is `~/.hermes/scripts/` but the Loragent customize layer redirects HERMES_HOME → `~/.loragent`, so the active scheduler reads from `~/.loragent/scripts/`. Copy scripts to both locations.
...
launchctl kickstart -k gui/$(id -u)/ai.loragent.gateway
sleep 5
grep "Cron ticker started" ~/.loragent/logs/gateway.log | tail -1
```

**Stale error clearing:** After restart, old `last_error` from before the patch persists in `jobs.json`. It auto-clears on the next successful tick. For long-cycle jobs (e.g. 6h), manually patch `last_status` → `"ok"` and `last_error` → `null` in `~/.loragent/cron/jobs.json`.

## Step 3: Apply recoveryscheduler reads from `~/.loragent/scripts/`. Copy scripts to both locations.
...
**The 4 pillars we adopt from [Garry Tan's GBrain](https://github.com/garrytan/gbrain)** (lightly adapted for a single-machine Loragent):

1. **Repo (vault)**: our `~/.loragent/P2-hippocampus/memories/MEMORY.md` + `~/.loragent/skills/` + incidents directory, all wikilinked. ✓ already had this.
...
3. **Graph traversal**: `~/.hermes/scripts/loragent_graph_lookup.sh` — given a topic, walks wikilinks across MEMORY.md, P0-brainstem neurons, P4-cortex growth, P6-prefrontal incidents. Returns direct hits + incoming wikilinks + outgoing wikilinks. ✓ implemented 6/10.
4. **Gap analysis**: `~/.hermes/scripts/loragent_graph_gap_analysis.sh` — detects (a) **dangling wikilinks** in MEMORY.md pointing to non-existent files, (b) **orphan vault files** not referenced from memory. ✓ implemented 6/10.
...
python3 -c "import json; d=json.load(open('$HOME/.loragent/cron/jobs.json')); [print(f'{j.get(\"name\",\"?\"):30} next={str(j.get(\"next_run_at\",\"\"))[:19] or \"NULL\"}') for j in d.get('jobs',[]) if j.get('enabled')]"

# Verify: output dir has new file from this tick
ls -lt ~/.loragent/cron/output/*/ 2>/dev/null | head -5
```
...
- "log rotation infrastructure" (NEW follow-up that emerged from F1-F3 work) → consolidated: wrote `loragent_log_rotate.sh` + cron registration. Done.
...
- `launchctl list | grep ai.loragent.cron-runner` shows PID=- with old exit code
- `~/.loragent/P6-prefrontal/logs/cron-runner.log` mtime is hours/days stale
...
Use the 3-phase QA gate pattern for any code change. Evidence goes in `~/.loragent/P2-hippocampus/qa-evidence/{task_id}/`.
...
- `loragent-runtime-checkup` — broader system checkup
