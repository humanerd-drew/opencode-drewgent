...
- `~/.{{AGENT_NAME_LOWER}}/cron/jobs.json` enabled jobs with `last_run_at` > 1 day old
- All enabled jobs may have `next_run_at: null`
- `~/.{{AGENT_NAME_LOWER}}/cron/output/{job_id}/` has no recent files
- Output files exist but contain junk (empty body, trivial title)

## Step 1: Read jobs.json directly (filesystem truth)

```bash
python3 -c "import json; d=json.load(open('$HOME/.{{AGENT_NAME_LOWER}}/cron/jobs.json')); [print(f'{j.get(\"name\",\"?\"):30} enabled={j.get(\"enabled\")} last={str(j.get(\"last_run_at\",\"\"))[:19]} next={str(j.get(\"next_run_at\",\"\"))[:19] or \"NULL\"} status={j.get(\"last_status\",\"\")}') for j in d.get('jobs',[])]"
...
```bash
# Find junk articles
ls -lt ~/.{{AGENT_NAME_LOWER}}/cron/output/{job_id}/ | head -10
# Check content of recent files
for f in $(ls -t ~/.{{AGENT_NAME_LOWER}}/cron/output/{job_id}/2026-*.md | head -5); do
  echo "=== $f ==="
  head -20 "$f"
done
```
...
- `launchctl list | grep {{AGENT_NAME_LOWER}}` shows no `ai.{{AGENT_NAME_LOWER}}.cron-runner` plist
...
tail -5 ~/.{{AGENT_NAME_LOWER}}/P6-prefrontal/logs/cron-runner.log
...
find ~/.{{AGENT_NAME_LOWER}}/cron/output/*/ -name "*.md" -mmin -10
...
- `launchctl list | grep ai.{{AGENT_NAME_LOWER}}.cron-runner` PID=- → launchd may not track detached processes
...
JOBS = "~/.{{AGENT_NAME_LOWER}}/cron/jobs.json"
OUT = Path("~/.{{AGENT_NAME_LOWER}}/cron/output")
...
- `~/.{{AGENT_NAME_LOWER}}/P6-prefrontal/logs/cron-runner.log` mtime is hours/days stale
...
ps aux | grep {{AGENT_NAME_LOWER}}_cli.main.*gateway | grep -v grep
...
sleep 8
tail -5 ~/.{{AGENT_NAME_LOWER}}/logs/gateway.log
...
python3 -c "import json; d=json.load(open('~/.{{AGENT_NAME_LOWER}}/cron/jobs.json')); [print(f\"{j.get('name','?'):30} next={str(j.get('next_run_at',''))[:19]}\") for j in d.get('jobs',[]) if j.get('enabled')]"
...
**Layer 3.5 of `{{AGENT_NAME_LOWER}}_harmony_check.sh` correctly detects this**:
```
...
- Script file exists at `~/.{{AGENT_NAME_LOWER}}/scripts/<name>.sh` and `~/.hermes/scripts/<name>.sh`
...
**Root cause:** `_run_script_subprocess()` in `~/.{{AGENT_NAME_LOWER}}/source/{{AGENT_NAME_LOWER}}-agent/cron/scheduler.py` (line 801–851):

1. **Relative path not resolved to scripts_dir**: `os.path.expanduser(script_path)` + `os.path.isfile()` checks the **current working directory** (`~/.{{AGENT_NAME_LOWER}}/source/{{AGENT_NAME_LOWER}}-agent/`) instead of `scripts_dir` (`~/.{{AGENT_NAME_LOWER}}/scripts/`).
...
2. Relative paths resolved against `get_{{AGENT_NAME_LOWER}}_home() / "scripts/"` (not cwd)
...
**Script location double-check:** no_agent scripts need to exist at BOTH locations. Canonical origin is `~/.hermes/scripts/` but the {{AGENT_NAME}} customize layer redirects HERMES_HOME → `~/.{{AGENT_NAME_LOWER}}`, so the active scheduler reads from `~/.{{AGENT_NAME_LOWER}}/scripts/`. Copy scripts to both locations.
...
**Stale error clearing:** After restart, old `last_error` from before the patch persists in `jobs.json`. It auto-clears on the next successful tick. For long-cycle jobs (e.g. 6h), manually patch `last_status` → `"ok"` and `last_error` → `null` in `~/.{{AGENT_NAME_LOWER}}/cron/jobs.json`.

## Step 3: Apply recoveryscheduler reads from `~/.{{AGENT_NAME_LOWER}}/scripts/`. Copy scripts to both locations.
...
**The 4 pillars we adopt from [Garry Tan's GBrain](https://github.com/garrytan/gbrain)** (lightly adapted for a single-machine {{AGENT_NAME}}):

1. **Repo (vault)**: our `~/.{{AGENT_NAME_LOWER}}/P2-hippocampus/memories/MEMORY.md` + `~/.{{AGENT_NAME_LOWER}}/skills/` + incidents directory, all wikilinked. ✓ already had this.
...
3. **Graph traversal**: `~/.hermes/scripts/{{AGENT_NAME_LOWER}}_graph_lookup.sh` — given a topic, walks wikilinks across MEMORY.md, P0-brainstem neurons, P4-cortex growth, P6-prefrontal incidents. Returns direct hits + incoming wikilinks + outgoing wikilinks. ✓ implemented 6/10.
4. **Gap analysis**: `~/.hermes/scripts/{{AGENT_NAME_LOWER}}_graph_gap_analysis.sh` — detects (a) **dangling wikilinks** in MEMORY.md pointing to non-existent files, (b) **orphan vault files** not referenced from memory. ✓ implemented 6/10.
...
python3 -c "import json; d=json.load(open('$HOME/.{{AGENT_NAME_LOWER}}/cron/jobs.json')); [print(f'{j.get(\"name\",\"?\"):30} next={str(j.get(\"next_run_at\",\"\"))[:19] or \"NULL\"}') for j in d.get('jobs',[]) if j.get('enabled')]"

# Verify: output dir has new file from this tick
ls -lt ~/.{{AGENT_NAME_LOWER}}/cron/output/*/ 2>/dev/null | head -5
```
...
- "log rotation infrastructure" (NEW follow-up that emerged from F1-F3 work) → consolidated: wrote `{{AGENT_NAME_LOWER}}_log_rotate.sh` + cron registration. Done.
...
Use the 3-phase QA gate pattern for any code change. Evidence goes in `~/.{{AGENT_NAME_LOWER}}/P2-hippocampus/qa-evidence/{task_id}/`.
...
- `{{AGENT_NAME_LOWER}}-runtime-checkup` — broader system checkup
