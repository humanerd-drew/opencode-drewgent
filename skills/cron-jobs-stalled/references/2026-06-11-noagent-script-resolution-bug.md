...
**Symptom:** All 3 no_agent jobs show `last_status=error`, `last_error="Script not found: <filename>.sh"` despite scripts existing at `~/.loragent/scripts/`

## Root Cause

Loragent's cron scheduler (`~/.loragent/source/loragent-agent/cron/scheduler.py`) has a bug in `_run_script_subprocess()` (line 801–851):

1. **Relative path not resolved to scripts_dir**: The function calls `os.path.expanduser(script_path)` then `os.path.isfile(expanded_script)`. For relative paths like `"loragent_launchd_watchdog.sh"`, this checks the **current working directory** (`~/.loragent/source/loragent-agent/`) instead of `scripts_dir` (`~/.loragent/scripts/`).
...
Patched `_run_script_subprocess()` in `~/.loragent/source/loragent-agent/cron/scheduler.py`:
...
def _run_script_subprocess(job, script_path, ...):
    ...
    if raw.is_absolute():
        ...
        resolved = get_loragent_home() / "scripts" / raw
    ...
    try:
        env = {**os.environ, "DREW_HOME": str(get_loragent_home())}
        result = subprocess.run(
            argv,
            capture_output=True, text=True, timeout=300,
            env=env, cwd=str(get_loragent_home()),
        )
...
python3 -c "
import json
j = json.load(open('~/.loragent/cron/jobs.json'))
for job in j['jobs']:
...
for job in j['jobs']:
    if job['id'] in ('96ad18409db7',):  # target job ids that need clearing
        job['last_status'] = 'ok'
        job['last_error'] = None
json.dump(j, open('~/.loragent/cron/jobs.json', 'w'), indent=2)
```
...
|----------|-------------|-------------------|
| `~/.loragent/scripts/<name>` | Loragent scheduler | `get_loragent_home() / "scripts"` |
| `~/.hermes/scripts/<name>` | Hermes scheduler | `get_hermes_home() / "scripts"` |

The Loragent customize layer redirects `HERMES_HOME` → `~/.loragent`, so the active scheduler reads from `~/.loragent/scripts/`. But the canonical origin of these scripts is `~/.hermes/scripts/` — they need to exist at both paths.

## How This Was Found

1. 3 no_agent jobs showed "Script not found" despite scripts existing at `~/.hermes/scripts/`
2. Inspected `_run_script_subprocess()` in Loragent's `cron/scheduler.py` — relative paths not resolved to scripts_dir
...
| File | Purpose |
|------|---------|
| `~/.loragent/cron/jobs.json` | **Active** — the `cronjob` tool writes here, Loragent scheduler reads from here |
| `~/.loragent/P3-sensors/cron/jobs.json` | **Stale** — old copy from May 14, not actively used |

Always modify `~/.loragent/cron/jobs.json`. The `P3-sensors` copy can be ignored.
