...
stalling the entire tick for 15-26 minutes. `{{AGENT_NAME_LOWER}}-cron-runner-001`
(script-based) could not fire behind it.

## Fix Applied

### 1. Disable redundant LLM job (`~/.{{AGENT_NAME_LOWER}}/cron/jobs.json`)

`d1ef68ced116` was redundant — `cron_runner.py` (the script behind
`{{AGENT_NAME_LOWER}}-cron-runner-001`) already dispatches all 3 boards: default, content,
...
```json
{
  ...
  "paused_reason": "T4 fix: redundant with {{AGENT_NAME_LOWER}}-cron-runner-001 (cron_runner.py handles all boards)"
}
```
...
| T4.4 | `{{AGENT_NAME_LOWER}}_harmony_check.sh` | Layer 3.5b: fire frequency detection |
| T4.6 | `{{AGENT_NAME_LOWER}}_cron_watchdog.sh` | Auto kickstart on 0 fires in 5 min |
...
grep -E '=== 2026-' ~/.{{AGENT_NAME_LOWER}}/logs/cron-runner/$(date +%Y-%m-%d).log | tail -3

# 4. Check Running job order (script first)
grep "Running job" ~/.{{AGENT_NAME_LOWER}}/P6-prefrontal/logs/gateway.log | tail -3

# 5. Harmony check
bash ~/.hermes/scripts/{{AGENT_NAME_LOWER}}_harmony_check.sh | grep -A 1 "Layer 3.5"
```

Expected: cron-runner fire present, `{{AGENT_NAME_LOWER}}-cron-runner-001` runs before any LLM job, Layer 3.5 shows ✓ or small gap (<120s).
