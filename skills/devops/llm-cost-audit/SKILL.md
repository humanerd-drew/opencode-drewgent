---
name: llm-cost-audit
title: Loragent LLM Cost Audit
description: Loragent의 LLM 호출 작업을 inventory하고 cost optimization 후보를 도출하는 표준 절차
domain: devops
space: growth
type: skill
...
# Loragent LLM Cost Audit

Loragent의 LLM 호출 작업을 inventory하고 cost optimization 후보를 도출하는 표준 절차.
...
```bash
cat ~/.loragent/cron/jobs.json | jq '.jobs[] | {id, name, schedule: .schedule.expr, enabled, model, prompt_len: (.prompt | length)}'
```
...
```bash
grep -n "AIAgent\|chat\.completions\|messages\.create" ~/.loragent/scripts/run_kanban_worker.py
grep -n "AIAgent" ~/.loragent/scripts/dispatch_once_*.py
```
...
grep -rn "AIAgent\|chat\.completions\|call_llm" \
  ~/.loragent/source/loragent-agent/acp_adapter/ \
  ~/.loragent/source/loragent-agent/gateway/run.py \
  ~/.loragent/source/loragent-agent/cli.py 2>/dev/null
```
...
```bash
grep -rn "call_llm" ~/.loragent/source/loragent-agent/agent/ | grep -v test
```
...
```bash
grep -A2 "model:\|provider:\|cheap_model:\|summary_provider:\|fallback_model" ~/.loragent/config.yaml | head -50
```
...
2. **Kanban worker 결정론적 여부** (Finding #2)
   ```bash
   grep -n "AIAgent" ~/.loragent/scripts/run_kanban_worker.py
   ```
...
3. **Cron scheduler LLM dispatch** (Finding #3)
   ```bash
   grep -n "AIAgent" ~/.loragent/source/loragent-agent/cron/scheduler.py
   ```
...
1. `display.show_cost: true` 켜기 (`~/.loragent/config.yaml`)
