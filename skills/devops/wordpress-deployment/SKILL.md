...
Docker (colima) ──→ WordPress (localhost:8080)
                      ...
                      ├── wp-content/ → /Volumes/YOUR_NAS/docker/wordpress/ (NAS)
...
| File | Path |
|------|------|
| Docker Compose | `~/.{{AGENT_NAME_LOWER}}/wordpress/docker-compose.yml` |
| Credentials | `~/.{{AGENT_NAME_LOWER}}/wordpress/.wp-env` (chmod 600) |
| Uploads | `/Volumes/YOUR_NAS/docker/wordpress/wp-content/` (NAS) |
...
A STDIO MCP server at `~/.{{AGENT_NAME_LOWER}}/scripts/wordpress-mcp-server.js` wraps wp-cli and exposes 7 tools:
...
The server wraps `docker exec YOUR_DOMAIN-wp wp --allow-root` calls and translates JSON-RPC 2.0 requests.
...
mcp_servers:
  wordpress:
    command: node
    args: ["~/.{{AGENT_NAME_LOWER}}/scripts/wordpress-mcp-server.js"]
```
...
```bash
# View logs
docker logs YOUR_DOMAIN-wp
docker logs YOUR_DOMAIN-db

# Backup DB
docker exec YOUR_DOMAIN-db mysqldump -u YOUR_DB_USER -p YOUR_DB_NAME > backup.sql

# Update WordPress
docker exec YOUR_DOMAIN-wp wp --allow-root core update

# Update plugins
docker exec YOUR_DOMAIN-wp wp --allow-root plugin update --all
```
