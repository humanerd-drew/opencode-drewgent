...
### Input files
- `~/.{{AGENT_NAME_LOWER}}/logs/errors.log` (primary)
- `~/.{{AGENT_NAME_LOWER}}/logs/agent.log` (fallback)
...
| Watchdog cron error | cron job "{{AGENT_NAME}} launchd watchdog" in errors | error |
...
The `ai.{{AGENT_NAME_LOWER}}.gateway-watchdog` launchd service is `OnDemand=true` — it does NOT stay running (no PID). The actual watchdog is the **cron job** "{{AGENT_NAME}} launchd watchdog" running every 5 minutes. Alert logic checks the cron job status, NOT the launchd service PID.
