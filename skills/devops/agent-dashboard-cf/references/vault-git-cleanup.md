...
When the dashboard shows many dirty files in the Git section (e.g., 246 uncommitted), use this workflow to clean up the vault repo and push to `YOUR_GITHUB_ORG/{{AGENT_NAME_LOWER}}`.
...
`YOUR_GITHUB_ORG/{{AGENT_NAME_LOWER}}` has `main` branch protected (force push blocked).

### If repos haven't diverged:
```bash
git push YOUR_GITHUB_ORG main
```

### If force push needed (local is the new source of truth):
```bash
# 1. Check current protection
gh api repos/YOUR_GITHUB_ORG/{{AGENT_NAME_LOWER}}/branches/main/protection

# 2. Disable protection (temporary)
gh api -X DELETE repos/YOUR_GITHUB_ORG/{{AGENT_NAME_LOWER}}/branches/main/protection

# 3. Force push
git push --force YOUR_GITHUB_ORG main

# 4. Re-enable protection
gh api -X PUT repos/YOUR_GITHUB_ORG/{{AGENT_NAME_LOWER}}/branches/main/protection \
  --input - <<'EOF'
{
...
```bash
git fetch YOUR_GITHUB_ORG main
git log --oneline HEAD..YOUR_GITHUB_ORG/main  # commits we don't have
git merge YOUR_GITHUB_ORG/main  # or rebase, depending on preference
```
