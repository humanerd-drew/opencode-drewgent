---
name: loragent-llm-cost-opt-impl
title: Loragent LLM Cost Optimization — 3-Round Implementation
description: 3-round implementation playbook (config → scheduler → worker) for reducing background/cron LLM calls in Loragent. Use after `llm-cost-audit` produces candidate list. Captures verified pitfalls from 2026-06-02 production patch.
domain: devops
space: growth
type: skill
tags: [devops, cost, llm, optimization, implementation, loragent]
created: 2026-06-02
updated: 2026-06-02
links:
  - "[[skills/devops/llm-cost-audit]]"
  - "[[skills/devops/cron-script-fastpath]]"
  - "[[skills/software-development/yaml-config-patch-loragent]]"
  - "[[skills/software-development/llm-model-migration]]"
  - "[[P4-cortex/growth/KANBAN-REVIEW-20260520]]"
  - "[[P0-brainstem/brain/rules]]"---

# Loragent LLM Cost Optimization — 3-Round Implementation
...
grep -E "model:|cheap_model:" ~/.loragent/config.yaml
```

Also check **DB path canonical** — `~/.loragent/P2-hippocampus/kanban/state/loragent_tasks.db`
is canonical. The top-level `~/.loragent/state/` is stale. Verify:

```bash
find ~/.loragent -name loragent_tasks.db -not -path '*/.git/*' -not -path '*/node_modules/*' -not -path '*/archive/*'
# Two files will appear. Use the P2-hippocampus one for INSERT/SELECT.
```

## Round 1 — Config-only (5 min, 0 risk)

**Scope**: `auxiliary.compression` in BOTH `~/.loragent/config.yaml` AND
`~/.loragent/P5-ego/config/config.yaml`. Affects `title_generator.py:38` and
...
def _run_script_subprocess(
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "DREW_HOME": str(_loragent_home)},
            cwd=str(_loragent_home),
        )
...
- Write the script under `~/.loragent/scripts/<job_name>.py` following the
...
jobs = json.load(open('~/.loragent/cron/jobs.json'))['jobs']
...
Then check `~/.loragent/cron/output/{board}/<job_id>_<timestamp>.md` exists
...
`P2-hippocampus/kanban/state/loragent_tasks.db`, NOT `~/.loragent/state/`.
...
VENV = '~/.loragent/source/loragent-agent/.venv/bin/python'
...
# 2. Run dispatcher
subprocess.run([VENV, '~/.loragent/scripts/dispatch_once_default.py'],
               env={'DREW_HOME': '~/.loragent', **os.environ}, check=True)

# 3. Wait for workers, check logs
log_dir = Path('~/.loragent/P4-cortex/scripts/kanban/logs/workers')
for _ in range(30):
...
- **DB path stale** (Round 3): `~/.loragent/state/` is stale; P2-hippocampus
  path is canonical. Two separate `loragent_tasks.db` files exist.
...
- `yaml-config-patch-loragent` — Round 1 only (dual-config patching)
