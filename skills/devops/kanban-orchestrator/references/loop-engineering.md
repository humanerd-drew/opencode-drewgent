...
## {{AGENT_NAME}} Assessment by Component
...
**Key gap**: `/goal` (keep going until condition met, separate judge model) only exists in kanban `goal_mode` (for dispatched workers). Main CLI session lacks it. But for {{AGENT_NAME}}'s architecture, kanban `goal_mode` is the right place — detached background worker beats a blocking in-session loop.
...
| Shared across repos | `skill_manage` tool + `~/.{{AGENT_NAME_LOWER}}/skills/` |
...
| Agent definition files | ✅ **NEW**: `~/.{{AGENT_NAME_LOWER}}/agents/*.md` — 8 profiles with frontmatter |
...
## Applying the Framework in {{AGENT_NAME}}
