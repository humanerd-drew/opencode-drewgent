...
space: growth
type: workflow
tags: [minimax, token-plan, api, usage, loragent]
created: 2026-06-02
updated: 2026-06-02
links:
...
**Auth source**: `MINIMAX_API_KEY` from `~/.loragent/.env` (also exported in `~/.zshrc`).
...
`~/.loragent/scripts/minimax_usage.py` (7.1KB, no external deps, stdlib only)

```bash
python3 ~/.loragent/scripts/minimax_usage.py            # colored table + progress bars
python3 ~/.loragent/scripts/minimax_usage.py --json     # raw JSON for monitoring
python3 ~/.loragent/scripts/minimax_usage.py --watch 30 # 30s refresh, Ctrl-C 종료
```
...
```bash
alias mm-usage='python3 ~/.loragent/scripts/minimax_usage.py'
alias mm-usage-watch='python3 ~/.loragent/scripts/minimax_usage.py --watch 30'
```
...
`start_time` / `end_time` are ms-epoch UTC, but `datetime.fromtimestamp(...).astimezone()` renders them in the user's local TZ (KST in Loragent's case). Don't double-convert or you'll show wrong times.

## 6. Future Extensions (suggested, not implemented)

- **Statusline integration**: `mm: 51% | 5d16h` next to the prompt. Touches Loragent identity layer → confirm with user before implementing.
...
- [[P4-cortex/growth/INTEGRATION_PROTOCOL]] — Loragent 3-file integration protocol
- [[P5-ego/SELF_MODEL]] — Loragent identity (for statusline extension)
- `~/.loragent/scripts/minimax_usage.py` — the implementation
- `~/.loragent/.env` — API key source
