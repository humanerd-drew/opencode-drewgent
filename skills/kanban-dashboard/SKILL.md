...
type: skill
description: Loragent kanban board UI — Flask dashboard server for visual task management
space: outcome
tags: [outcome]
created: 2026-05-20
updated: 2026-06-15
...
- MacMini 재부팅해도 자동 실행
- 로그: `~/.loragent/P6-prefrontal/logs/kanban-server.log`
...
# 로그 확인
tail -f ~/.loragent/P6-prefrontal/logs/kanban-server.log
```
...
| `_obsidian_url(vault_path)` | 절대경로 → `obsidian://open?vault=Loragent&file=...` URL 변환 (`urllib.parse.quote`로 인코딩) |
...
- URI scheme: `obsidian://open?vault=Loragent&file={encodeURIComponent(relative_path)}`
- draft 경로가 없으면 버튼 미표시

### File path convention
- 절대경로: `~/.loragent/P2-hippocampus/memories/insights/YYYY-MM-slug.md`
- 상대경로 (URL용): `P2-hippocampus/memories/insights/YYYY-MM-slug.md`
- vault name: `Loragent` (고정 — `~/.loragent` 디렉토리명)
...
- Server script: `~/.loragent/P4-cortex/scripts/kanban_dashboard_server.py`
- LaunchAgent plist: `~/Library/LaunchAgents/ai.loragent.kanban-dashboard.plist`

## Pitfalls

- `get_tasks()` in server uses its own `init_db()` — task table schema must match `loragent_tasks.db` (both use same board column). If schema mismatches, board returns empty.
...
- **Server restart required**: Changing `kanban_dashboard_server.py` doesn't auto-reload. Must run `launchctl stop ai.loragent.kanban-dashboard && launchctl start ai.loragent.kanban-dashboard` to apply changes.
...
- **Obsidian URL dependency**: `obsidian://open` requires (1) Obsidian.app installed on macOS, (2) browser-registered URL scheme handler. Links silently fail on non-macOS or without Obsidian. The `_obsidian_url()` function only generates URLs for paths under `~/.loragent/` — other paths render as plain text.
...
1. Cron Trigger (every 5min)
   ↓
2. SQLite Node — Query loragent_tasks.db
   SQL: |
...
Task event fires (completed/blocked/crashed)
  ↓
1. Event Listener (from LoragentTaskStore.task_events table)
   ↓
