...
- URI: `obsidian://open?vault=Loragent&file=...`
- Fallback: if path doesn't match `~/.loragent/` prefix, show as text only

**Server-side helpers**:
- `_extract_draft_path(body)` — regex `## Draft 파일 위치\s*\n\s*(/[^\n]+)`
- `_obsidian_url(vault_path)` — strips `~/.loragent/` prefix, URL-encodes with `urllib.parse.quote`
...
4. Final state: 6 tasks, all loragent-assigned
