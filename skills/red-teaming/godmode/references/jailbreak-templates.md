...
## 5. GODMODE FAST — {{AGENT_NAME}} 4 405B (Zero Refusal)

**Model:** `nousresearch/{{AGENT_NAME_LOWER}}-4-405b`
**Strategy:** {{AGENT_NAME}} models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with {{AGENT_NAME}} Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.{{AGENT_NAME_LOWER}}/config.yaml`:
