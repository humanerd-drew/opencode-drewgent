...
Deployed as `ai.{{AGENT_NAME_LOWER}}.huly-bridge` (launchd daemon):
...
| File | Purpose |
|------|---------|
| `~/.{{AGENT_NAME_LOWER}}/scripts/huly_bridge.js` | Bridge daemon — connects, registers pushHandler, processes events |
| `~/.{{AGENT_NAME_LOWER}}/scripts/huly_bridge.sh` | Bash wrapper (reads key, launches node) |
| `~/.{{AGENT_NAME_LOWER}}/logs/huly-bridge.log` | Daemon stdout/stderr |

## launchd plist

Installed at `~/Library/LaunchAgents/ai.{{AGENT_NAME_LOWER}}.huly-bridge.plist`:

- Label: `ai.{{AGENT_NAME_LOWER}}.huly-bridge`
