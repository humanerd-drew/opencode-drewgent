...
**{{AGENT_NAME}}의 현재 상태**:
- Kanban이 자체 task queue를 관리
- MCP client로 외부 MCP 서버에 연결 가능
...
**{{AGENT_NAME}} 적용 시**:
- Kanban event → Discord/Telegram notification
...
Kanban orchestrator가 이미 parent-child link를 지원:
...
{{AGENT_NAME}}는 자체 `memories/concepts/` + `P4-cortex/knowledge/`를 검색 인프라로 활용.
...
**{{AGENT_NAME}} 적용**: kanban HTML dashboard가 이미 비슷함. 추가로:
...
**{{AGENT_NAME}} 적용**: QA gate가 이미 유사. `contract.json` criteria를 AI가 제안하고 사용자가 승인하는 구조로 발전 가능.
...
**{{AGENT_NAME}} 적용**: kanban task의 `assignee`를 역할 단위로 관리:
- `assignee="admin"`, `assignee="customer"`, `assignee="planner"`
- dispatcher가 역할 기반으로 worker 배정
...
**{{AGENT_NAME}} 적용**: AI provider별 cost tracking:
- API cost per token 추적
- Kanban task 완료 시 cost 기록 (`task_runs.cost`)

---

## 정리: Manyfast의 것을 {{AGENT_NAME}}에 내재화

| Manyfast 기능 | {{AGENT_NAME}} 구현 | 상태 |
|---------------|-------------------|------|
...
| MCP integration | MCP client | Done |
...
```
~/.youragent/memories/concepts/prd-schema.md      — PRD 스키마 정의
~/.youragent/P4-cortex/knowledge/prd-template.md — PRD 작성 템플릿
~/.youragent/P4-cortex/growth/manyfast-study.md  — 이 문서 (아키텍처 분석 결과)
```
