---
title: gbrain-integration-loragent
name: gbrain-integration-loragent
description: Install and configure GBrain as a Hermes MCP server for Loragent vault hybrid search
type: skill
tags: [gbrain, mcp, vault, search, loragent]
...
# GBrain Integration for Loragent

Integrate GBrain (by Garry Tan) as an MCP server into Loragent/Hermes for hybrid search over the `.loragent` Obsidian vault.
...
gbrain sources add loragent --path ~/.loragent --name "Loragent Vault"
gbrain sources default loragent
gbrain import ~/.loragent/P0-brainstem --source loragent --yes --no-embed
...
import yaml
with open('~/.loragent/config.yaml') as f:
    d = yaml.safe_load(f)
d['mcp_servers']['gbrain']['env'] = {'OPENAI_API_KEY': 'ollama-local'}
with open('~/.loragent/config.yaml', 'w') as f:
...
Two types of parent:
- **Gateway** (`loragent_cli.main gateway run --replace`) — KEEP this one
...
```bash
# Script: ~/.hermes/scripts/loragent_gbrain_watchdog.sh
