...
### 상황
`~/.{{AGENT_NAME_LOWER}}/config.yaml`에 MCP 서버 추가. `mcp_servers` 블록에 두 개의 secret이 있었음:
...
- [[@identity/brain/{{AGENT_NAME}}-brain/P0-brainstem/禁/禁secrets_in_code.neuron]] — 코드에 secret 박지 말라는 P0 규칙 (이 skill은 **편집 시**의 secret 보존)
- [[@identity/brain/{{AGENT_NAME}}-brain/P0-brainstem/禁/禁blind_write.neuron]] — 읽기 없이 쓰지 말라 (이 skill도 같은 원칙을 patch에 적용)
