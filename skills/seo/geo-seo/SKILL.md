---
name: geo-seo
title: GEO-first SEO — Generative Engine Optimization for {{AGENT_NAME}}
...
created: 2026-06-27
updated: 2026-06-27
trigger: "Kanban task trend-apply-geo-seo-claude — zubair-trabzada/geo-seo-claude를 {{AGENT_NAME}} 아키텍처(WordPress + Cloudflare Workers + Obsidian vault)에 맞게 재구성"
provenance:
  session: "2026-06-27 trend-apply-geo-seo-claude"
  decision: "{{AGENT_NAME}}의 기존 skill 레이아웃(단일 SKILL.md)을 따르고, seo-article-harvester/trend-scorer 파이프라인과 연결. geo-seo-claude의 13개 sub-skill 대신 6개 GEO 모듈로 압축하여 opencode run 기반 워커에서 실행 가능하도록 설계."
links:
...
### 2.2 llms.txt
`https://YOUR_DOMAIN/llms.txt`에 사이트의 정체성과 중요 경로를 요약한다.
...
**배포 방법**:
- WordPress MU plugin(`YOUR_AGENT-ard.php`)에 경로 추가
...
```bash
python3 ~/.{{AGENT_NAME_LOWER}}/P4-cortex/scripts/seo_audit.py --modules ai_crawlers,llms,citability,brand
```
...
| Signal | Action | Owner |
|--------|--------|-------|
| Entity consistency | "{{AGENT_NAME}}", "Your Name", "YOUR_BRAND" 철자/형식 통일 | 콘텐츠 전반 |
...
```bash
python3 ~/.{{AGENT_NAME_LOWER}}/P4-cortex/scripts/brand_scanner.py --brand "{{AGENT_NAME}}" --platforms reddit,youtube,linkedin,wikipedia
```
...
```json
{
  ...
  "founder": {
    "@type": "Person",
    "name": "Your Name",
    "sameAs": "https://YOUR_DOMAIN/about"
  }
}
```
...
```json
{
  ...
  "publisher": {
    "@type": "Organization",
    "name": "{{AGENT_NAME}}",
    "logo": { "@type": "ImageObject", "url": "https://YOUR_DOMAIN/logo.png" }
  },
...
## 8. Integration with {{AGENT_NAME}} SEO Pipeline
...
```markdown
# GEO Report — YOUR_DOMAIN (2026-06)
...
### 10.1 One-off audit
```bash
python3 ~/.{{AGENT_NAME_LOWER}}/P4-cortex/scripts/seo_audit.py \
  --url https://YOUR_DOMAIN \
  --output ~/.{{AGENT_NAME_LOWER}}/P6-prefrontal/logs/geo-seo-audit-$(date +%Y%m%d).json
```
...
```bash
python3 ~/.{{AGENT_NAME_LOWER}}/P4-cortex/scripts/brand_scanner.py \
  --brand "{{AGENT_NAME}}" \
  --output ~/.{{AGENT_NAME_LOWER}}/P4-cortex/growth/seo/brand-mentions/$(date +%Y-%m-%d).json
```

### 10.4 Retrofit task creation (kanban)
```bash
# via kanban_create in opencode run
python3 ~/.{{AGENT_NAME_LOWER}}/scripts/kanban_create.py \
