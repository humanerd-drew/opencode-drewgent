...
### Cron job script execution: .sh vs .js/.py

`_run_job_script()` in `cron/scheduler.py` chooses the interpreter by file extension:

- `.sh` / `.bash` → `/bin/bash`
- **everything else** → Python's `sys.executable`

This means a `.js` file run as a `no_agent` script is executed by **Python**, not Node.js, producing a `SyntaxError` on non-ASCII characters. A `.py` script works fine (Python running Python), but any non-Python script (`.js`, `.rb`, etc.) will fail.

**Fix for non-Python scripts:** Wrap in a `.sh` launcher that calls the right interpreter. Example for Node.js:
