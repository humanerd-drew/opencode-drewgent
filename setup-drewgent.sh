...
# 5. Symlinks the 'drewgent' CLI command into ~/.local/bin
...
echo ""
echo -e "${CYAN}⚕ Drewgent Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink drewgent into ~/.local/bin
# ============================================================================

echo -e "${CYAN}→${NC} Setting up drewgent command..."

HERMES_BIN="$SCRIPT_DIR/venv/bin/drewgent"
mkdir -p "$HOME/.local/bin"
ln -sf "$HERMES_BIN" "$HOME/.local/bin/drewgent"
echo -e "${GREEN}✓${NC} Symlinked drewgent → ~/.local/bin/drewgent"
...
echo ""
echo "Other commands:"
echo "  drewgent status        # Check configuration"
echo "  drewgent gateway install # Install gateway service (messaging + cron)"
echo "  drewgent cron list     # View scheduled jobs"
echo "  drewgent doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m drewgent_cli.main setup
fi
