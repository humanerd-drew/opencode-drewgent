...
|---|------|------|----------|----------------|
| 1 | cron tick (1분) | `dispatch_once()` → ready task spawned | `scheduler.py` | cron → scheduler.py → loragent_kanban_db.py |
| 2 | TTL 만료 | `_reclaim_stale_tasks()` → worker reclaim | `loragent_kanban_db.py:760` | dispatcher tick → reclaim → status='todo' |
| 3 | kanban_complete | created_cards hallucination check | `loragent_kanban_db.py:task_complete()` | tool call → DB verify → completion_blocked event |
| 4 | task_link | DFS cycle detection | `loragent_kanban_db.py:task_link()` | tool call → graph check → cycle이면 예외 |
...
agent가 kanban_complete(task_id, created_cards=[...])
  → task_complete()에서 각 ID가 loragent_tasks.db에 존재하는지 검증
...
grep -l "failed" ~/.loragent/cron/output/*/*.log 2>/dev/null | tail -5
...
| dispatcher tick 누락 | `launchctl start ai.loragent.gateway` (gateway restart) |
...
- [[P3-sensors/gateway/loragent-architecture-dataflow]] — gateway architecture (하네스가 embedded된运行环境)
- [[P5-ego/SELF_MODEL]] — 하네스 autonomous behaviors 원본 (P5-Ego bidirectional)

---

*Generated by Loragent — 2026-05-29*
