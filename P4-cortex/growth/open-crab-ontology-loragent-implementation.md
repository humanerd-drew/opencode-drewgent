---
title: OpenCrab Ontology — Loragent 적용 보고서
type: document
space: growth
...
# OpenCrab Ontology — Loragent 적용 보고서
...
Loragent의 7-layer subsumption architecture와 OpenCrab 9-Space는 같은 문제를 다른 축에서 본 것이다.
P-layer가 "수직적 권한 계층"이라면, space는 "수평적语义 범주"다.

---

## 2. Loragent에 적용한 것 — P0-P6 전체

### 2.1 범위: 전체 vault (P0-P6 + skills + memories + etc.)

OpenCrab ontology는 Loragent vault의 **전체 파일**에 적용되었다. P0-brainstem만 아니라 모든 레이어를 포함한다.
...
```
P0-brainstem/brain/Loragent-brain/P0-brainstem/ (10개)
...
P0-brainstem/brain/Loragent-brain/P0-brainstem/禁/ (6개)
...
| OpenCrab Field | Loragent Field | Description |
|----------------|---------------|-------------|
...
Loragent의 7-layer architecture를 OpenCrab 9-Space에 매핑:
...
```
1. ~/.loragent에서 모든 *.md 파일 탐색 (rglob)
...
→ 10 (Group A 전부) ✅
grep -l "space:" ~/.loragent/P0-brainstem/brain/Loragent-brain/P0-brainstem/禁/*.neuron | wc -l
→ 4 (Group B 전부) ✅

# 3. orphan .neuron 없음 (모든 .neuron이 links: 보유)
for f in $(find ~/.loragent/P0-brainstem -name "*.neuron"); do
...
# 5. cron job 존재 확인
jq '.jobs[] | select(.name=="ontology-frontmatter-sync")' ~/.loragent/cron/jobs.json
→ {"id":"frontmatter-sync", "name":"ontology-frontmatter-sync", ...} ✅

# 6. 스크립트 존재 확인
ls -la ~/.loragent/P4-cortex/scripts/ontology_frontmatter_sync.py
→ 6208 bytes ✅
```
...
| [[P4-cortex/knowledge/NEURONFS_RULES]] | NeuronFS architecture (Loragent native rules) |
...
**OpenCrab ontology를 Loragent vault 전체에 적용 완료.**
...
**핵심 인사이트**: OpenCrab의 9-space ontology와 Loragent의 7-layer subsumption architecture는 같은 문제를 다른 축에서 본 것이다. P-layer가 "수직적 권한 계층"이라면, space는 "수평적 semantic 범주"다. 두 시스템을 함께 사용하여 그래프 무결성을 보장한다.

---

*Generated by Loragent — 2026-05-21*
