...
| **터미널** | 직접 조작 — 모든 기능 | Loragent agent tool 직접 호출 |
...
### 1-3. @Loragent 명령어로 Task 생성

Discord에서 Loragent에게 직접 말하기:

```
@Loragent 새 테스크 만들어줘: SEO article harvester 개선
@Loragent 테스크 완료: t_abc123 결과=설정 파일 업데이트함
@Loragent 테스크 목록 보여줘
@Loragent 테스크的状态: t_abc123
```
...
cd ~/.loragent
python3 -c "
import sys; sys.path.insert(0, 'source/loragent-agent')
from tools.loragent_kanban_db import dispatch_once
