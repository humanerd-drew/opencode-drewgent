---
title: Loragent Kanban — Implementation Review & Remediation Plan
domain: growth
space: growth
type: review
...
aliases:
  ...
  - /projects/loragent-kanban-review
links:
...
# Loragent Kanban — Implementation Review & Remediation Plan
...
| Phase | Item | Status |
|-------|------|--------|
| 1 | loragent_kanban_db.py (SQLite store) | ✅ Done |
...
The implementation plan explicitly says `jobs.json` should have a `kanban-dispatcher` cron job. The cron runs (858 times confirmed), but the job is not in `jobs.json` — meaning Loragent has no declarative record of it, and Loragent's own cron management API cannot see/control it.

**Evidence**: 858 cron output files in `/Users/drew/.loragent/cron/output/d1ef68ced116/` but `jobs.json` has only 3 jobs (SEO, Trend, kanban-dispatcher only exists as a separate config).
...
In `loragent_kanban_db.py:864-867`:
```python
...
`task_list` in `loragent_kanban_db.py` does not have a `board` parameter. The dispatcher hardcodes `board='default'` in SQL, but the agent's `kanban_list` tool cannot filter by board. The schema in `KANBAN_LIST_SCHEMA` has no board field either.
...
The worker spawn sends a raw text prompt via stdin. But `KANBAN_WORKER_MODE=1` is set — no code actually checks this env var in the agent. The loragent CLI doesn't have a worker mode that reads `KANBAN_TASK_ID` and executes it directly. The worker is sent a text prompt and will start a full conversation with the LLM — this is not a targeted execution, it's an entire agent session.

More critically: the spawned process uses `venv_python -m loragent_cli.main --acp --stdio` but the ACP subprocess transport requires proper handshake. If the worker fails to initialize, there's no error handling — just a silent `proc.pid` returned and a task stuck in `in_progress`.
...
`notify_task_event` (loragent_kanban_db.py:713) logs to DB and returns subscriber list. The two paths (DB logging vs emoji-parsing hook) remain separate until a hook implementation is built.
...
`_reclaim_stale_tasks` is implemented in `loragent_kanban_db.py:760` and called from `dispatch_once`. However, the `kanban-worker` skill doesn't document what happens when a worker dies (TTL expiry → automatic reclaim). This is a documentation gap, not a code bug.
...
`kanban_worker/SKILL.md` references `task_get_events(task_id)` for monitoring. This function exists in `loragent_kanban_db.py` (implicit via `task_get` returning full task including events via the task_events table join), but there's no `kanban_get_events` tool in the toolset and no `handle_kanban_get_events` function.
...
**Priority**: LOW
**Status**: ✅ FIXED — parent_results() injected into worker prompt (loragent_kanban_db.py:939-955)
...
|---|--------|-------|----------|
| 1 | Add `board` to `KANBAN_CREATE_SCHEMA` and pass through `task_create()` | kanban_tools.py, loragent_kanban_db.py | P0 |
...
| 4 | Close stdin in `_spawn_worker_for_task` after writing prompt | loragent_kanban_db.py | P0 |
...
|---|--------|-------|----------|--------|
| 6 | Fix `_reclaim_stale_tasks` to promote children after reclaim | loragent_kanban_db.py | P1 | ✅ Done (line 831-832) |
...
| 9 | Implement `parent_results(task_id)` helper for worker handoff | loragent_kanban_db.py | P1 | ✅ Done (line 760-778) |
...
| # | Action | Files | Priority | Status |
|---|--------|-------|----------|--------|
| 11 | Implement prose scan for unresolved_refs in task_complete | loragent_kanban_db.py | P2 | ✅ Done (line 394-413) |
...
| 13 | Worker mode: implement KANBAN_WORKER_MODE check or replace spawn with targeted execution | loragent_kanban_db.py | P2 | ✅ Done (line 876-877 env vars + line 905 stdin.close()) |
| 14 | Add board filter to `dispatch_once` board-scoped ready-task query | loragent_kanban_db.py + jobs.json | P2 | ✅ Done (board='default' in jobs.json, dispatch_once(board=) param documented) |
...
**loragent_kanban_db.py** — `task_create()`:
...
**loragent_kanban_db.py** — `task_list()`:
...
**loragent_kanban_db.py** — `_spawn_worker_for_task()`:
```python
proc.stdin.write(prompt.encode("utf-8"))
proc.stdin.flush()
...
| P4-cortex/growth/loragent-kanban-implementation-plan.md | P4, cortex, kanban | P5-ego/SELF_MODEL, INTEGRATION_PROTOCOL |
...
- `parent_results()` wired into worker spawn prompt (loragent_kanban_db.py:939-955)
...
**File**: `~/.loragent/scripts/run_kanban_worker.py` (decide 175 line)
...
*Generated by Loragent self-review — 2026-05-20*
