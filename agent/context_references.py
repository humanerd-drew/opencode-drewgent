...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    drewgent_home = Path(
        os.getenv("DREW_HOME", str(home / ".hermes"))
    ).expanduser().resolve()

    blocked_exact = {home / rel for rel in _SENSITIVE_HOME_FILES}
    blocked_exact.add(drewgent_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(drewgent_home / rel for rel in _SENSITIVE_HERMES_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal Drewgent path and cannot be attached")
