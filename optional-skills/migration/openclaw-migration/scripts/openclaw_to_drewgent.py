#!/usr/bin/env python3
"""OpenClaw -> Drewgent migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into Drewgent Agent, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into Drewgent.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into Drewgent memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into Drewgent memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import Drewgent-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of Drewgent-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into Drewgent command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.drewgent/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.drewgent/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into Drewgent .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into Drewgent .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into Drewgent .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into Drewgent .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into Drewgent .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Drewgent config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Drewgent config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into Drewgent.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into Drewgent MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into Drewgent config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'drewgent cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into Drewgent config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into Drewgent session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into Drewgent custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Drewgent config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Drewgent config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Drewgent config.yaml approvals section.",
    },
    "memory-backend": {
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Drewgent config.yaml")
    ensure_parent(path)
    path.write_text(
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> Drewgent Migration Report",
        "",
...
class Migrator:
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Drewgent config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No Drewgent-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted Drewgent-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
        ...
        for oc_key, drewgent_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and drewgent_key not in secret_additions:
                secret_additions[drewgent_key] = val
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        drewgent_config = load_yaml_file(destination)
        current_model = drewgent_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = drewgent_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                drewgent_config["model"] = {"default": model_str}
            dump_yaml_file(destination, drewgent_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        drewgent_config = load_yaml_file(destination)
        existing_tts = drewgent_config.get("tts", {})
        ...
        if self.execute:
            ...
            drewgent_config["tts"] = merged_tts
            dump_yaml_file(destination, drewgent_config)
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct Drewgent destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected Drewgent-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected Drewgent-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        drewgent_cfg_path = self.target_root / "config.yaml"
        drewgent_cfg = load_yaml_file(drewgent_cfg_path)
        existing_mcp = drewgent_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Drewgent config")
                continue

            drewgent_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                drewgent_srv["command"] = srv["command"]
                if srv.get("args"):
                    drewgent_srv["args"] = srv["args"]
                if srv.get("env"):
                    drewgent_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    drewgent_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                drewgent_srv["url"] = srv["url"]
                if srv.get("headers"):
                    drewgent_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    drewgent_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                drewgent_srv["enabled"] = False
            if srv.get("timeout"):
                drewgent_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                drewgent_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                drewgent_srv["tools"] = {}
                if tools_cfg.get("include"):
                    drewgent_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    drewgent_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
            sampling = srv.get("sampling")
            if sampling and isinstance(sampling, dict):
                drewgent_srv["sampling"] = {
                    k: v for k, v in {
            ...
            existing_mcp[name] = drewgent_srv
            added += 1
        ...
        if added > 0 and self.execute:
            self.maybe_backup(drewgent_cfg_path)
            drewgent_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(drewgent_cfg_path, drewgent_cfg)
    ...
    def migrate_cron_jobs(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if self.archive_dir and self.execute:
            ...
            self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                        "Cron config archived. Use 'drewgent cron' to recreate jobs manually.")
        else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = drewgent_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> Drewgent reasoning_effort
        ...
        if compaction:
            compression = drewgent_cfg.get("compression") or {}
            ...
            drewgent_cfg["compression"] = compression
            changes = True

        # Map humanDelay
        human_delay = defaults.get("humanDelay") or {}
        if human_delay:
            hd = drewgent_cfg.get("human_delay") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'drewgent gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("DREW_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        drewgent_cfg_path = self.target_root / "config.yaml"
        drewgent_cfg = load_yaml_file(drewgent_cfg_path)
        sr = drewgent_cfg.get("session_reset") or {}
        changes = False
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        drewgent_cfg_path = self.target_root / "config.yaml"
        drewgent_cfg = load_yaml_file(drewgent_cfg_path)
        custom_providers = drewgent_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(drewgent_cfg_path)
            drewgent_cfg["custom_providers"] = custom_providers
            dump_yaml_file(drewgent_cfg_path, drewgent_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to Drewgent config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            drewgent_cfg_path = self.target_root / "config.yaml"
            drewgent_cfg = load_yaml_file(drewgent_cfg_path)
            discord_hermes = drewgent_cfg.get("discord") or {}
            changed = False
            ...
            if changed and self.execute:
                drewgent_cfg["discord"] = discord_hermes
                dump_yaml_file(drewgent_cfg_path, drewgent_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        drewgent_cfg_path = self.target_root / "config.yaml"
        drewgent_cfg = load_yaml_file(drewgent_cfg_path)
        browser_hermes = drewgent_cfg.get("browser") or {}
        changed = False

        # Map fields that have Drewgent equivalents
        ...
        if changed:
            drewgent_cfg["browser"] = browser_hermes
            if self.execute:
                self.maybe_backup(drewgent_cfg_path)
                dump_yaml_file(drewgent_cfg_path, drewgent_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            drewgent_mode = mode_map.get(mode, "manual")
            drewgent_cfg.setdefault("approvals", {})["mode"] = drewgent_mode
            if self.execute:
                self.maybe_backup(drewgent_cfg_path)
                dump_yaml_file(drewgent_cfg_path, drewgent_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{drewgent_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> Drewgent Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in Drewgent. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Drewgent Config Not Overwritten)",
                "",
                "These items already existed in your Drewgent config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the Drewgent agent discovers these",
            "directories, it may read/write to them instead of the Drewgent state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `drewgent claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `drewgent claw cleanup` to fix it.",
            "",
            "## Drewgent-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `drewgent claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `drewgent setup` to configure any remaining settings",
            "- Run `drewgent mcp list` to verify MCP servers were imported correctly",
            "- Run `drewgent cron` to recreate scheduled tasks (see archive/cron-config.json)",
            "- Run `drewgent gateway install` if you need the gateway service",
            "- Review `~/.drewgent/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Drewgent Agent.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=str(Path.home() / ".drewgent"), help="Drewgent home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Drewgent targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of Drewgent-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print(f"  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> Drewgent Migration   [{mode_label:>8s}]   ║")
    print(f"  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.drewgent/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.drewgent/config.yaml")
        print("    2. Run: drewgent mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: drewgent cron")
