"""Centralized logging setup for Drewgent Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.drewgent/logs/`` (profile-aware via ``get_drewgent_home()``).
...
def setup_logging(
    *,
    drewgent_home: Optional[Path] = None,
    ...
    """Configure the Drewgent logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    drewgent_home
        Override for the Drewgent home directory.  Falls back to
        ``get_drewgent_home()`` (profile-aware).
    log_level
    ...
    if _logging_initialized and not force:
        home = drewgent_home or get_drewgent_home()
        return home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_drewgent_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._drewgent_verbose = True  # type: ignore[attr-defined]
