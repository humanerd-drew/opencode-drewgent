...
=======================
state.db + Loragent vault에서 스킬 활용 데이터를 추출하고,
...
# ── Paths ──────────────────────────────────────────────────────────────────────
DREWENT_HOME = Path.home() / ".loragent"
...
STOPWORDS = {
    ...
    # Misc
    'friction', 'hugh-kim', 'humanerd', 'loragent',  # too specific to user/system names
...
def gather_project_context() -> dict:
    """Loragent vault에서 현재 프로젝트 맥락 추출"""
    context = {
        'keywords': [],
        'sources': [],
        'sessions_summary': '',
...
def generate_usage_scenario(skill: dict) -> str:
    ...
    domain_keywords = {
        ...
        'brain': ('Loragent Brain/治理', "현재 Loragent Brain/治理 체계 작업 중이라면 이 스킬로治理 규칙을 점검할 수 있습니다."),
