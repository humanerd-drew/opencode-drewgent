...
type: skill
space: outcome
description: Maintain .loragent as an Obsidian vault — graph connectivity overhaul (P-layer mesh, skill clusters, SEO article web), broken link detection, backlink density verification via Obsidian CLI, filename deduplication, bidirectional linking, auto-generated orphan management, cron output graph bloat prevention, and Obsidian compatibility.
...
Loragent's internal knowledge base (.loragent/) IS an Obsidian vault. This skill covers maintaining its health — broken link detection, backlink density, graph connectivity, auto-generated orphan management, and Obsidian compatibility.
...
**Fix** (`~/.loragent/.obsidian/app.json`):
```json
{
...
```
Loragent Agent ──MCP client──→ GBrain MCP server (gbrain serve)
                                       │
...
5. Import vault: `gbrain import ~/.loragent --yes --source loragent-vault`
...
- **GBrain's two-repo architecture** differs from Loragent's unified vault approach. GBrain is a search overlay, not a vault replacement.
...
- Gap analysis / dangling link detection (use `bash ~/.hermes/scripts/loragent_graph_gap_analysis.sh`)
...
grep -roh '\[\[[^]]*\]\]' ~/.loragent --include='*.md' | sort | uniq -c | sort -rn | head -20

# Check cron output volume
find ~/.loragent/cron/output -name '*.md' | wc -l

# Verify cron output embeds a skill with frontmatter links
head -30 ~/.loragent/cron/output/<job_id>/<recent-run>.md | grep -A5 '^links:'
```
...
|--------|-------------|-----|
| 1. Exclude from Obsidian | Cron outputs are reference-only (agent reads via `session_search`, not vault browser) | Add `"userIgnoreFilters": ["cron/output"]` to `~/.loragent/.obsidian/app.json`. Note: use `cron/output` (not `cron/output/*`) — Obsidian interprets directory patterns as recursive. |
| 2. Move to `.trash/` | Need recovery option before deletion | `mkdir -p ~/.loragent/.trash && mv ~/.loragent/cron/output/<job_id> ~/.loragent/.trash/<label>-$(date +%Y%m%d)` |
| 3. Periodic cleanup | Old outputs accumulate without value | `find ~/.loragent/cron/output -name '*.md' -mtime +X -delete` as a cron job |
...
| 7 | `[[P0-brainstem/.../禁/禁xxx]]` — missing `.neuron` extension | Add `.neuron`: `[[.../禁/禁xxx.neuron]]` |
| 8 | `[[skills/content-pipeline]]` — SKILL.md is nested under directory | Use `[[skills/content-pipeline/SKILL]]` |
| 9 | `[[P4-cortex/knowledge/index]]` — ambiguous (29 `index.md` files) | Use full path or disambiguate |
...
3. **Link to P-layer rules**: `[[P0-brainstem/brain/rules]]` — every skill is governed by the rules
4. **Link to SKILL-INDEX**: `[[P3-sensors/skills/SKILL-INDEX]]` — every skill is indexed
...
cat ~/.loragent/.obsidian/app.json
