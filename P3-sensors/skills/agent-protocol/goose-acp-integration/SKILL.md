...
description: Integrate Loragent with ACP (Agent Client Protocol) agents. Reverse-engineered from goose's Rust ACP provider implementation.
space: outcome
type: document
tags: [acp, protocol, goose, integration]
links:
  - "[[P3-sensors/skills/agent-protocol/DESCRIPTION]]"
  - "[[P3-sensors/skills/SKILL-INDEX]]"
created: 2026-05-10
---

# Goose ACP Integration for Loragent
...
let config = AcpProviderConfig {
    ...
    mcp_servers: extension_configs_to_mcp_servers(&extensions),  // Loragent tools as MCP
...
## Loragent Implementation Points

### 1. ACP Provider Tool
Spawn any ACP agent as subprocess, communicate via newline-delimited JSON-RPC over stdio.

### 2. MCP Extension Bridge
Convert Loragent tools → MCP server definitions → pass to ACP agent via `mcp_servers` config.

### 3. Permission Tiers
Map Loragent P0-P6 brain rules to ACP permission decisions (AllowAlways/AllowOnce/RejectOnce/RejectAlways/Cancel).

## Implementation Plan

**Phase 1:** `tools/acp_provider.py` — subprocess spawn + JSON-RPC over stdio + session lifecycle
**Phase 2:** Delegate to Claude Code via `claude-agent-acp`, pass Loragent tools as MCP servers
**Phase 3:** Permission callback system bridging Loragent P0 rules to ACP permission requests
