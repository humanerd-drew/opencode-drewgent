...
description: Loragent brain signal architecture — event bus, signal processor, awareness reporter, brain monitor
type: guide
space: outcome
...
# Brain Signal System — Loragent Self-Awareness Infrastructure
...
**Purpose**: Single source of truth for Loragent's brain signal architecture.
...
```bash
rm ~/.loragent/monitor/brain_signals_*.md  # 5,123 files → 0
# Retained: ~/.loragent/monitor/brain_signal_log.jsonl (5KB, compressed alternative)
```

### Verification

```bash
ls ~/.loragent/monitor/  # should show only brain_signal_log.jsonl
```
