...
Loragent의 NeuronFS Brain 상태를 실시간으로 모니터링하는 통합 시스템.
...
│                                                     │
│  ┌─────────────┐  ┌──────────────────────────────┐ │
│  │ Brain Rules │  │   Scripts (~/.loragent/scripts) │ │
...
```bash
python3 ~/.loragent/scripts/brain_html_dashboard.py
```
출력: `~/.loragent/brain_dashboard.html`
...
```bash
python3 ~/.loragent/scripts/brain_rule_monitor.py        # 실행
python3 ~/.loragent/scripts/brain_rule_monitor.py --check # 상태 확인
```
- brain_rule_state.json 와 비교하여 신규 규칙 탐지
- 새 규칙 발견 시 Discord 알림 (webhook)
- Discord 설정: `~/.loragent/config/discord.json` (webhook_url 필요)

### 4. session_end_hook.py (세션 종료 감시)
```bash
python3 ~/.loragent/scripts/session_end_hook.py          # 실행
python3 ~/.loragent/scripts/session_end_hook.py --check   # 상태 확인
```
- session_checkpoint.json 수정시간 감시
- 세션 종료 (5분 이상 비활성 후 재접속) 감지
- Dashboard 자동 실행 → ~/.loragent/logs/ 저장
...
| File | 용도 |
|------|------|
| `~/.loragent/brain_snapshot.json` | brain_nodes.py 성장 delta 저장 |
| `~/.loragent/brain_rule_state.json` | brain_rule_monitor.py 신규 규칙 탐지용 |
| `~/.loragent/.last_hook_run` | session_end_hook.py 마지막 실행 시간 |
| `~/.loragent/brain_dashboard.html` | 웹 대시보드 출력 |

## Discord Notification 설정

Discord webhook URL을 `~/.loragent/config/discord.json`에 저장:
```json
{
...
# 성장 모니터 수동 실행
python3 ~/.loragent/scripts/brain_rule_monitor.py

# 세션 종료 감지 확인
python3 ~/.loragent/scripts/session_end_hook.py --check
```
...
- **새 규칙 추가 시**: 새 .neuron 파일을 `~/.loragent/brain/Loragent-brain/<layer>/`에 추가하면 자동 탐지
