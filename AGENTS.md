...
# Loragent — Agent Guide

AI agent를 위한 Loragent 프로젝트 구조·규칙·컨벤션 가이드.

---

## Current Architecture (2026-07-08)

Loragent는 **opencode** 중심으로 운영한다. Hermes-Agent 제거됨, n8n 제거됨 (2026-06-18).

### Native Agent System

14개 subagent profiles가 `~/.loragent/.opencode/agents/*.md`에 opencode native 형식으로 정의됨.
TUI에서 `@agent-name` 으로 호출, 또는 `opencode run --agent <name>` 으로 CLI 호출.
자세한 목록은 `.opencode/agents/` 참조.

```
~/.config/opencode/              # opencode 설정
  opencode.jsonc                   # 메인 설정 (+ MCP: notion)
  skills/                          # opencode 스킬

~/.loragent/                     # Loragent 작업 디렉토리
  @identity/                       # 정체성 — brain rules, persona, SELF_MODEL
  @action/                         # 행동 로그 — incidents, migrations, plans, skills
  .opencode/
    agents/                        # 14개 native agent 프로필 (opencode format)
    tools/                         # 커스텀 TypeScript 툴
  .well-known/
    ai-catalog.json                # AI agent catalog
  harness/patterns/                # Manufacturing quality patterns
  launchd/                         # launchd plist 템플릿
  scripts/                         # Python/bash 스크립트
    discord_bot.py                 # Discord ↔ opencode 게이트웨이 봇 (--attach, streaming)
    discord_send.py                # Discord 메시지 청크 분할 전송
    vault_cli.py                   # 암호화 키 관리
    push-template.sh               # 템플릿 동기화
    ...
  cron/                            # 크론 스케줄러
  skills/                          # Loragent 전용 스킬 (100+)
  P0-P6/                           # Obsidian vault (P-layer brain architecture)

~/Library/LaunchAgents/          # launchd 서비스 (재부팅/크래시 자동 복구)
  ai.loragent.opencode.plist       # opencode serve (:8642, headless daemon)
  ai.loragent.discord-bot.plist    # Discord ↔ opencode 게이트웨이 (--attach, WaitForNetworkInterface)
  ai.loragent.cron.plist           # cron dispatcher (60초)
```
...
- **Loragent launchd watchdog**: 5분마다 서비스 상태 체크 (cron → loragent_launchd_watchdog.sh)

### Launchd Cron (2026-06-18, n8n 대체)

1개 launchd plist (`ai.loragent.cron`)가 60초마다 `loragent_cron.py` 실행. 상태는 `~/.loragent/logs/cron_state.json`에 저장.
...
~/.loragent/                    # Loragent 커스터마이징 + Obsidian vault (P0-P6)
  customize/                    # PYTHONPATH override 레이어
    sitecustomize.py            # Python startup hook
    hermes_cli/gateway.py       # launchd label override (ai.loragent.*)
...
1. `~/.zshrc`에서 `export PYTHONPATH=~/.loragent/customize:$PYTHONPATH`
...
| Hermes 기본값 | Loragent override | 파일 |
|--------------|-------------------|------|
| `ai.hermes.gateway` | `ai.loragent.gateway` | `customize/hermes_cli/gateway.py` |
| `ai.hermes.cron-runner` | `ai.loragent.cron-runner` | `customize/hermes_cli/gateway.py` |
| macOS Sonoma+ plist 파싱 | pid 추출 보정 | `customize/hermes_cli/gateway.py` |

### Config

- **Primary**: `~/.hermes/config.yaml` — Hermes-Agent 표준 설정
- **Legacy**: `~/.loragent/config.yaml` — 일부 Loragent 설정 (점진적 마이그레이션 중)
...
| 파일 | 용도 |
|------|------|
| `@identity/brain/rules.md` (or `P0-brainstem/brain/rules.md`) | 禁 Critical Rules (P0 우선) |
| `@identity/SELF_MODEL.md` (or `P5-ego/SELF_MODEL.md`) | 셀프 모델, 정체성 |
| `@identity/persona/SOUL.md` (or `P1-limbic/persona/SOUL.md`) | 성격, 어조, voice |
| `@identity/persona/writing-style-guide.md` (or `P1-limbic/...`) | 글쓰기 컨벤션 |
| `@action/skills/` | Bundled skill index (48 skills) |
| `skills/<category>/SKILL.md` | 각 스킬 가이드 |
| `.opencode/agents/` | 14 subagent profiles (native opencode format) |
| `.opencode/tools/` | Custom opencode tool definitions |
| `harness/patterns/manufacturing-bridge.md` | Manufacturing quality patterns |
| `.well-known/ai-catalog.json` | AI agent catalog |

### 일반 작업 시 탐색 순서

1. **설정 변경** → `~/.config/opencode/opencode.jsonc`
2. **스킬 생성/수정** → `~/.loragent/skills/<category>/SKILL.md` 또는 `@action/skills/<category>/SKILL.md`
3. **메모리 저장** → `memory()` 툴 (target: memory 또는 user)
4. **규칙 추가** → `@identity/brain/rules.md` (禁 규칙은 별도 .neuron 파일)
5. **오류 진단** → `@action/incidents/` 참조
6. **Subagent 호출** → `.opencode/agents/` 프로필 참조
...
```
---
title: loragent-provenance-convention
...
`@identity/`와 `@action/`은 upstream과의 호환성을 위한 flat 네임스페이스로, 내용은 P0-P6의 서브셋을 미러링함.
...
2. **훔칠 Taste 결정**: 제작자가 내린 결정 중 Loragent에 적용할 가치가 있는 것
3. **아키텍처 인사이트**: 구조적으로 배울 점
4. **Loragent 적용 가능성**: 적용 가능? 어떻게?
...
### Agent Office (2026-07-08)

14개 subagent profiles at `~/.loragent/.opencode/agents/*.md` — full org chart with orchestrator, designer, SRE, analyst, editor. 호출 방식:
...
| editor | deepseek-v4-pro | 콘텐츠 검수/한국어 인퓨전 |
...
```
~/.loragent/skills/ui/baseline-ui/SKILL.md
```
...
### Launchd plist 패턴
모든 Loragent 서비스: `KeepAlive { SuccessfulExit: false, ThrottleInterval: 10 }`
