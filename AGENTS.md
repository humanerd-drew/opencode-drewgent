...
| 3시간 | **content-manager-periodic** | opencode run (deepseek-v4-pro) |
...
| 매일 10:00 | trend-evaluate-trigger | opencode run (deepseek-v4-flash) → kanban task |
| 매일 20:00 | daily retro | opencode run |
| 월 03:00 | wiki-compile (archiver) | opencode run |
| 월 10:00 | trend-retire-trigger | opencode run (deepseek-v4-flash) → kanban task |
| 월 14:00 | seo-trend-trigger | opencode run (deepseek-v4-flash) → kanban task |
| 화/금 10:00 | taste-review-trigger | opencode run (deepseek-v4-flash) → kanban task |

**추가/변경:**
- `trend-evaluate-legacy` (every 2m, n8n trigger) → **비활성화** (n8n 제거됨, 중복)
...
| **Groq 내부 작업** (cron/batch/STT) | provider: groq, model: openai/gpt-oss-20b |
...
| **content-manager-periodic** | `deepseek-v4-pro` ($0.0035) | `deepseek-v4-flash` ($0.00038) | 글쓰기=수행. cron jobs.json override |
...
| **content-planner** | `deepseek-v4-pro` ($0.0035) | `deepseek-v4-pro` | 유지 — 계획은 pro 적정 |
