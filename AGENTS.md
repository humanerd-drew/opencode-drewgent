...
**Enforcement (2026-07-05):**
| 경로 | 시점 | 방식 |
|------|------|------|
| pre-commit hook | `git commit` 시 | bridge-lint 자동 실행, WARNING 출력 (block 안 함) |
| housekeeper deep clean | 매일 04:00 | bridge-lint 결과 Discord 리포트에 포함 |
...
| 두눈 실증 | CI-green ≠ live-works | D1/knowledge.db 직접 쿼리, process list, screenshot |
...
~/.config/opencode/              # opencode 설정
  opencode.jsonc                   # 메인 설정 (+ MCP 서버: discord/wordpress/gajae-code/astryx)
  skills/                          # opencode 스킬
...
| 15분 | gbrain watchdog | shell 스크립트 (deprecated, gbrain 제거됨) |
| 매시간 | **housekeeper** (pulse) | drewgent_housekeeper.py → Discord Bot API |
| 3시간 | **session-ingest** | ingest_sessions.py — P2 세션 로그 → knowledge.db (SQLite FTS5) |
| 3시간 | **content-curator** | content_curator.py ($0, Python) — 모든 소스 heuristic curation → kanban INSERT |
| 3시간 | **content-manager-periodic** | opencode run (agent) — kanban task → WordPress draft 작성 |
| 3시간 | **content-editor** | opencode run (agent) — draft QA → publish |
| 5분 | **office-autopilot** | 1개 task씩 순차 처리 (token 폭주 방지) |
| 일 10:00 | **content-weekly** | opencode run (agent) — 주간 요약 포스트 |
| 6시간 | trend-collect (parallel, 8 workers) → **trend-scorer** (30분 후, heuristic) → **trend-ingest** (6시간마다, $0) | Python/shell |
| 6시간 | **seo-ingest** (6시간마다, $0) | ingest_seo_articles.py |
| 매일 03:00 | seo-analyze | seo_analyzer.sh |
| 매일 04:00 | **housekeeper** (deep clean), log rotation, wiki-lint, QA evidence digest, session ingest | drewgent_housekeeper.py |
...
**추가/변경 (2026-07-05):**
- `ingest_sessions.py` — Ollama embedding 추가: nomic-embed-text(768d)로 semantic search 지원. 12,692건 전량 임베딩 완료.
- `ingest_trend_keeps.py` 🆕 — trend-harvester keep items를 knowledge.db에 ingest. 317건 임베딩 완료. cron: `trend-ingest` (6h, no_agent, $0).
- `ingest_seo_articles.py` 🆕 — SEO collected articles를 knowledge.db에 ingest. 2,062건 임베딩 완료. cron: `seo-ingest` (6h, no_agent, $0).
- `scripts/recall.py` 🆕 — CLI semantic search (cosine sim + FTS5 fallback). `$0`, numpy.
- `scripts/ingest_fact.py` 🆕 — 단건 지식 저장 (remember() 백엔드).
- `tools/recall.ts` 🆕 → `~/.config/opencode/tools/` — opencode native recall() MCP 툴.
- `tools/remember.ts` 🆕 → `~/.config/opencode/tools/` — opencode native remember() MCP 툴.
- `tools/memory-stats.ts` 🆕 → `~/.config/opencode/tools/` — opencode native memory-stats() MCP 툴.
- `opencode-knowledge-convention` 🆕 — Tier 1 규칙: 세션 시작 시 자동 recall, secrets 저장 금지.
- `.git/hooks/pre-commit` 🆕 — bridge-lint 자동 실행 (Tier 2 WARNING).
- `drewgent_housekeeper.py` — deep clean에 bridge-lint 체크 추가 (매일 04:00).
- `session_to_gbrain.py`, `contradiction_audit.py` → 제거 (gbrain 의존).
- `analyze_sessions.sh` → knowledge.db 기반으로 변경.

**추가/변경 (2026-07-04):**
- `session-to-gbrain` → `session-ingest`로 변경. gbrain(PGLite + Haiku) 대신 **SQLite FTS5**로 직접 저장.
- `ingest_sessions.py` 🆕 — P2 세션 로그 → knowledge.db 직접 INSERT. embedding 불필요, 500x 빠름.
- `tools/remember.ts` 🆕 — FTS5 기반 지식 저장 (구현 보류, 7/5에 완료).
- `opencode-knowledge-convention` 🆕 — Tier 1 규칙 도입.
...
### Knowledge Management (2026-07-05) — gbrain 대체 + Vector Search

Drewgent는 gbrain(Postgres + Haiku + 별도 바이너리)에서 **SQLite FTS5 + Ollama Embeddings** 기반 시스템으로 전환.
2026-07-04: SQLite FTS5 도입 (keyword search)
2026-07-05: **Ollama nomic-embed-text** 추가 — 12,692건 전량 임베딩 완료, semantic search 지원

**아키텍처:**
```
P2-hippocampus/sessions/    ← raw 세션 로그 (.jsonl / .json.gz)
    │
    ▼ ingest_sessions.py    ← 3h cron, SQLite INSERT + Ollama embedding (2000자 truncation)
    │
    └──→ knowledge.db       ← SQLite + FTS5 + embeddings (nomic-embed-text, 768d)
                                  │
                                  ├─ recall(query) → Ollama embed → cosine sim → ranked
                                  └─ FTS5 fallback (similarity < threshold)
```

**Tools (`~/.config/opencode/tools/`, auto-discovered):**
| 도구 | 파일 | 설명 |
|------|------|------|
| `recall(query, limit?, fts?)` | `tools/recall.ts` | 의미 검색 + FTS5 fallback |
| `remember(fact, type?)` | `tools/remember.ts` | 지식 저장 + Ollama embedding 자동 생성 |
| `memory-stats()` | `tools/memory-stats.ts` | DB 통계 |

**CLI equivalents:**
| 명령 | 설명 |
|------|------|
| `python3 scripts/recall.py "질문"` | 의미 검색 |
| `python3 scripts/recall.py "질문" --fts` | FTS5 keyword only |
| `python3 scripts/recall.py "질문" --json` | JSON 출력 |
| `echo '{"fact":"...","type":"..."}' \| python3 scripts/ingest_fact.py` | 단건 저장 |

**Convention (Tier 1 — 자동 수행):**
- 세션 시작 시 `recall("recent decisions")` 자동 호출
- 결정/선호/패턴 발견 시 `remember()` 즉시 저장
- secrets 절대 저장 금지

**의존성:**
- gbrain ❌ (완전 제거)
- Postgres / Anthropic Haiku / API key ❌
- **SQLite (Bun 내장)** ✅ — knowledge.db
- **Ollama** ✅ — nomic-embed-text (274MB, local, $0)
- **numpy** ✅ — cosine similarity

**성능:**
- 세션당 embedding: ~150ms (nomic-embed-text, 768d)
- 검색 latency: ~50ms (cosine sim over 12K vectors)
- 저장소: ~38MB (12,692 x 3,072 bytes)
- FTS5 검색: sub-millisecond
...
### discord_bot.py 아키텍처 (2026-07-05)

`discord_bot.py`는 Discord 메시지 → opencode 세션의 게이트웨이 봇.

**스트리밍 방식:**
- `opencode run --format json --thinking` 서브프로세스 → stdout JSON 라인을 읽어 실시간 Discord 업데이트
- **타이핑 heartbeat**: 8초마다 `trigger_typing()` → 무제한 대기 가능 (5분 타임아웃 없음)
- **이벤트 종류**: `reasoning`(🤔), `text`(✅ 최종 메시지), `tool_use`(🔧/📝/💻 등), `file`(📦), `error`(❌)
- `readline` buffer 1MB (기본 64KB — 초과 시 ValueError 크래시 방지)
- `proc.stdout` EOF까지 지속 → 서브프로세스 자연 종료 후 정리

**프로세스 관리:**
- 각 Discord 스레드 = 1개의 `asyncio.Task` (`_active_tasks[thread_id]`)
- 새 메시지가 오면 이전 task `cancel()` + 새로 spawn (메시지 수정/재요청 대응)
- `on_thread_update`(아카이브) / `on_thread_remove`(삭제) → task cancel + DB 세션 삭제
- 예외 처리: `CancelledError`/`Exception` 모두 `proc.kill()` 후 정리 (orphan 방지)

**discord-mcp MCP 서버:**
- `opencode serve`(launchd)의 자식 프로세스로 1개만 유지
- 새 opencode 세션 spawn마다 추가 프로세스 생성 방지 (watch: `pgrep -fl discord-mcp`로 중복 확인)
...
**Note:** Cross-session memory tools (`recall()`, `remember()`, `memory-stats()`) are provided by the `@drewgent/opencode-knowledge` plugin — SQLite FTS5 + Ollama vector search. No separate daemon needed.
...
Plugin `@drewgent/opencode-knowledge`가 설치된 환경에서는
[[skills/brain/opencode-knowledge-convention/SKILL.md]] 규칙을 반드시 따른다.
이 규칙들은 Tier 1 — 자동 수행. 확인 불필요.
