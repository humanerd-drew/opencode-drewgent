"""drewgent claw — OpenClaw migration commands.

Usage:
    drewgent claw migrate              # Interactive migration from ~/.openclaw
    drewgent claw migrate --dry-run    # Preview what would be migrated
    drewgent claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
    drewgent claw cleanup              # Archive leftover OpenClaw directories
    drewgent claw cleanup --dry-run    # Preview what would be archived
"""
...
from drewgent_cli.config import get_drewgent_home, get_config_path, load_config, save_config
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_drewgent.py script in known locations."""
...
def claw_command(args):
    """Route drewgent claw subcommands."""
    ...
    if action == "migrate":
        ...
        print("Usage: drewgent claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Drewgent")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'drewgent claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Drewgent migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: drewgent claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {drewgent_home}")
...
def _offer_source_archival(source_dir: Path, auto_yes: bool = False):
    ...
    if auto_yes or prompt_yes_no(f"Archive {source_dir} now?", default=True):
        ...
        print_info("Skipped. You can archive later with: drewgent claw cleanup")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  drewgent claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  drewgent claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  drewgent config set OPENROUTER_API_KEY sk-or-v1-...")
