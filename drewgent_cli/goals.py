"""Persistent session goals — autonomous cross-turn goal loop for Drewgent.

A goal is a free-form user objective that stays active across turns. After
each turn completes, a small judge call asks an auxiliary model "is this
goal satisfied by the assistant's last response?". If not, Drewgent feeds a
...
- When a real user message arrives mid-loop it preempts the continuation
  ...
  will say ``done``).
- This module has zero hard dependency on ``cli.DrewgentCLI`` or the gateway
...
# against small models that cannot follow the strict JSON reply contract.
...
def _get_session_db() -> Optional[Any]:
    """Return a SessionDB instance for the current DREWAGENT_HOME.

    SessionDB has no built-in singleton, so we cache one instance per
    home path so profile switches still pick up the right DB.
...
def _parse_judge_response(raw: str) -> Tuple[bool, str, bool]:
    ...
    auto-pause after N consecutive parse failures.
    """
    if not raw:
...
class GoalManager:
    ...
    def evaluate_after_turn(
        ...
        *,
        ...
        # Track consecutive judge parse failures.
        if parse_failed:
