...
Usage:
    drewgent webhook subscribe <name> [options]
    drewgent webhook list
    drewgent webhook remove <name>
    drewgent webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.drewgent/webhook_subscriptions.json and are
...
def _drewgent_home() -> Path:
    return Path(
        os.getenv("DREW_HOME", str(Path.home() / ".drewgent"))
    ).expanduser()


def _subscriptions_path() -> Path:
    return _drewgent_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: drewgent gateway run
"""
...
def webhook_command(args):
    ...
    if not sub:
        print("Usage: drewgent webhook {subscribe|list|remove|test}")
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (drewgent gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: drewgent webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from drewgent webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (drewgent gateway run)")
