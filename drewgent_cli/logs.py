"""``hermes logs`` — view and filter Drewgent log files.

Supports tailing, following, session filtering, level filtering, and
relative time ranges.  All log files live under ``~/.drewgent/logs/``.

Usage examples::

    drewgent logs                    # last 50 lines of agent.log
    drewgent logs -f                 # follow agent.log in real time
    drewgent logs errors             # last 50 lines of errors.log
    drewgent logs gateway -n 100     # last 100 lines of gateway.log
    drewgent logs --level WARNING    # only WARNING+ lines
    drewgent logs --session abc123   # filter by session ID substring
    drewgent logs --since 1h         # lines from the last hour
    drewgent logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_drewgent_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Drewgent runs — try 'hermes chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_drewgent_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_drewgent_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_drewgent_home()}/logs/")
        return

    print(f"Log files in {display_drewgent_home()}/logs/:\n")
    found = False
