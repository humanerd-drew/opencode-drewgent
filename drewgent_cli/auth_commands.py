...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_drewgent_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:drewgent_pkce",
    ...
    raise SystemExit(f"`drewgent auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    if removed.source.startswith("env:"):
        ...
        if env_var:
            from drewgent_cli.config import remove_env_value
            cleared = remove_env_value(env_var)
            if cleared:
                print(f"Cleared {env_var} from .env")

    # If this was a singleton-seeded credential (OAuth device_code, drewgent_pkce),
    ...
    elif removed.source == "drewgent_pkce" and provider == "anthropic":
        from drewgent_constants import get_drewgent_home
        oauth_file = get_drewgent_home() / ".anthropic_oauth.json"
        if oauth_file.exists():
            oauth_file.unlink()
            print("Cleared Drewgent Anthropic OAuth credentials")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `drewgent auth` is called bare."""
