...
from drewgent_cli.auth import get_nous_auth_status
from drewgent_cli.config import get_env_value, load_config
...
def get_nous_subscription_explainer_lines() -> list[str]:
    ...
    return [
        ...
        "Change these later with: drewgent setup tools, drewgent setup terminal, or drewgent status.",
    ]


def apply_nous_provider_defaults(config: Dict[str, object]) -> set[str]:
    """Apply provider-level Nous defaults shared by `drewgent setup` and `drewgent model`."""
    if not managed_nous_tools_enabled():
        return set()
