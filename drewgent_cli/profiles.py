"""
Profile management for multiple isolated Drewgent instances.

Each profile is a fully independent DREW_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.drewgent/profiles/<name>/`` by default.

The "default" profile is ``~/.drewgent`` itself — backward compatible,
zero migration needed.

Usage::

    drewgent profile create coder          # fresh profile + bundled skills
    drewgent profile create coder --clone  # also copy config, .env, SOUL.md
    drewgent profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    drewgent -p coder chat                 # or via flag
    drewgent profile use coder             # set as sticky default
    drewgent profile delete coder          # remove profile + alias + service
"""
...
# Directories/files to exclude when exporting the default (~/.drewgent) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "drewgent-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "drewgent", "default", "test", "tmp", "root", "sudo",
})

# Drewgent subcommands that cannot be used as profile names/aliases
...
def _get_profiles_root() -> Path:
    ...
    Always ``~/.drewgent/profiles/`` — anchored to the user's home,
    NOT to the current DREW_HOME (which may itself be a profile).
    This ensures ``coder profile list`` can see all profiles.
    """
    return Path.home() / ".drewgent" / "profiles"


def _get_default_drewgent_home() -> Path:
    """Return the default (pre-profile) DREW_HOME path."""
    return Path.home() / ".drewgent"


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_drewgent_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.drewgent
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its DREW_HOME directory."""
    if name == "default":
        return _get_default_drewgent_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, drewgent subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _HERMES_SUBCOMMANDS:
        return f"'{name}' conflicts with a drewgent subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_drewgent_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.drewgent)."
        )
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches DREW_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "DREW_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.drewgent).\n"
            "To remove everything, use: drewgent uninstall"
        )
    ...
    # Check for service
    from drewgent_cli.gateway import _profile_suffix, get_service_name
    wrapper_path = _get_wrapper_dir() / name
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set DREW_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("DREW_HOME")
    try:
        os.environ["DREW_HOME"] = str(profile_dir)
        from drewgent_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["DREW_HOME"] = old_home
        elif "DREW_HOME" in os.environ:
            del os.environ["DREW_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.drewgent/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: drewgent profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from DREW_HOME.

    Returns ``"default"`` if DREW_HOME is not set or points to ``~/.drewgent``.
    Returns the profile name if DREW_HOME points into ``~/.drewgent/profiles/<name>``.
    Returns ``"custom"`` if DREW_HOME is set to an unrecognized path.
    """
    from drewgent_constants import get_drewgent_home
    drewgent_home = get_drewgent_home()
    resolved = drewgent_home.resolve()

    default_resolved = _get_default_drewgent_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.drewgent itself — its parent is ~/ and its
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: drewgent profile import <archive> --name <name>"
        )

    # Archives exported from the default profile have "default/" as top-level
    # dir.  Importing as "default" would target ~/.drewgent itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.drewgent). "
            "Specify a different name: drewgent profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for drewgent profile names."""
    return '''# Drewgent Agent profile completion
...
_drewgent_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_drewgent_profiles)" -- "$cur"))
        return
    fi
...
complete -F _drewgent_completion hermes
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for drewgent profile names."""
    return '''#compdef hermes
# Drewgent Agent profile completion
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a DREW_HOME path string.

    Called early in the CLI entry point, before any drewgent modules
    are imported, to set the DREW_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: drewgent profile create {profile_name}"
        )
