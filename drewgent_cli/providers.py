"""
Single source of truth for provider identity in Drewgent Agent.
...
2. **Drewgent overlays** — transport type, auth patterns, aggregator flags,
...
# -- Drewgent overlay ----------------------------------------------------------
# Drewgent-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class DrewgentOverlay:
    """Drewgent-specific provider metadata layered on top of models.dev."""
...
HERMES_OVERLAYS: Dict[str, DrewgentOverlay] = {
    "openrouter": DrewgentOverlay(
        transport="openai_chat",
        is_aggregator=True,
        extra_env_vars=("OPENAI_API_KEY",),
        base_url_env_var="OPENROUTER_BASE_URL",
    ),
    "nous": DrewgentOverlay(
        transport="openai_chat",
        auth_type="oauth_device_code",
        base_url_override="https://inference-api.nousresearch.com/v1",
    ),
    "openai-codex": DrewgentOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "copilot-acp": DrewgentOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": DrewgentOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": DrewgentOverlay(
    ...
    "kimi-for-coding": DrewgentOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "minimax": DrewgentOverlay(
        transport="openai_chat",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-cn": DrewgentOverlay(
        transport="openai_chat",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": DrewgentOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": DrewgentOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "vercel": DrewgentOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": DrewgentOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": DrewgentOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_GO_BASE_URL",
    ),
    "kilo": DrewgentOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="KILOCODE_BASE_URL",
    ),
    "huggingface": DrewgentOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "drewgent", "user-config"
...
def get_overlay(provider_id: str) -> Optional[DrewgentOverlay]:
    """Get Drewgent overlay for a provider, if one exists."""
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Drewgent overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Drewgent overlay
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + drewgent extra
    ...
    if overlay is not None:
        # Drewgent-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
LABELS: Dict[str, str] = {
    ...
    # Legacy Drewgent IDs (point to same providers)
