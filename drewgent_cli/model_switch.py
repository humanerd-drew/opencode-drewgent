...
- ``drewgent_cli.providers``        -- canonical provider identity + overlays
- ``drewgent_cli.model_normalize``  -- per-provider name formatting
...
from drewgent_cli.providers import (
    ALIASES,
    LABELS,
...
from drewgent_cli.model_normalize import (
    detect_vendor,
...
def _check_drewgent_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* looks like a Drewgent LLM model."""
    if "drewgent" in model_name.lower():
        return _DREW_MODEL_WARNING
    return ""
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'drewgent model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'drewgent doctor' — config issues detected:"
    ...
    drewgent_warn = _check_drewgent_model_warning(new_model)
    if drewgent_warn:
        warnings.append(drewgent_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from drewgent_cli/models.py (OPENROUTER_MODELS,
    ...
    from drewgent_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS
    ...
    # Build curated model lists keyed by drewgent provider ID
    ...
    # --- 1. Check Drewgent-mapped providers ---
    for drewgent_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        model_ids = curated.get(drewgent_id, [])
        total = len(model_ids)
    ...
    # --- 2. Check Drewgent-only providers (nous, openai-codex, copilot) ---
    from drewgent_cli.providers import HERMES_OVERLAYS
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        if overlay.auth_type in ("oauth_device_code", "oauth_external", "external_process"):
            ...
            try:
                from drewgent_cli.auth import _read_auth_store
    ...
    if not cust_base or "openrouter.ai" in cust_base:
        return CustomAutoResult(
            ...
            error_message=(
                ...
                "in .env, or run: drewgent setup -> Custom OpenAI-compatible endpoint"
            ),
        )
