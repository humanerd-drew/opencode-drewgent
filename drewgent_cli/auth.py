"""
Multi-provider authentication system for Drewgent Agent.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.drewgent/auth.json with cross-process file locking.
...
from drewgent_cli.config import get_drewgent_home, get_config_path
...
DEFAULT_NOUS_CLIENT_ID = "drewgent-cli"
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `drewgent model` to re-authenticate."
...
# =============================================================================
# Auth Store — persistence layer for ~/.drewgent/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    return get_drewgent_home() / "auth.json"
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        ...
        lines = ["Config issue detected — run 'drewgent doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'drewgent model' for available providers, or run 'drewgent doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'drewgent model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.drewgent/.env.",
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.drewgent/auth.json (not ~/.codex/)
#
# Drewgent maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Drewgent auth store (~/.drewgent/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `drewgent auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `drewgent auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `drewgent auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `drewgent auth` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Drewgent auth store (~/.drewgent/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Drewgent auth state."""
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `drewgent auth` to re-authenticate."
            )
            relogin_required = True
        raise AuthError(
            message,
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Drewgent auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Drewgent's own Codex token store."""
    try:
        ...
        if cli_tokens:
            logger.info("Migrating Codex credentials from ~/.codex/ to Drewgent auth store")
            print("⚠️  Migrating Codex credentials to Drewgent's own auth store.")
            print("   This avoids conflicts with Codex CLI and VS Code.")
            print("   Run `drewgent auth` to create a fully independent session.\n")
    ...
    if should_refresh:
        # Re-read under lock to avoid racing with other Drewgent processes
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Drewgent models — they're not reliable for agentic tool-calling
            if "drewgent" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Drewgent is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    return refresh_nous_oauth_pure(
        ...
        state.get("client_id", "drewgent-cli"),
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Drewgent is not logged into Nous Portal.",
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for `drewgent status` output."""
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `drewgent auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `drewgent auth` and
    # `drewgent model` store device_code tokens.
    try:
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("DREW_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("DREW_COPILOT_ACP_ARGS", "").strip()
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set DREW_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def detect_external_credentials() -> List[Dict[str, Any]]:
    """Scan for credentials from other CLI tools that Drewgent can reuse.

    Returns a list of dicts, each with:
      - provider: str   -- Drewgent provider id (e.g. "openai-codex")
    ...
    if cli_tokens:
        ...
        found.append({
            "provider": "openai-codex",
            "path": str(codex_path),
            "label": f"Codex CLI credentials found ({codex_path}) — run `drewgent auth` to create a separate session",
        })
...
def _prompt_model_selection(
    ...
    """
    from drewgent_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from drewgent_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'drewgent model' or 'drewgent setup' instead."""
    print("The 'hermes login' command has been removed.")
    print("Use 'drewgent auth' to manage credentials,")
    print("'drewgent model' to select a provider, or 'drewgent setup' for full setup.")
    raise SystemExit(0)


def _login_openai_codex(args, pconfig: ProviderConfig) -> None:
    """OpenAI Codex login via device code flow. Tokens stored in ~/.drewgent/auth.json."""

    # Check for existing Drewgent-owned credentials
    try:
        ...
        if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
            print("Existing Codex credentials found in Drewgent auth store.")
            try:
    ...
    if cli_tokens:
        ...
        print("Drewgent will create its own session to avoid conflicts with Codex CLI / VS Code.")
        try:
        ...
        if do_import in ("y", "yes"):
            ...
            print("Drewgent will keep working independently with its own session.")
            print(f"  Config updated: {config_path} (model.provider=openai-codex)")
            return

    # Run a fresh device code flow — Drewgent gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Drewgent creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Drewgent auth store
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Drewgent login via {pconfig.name}...")
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `drewgent model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target):
        ...
        if os.getenv("OPENROUTER_API_KEY"):
            print("Drewgent will use OpenRouter for inference.")
        else:
            print("Run `drewgent model` or configure an API key to use Drewgent.")
    else:
