...
| # | 단계 | 명령어 / 설명 | 필수 |
|---|------|--------------|------|
| 1 | 셋업 | `bash scripts/setup.sh` — 의존성 설치, .env 생성, 환경 확인 | ✅ |
| 2 | API 키 입력 | `~/.drewgent/.env` 편집 — `OPENCODE_API_KEY`(필수), `DISCORD_BOT_TOKEN`(선택), `OPENAI_API_KEY`(선택) | ✅ |
| 3 | 이름 변경 (선택) | `skill("rename-drewgent")` — 모든 `drewgent` → `<yourname>gent` | 🔄 |
| 4 | 검증 | `bash scripts/bridge-lint.sh` — 태그 규정 준수 확인 | 🔄 |
| 5 | launchd 설치 (macOS) | launchd 템플릿을 ~/Library/LaunchAgents/로 복사 + launchctl load — cron, opencode serve, Discord 봇 데몬 활성화 | 🔄 |
| 6 | opencode 실행 | `opencode` | ✅ |

**안 되나요?** 아래 [트러블슈팅](#트러블슈팅) 참조. API 키 누락 시 `~/.drewgent/.env` 확인.
...
├── launchd/                    launchd plist 템플릿 (macOS 데몬)
