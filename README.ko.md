...
1. **`/rename "내 에이전트"`** — 템플릿 전체의 에이전트 이름을 변경합니다.
2. **`/connect`** — LLM 제공자를 설정합니다 (opencode 무료 티어로 바로 시작 가능).
3. **그냥 사용하세요** — `"내 프로젝트에 로그인 기능을 추가해줘"` 또는 `"이 구조 설명해줘"` 같은 걸 말해보세요.
...
백그라운드 서비스 등록은 `services/` 디렉토리를 참고하세요 (macOS → launchd, Linux → systemd, Windows → Scheduled Tasks).
...
파일: `scripts/discord_bot.py`, `scripts/discord_send.py`, `services/` (플랫폼별 서비스 설정)

---

## 플랫폼 서비스

에이전트를 항상 사용 가능하게 유지하려면 백그라운드 서비스 등록이 필요합니다:
...
- **플랫폼 서비스**: macOS → `services/launchd/`, Linux → `services/systemd/`, Windows → `services/windows/` 참고
